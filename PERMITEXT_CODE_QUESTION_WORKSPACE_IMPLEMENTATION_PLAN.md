...
**First incremental migration slice (August 9, 2026):** Commits `13cc5d5e6` and `2f1b509da` remove the stage control from the ordinary path, relabel the index as Code Decisions, open Permitext's existing persisted Research workspace as the primary surface, place a derived Code Decision record beside it, simplify visible states, and make **Create Code Memo** the primary issuance action. They deliberately reuse the existing Research and internal Code Question stores. Server-derived list summaries prevent unopened records from being mislabeled by stale detail caches; until a durable Research Conversation ↔ Code Decision link exists, selecting an unlinked saved conversation clears the active decision context rather than implying a relationship. Contract/check/smoke suites and a signed-in rendered localhost flow pass for this slice. Automatic conversation-to-governed-record extraction, the durable conversation/decision link, lightweight in-conversation acceptance, and the save/finalize interaction remain open; this slice does not complete Phase 5.

**Second incremental migration slice (August 9, 2026):** Commit `b973356dd` adds a durable actor-private Research Conversation ↔ shared Code Decision association without creating another conversation or governed-record store. It deterministically starts or resumes the exact actor/Project/decision conversation, reopens the linked Research and Code Decision together, preserves unlinked-conversation separation, and lets an authorized professional explicitly capture a user-authored Research message as a confirmed fact, proposed assumption, or proposed missing-information Question Input through the existing authoritative input command. Ordinary Research now refuses generation until at least one enacted-code selection is attached. This guard does not convert a Research selection into approved Code Decision evidence: ordinary Research selections and answers remain exploratory, are not entries in an immutable approved Evidence Set, and are not governed question-bound analyses. Linked ordinary Research answers preserve an immutable decision-context snapshot (Project, Code Decision, definition revision/hash, and capture time) so a later relink does not rewrite their provenance; that snapshot still does not make the answer governed question-bound analysis. The actor-private one-current-conversation-per-Project/Code-Decision invariant is implemented for Postgres with a partial unique index and serializable, CAS-guarded replacement and removal transactions; storage conflicts return `CODE_QUESTION_RESEARCH_LINK_CONFLICT`. This is not a Project-global shared-conversation invariant. Local automated verification is complete: `npm run test:code-question`, `npm run test:offline`, `npm run check`, `npm run smoke`, and `git diff --check` pass. A signed-in rendered localhost scenario also passes exact linked reconstruction, fact/assumption/missing-information capture, reload, sign-out teardown, sign-in restoration, feature gating, and a clean runtime log. The HTTP suite covers deterministic start/replay, actor privacy, explicit versioned link/relink/unlink/replacement, concurrent one-current replacement/removal, immutable Research-context history, the ordinary-Research/governed-artifact boundary, and server-validated capture provenance/disposition. A configured Postgres integration run, cold-reload offline/device/mixed-client proof, push, deployment, and Production verification are not yet recorded. Automatic extraction/proposal generation, governed candidate-evidence acceptance/rejection into Evidence Set versions, conversational invocation of question-bound analysis, Professional Conclusion save/finalize/**Mark Final**, contextual Review and end-to-end Code Memo UX, and extraction of the new orchestration from the monolithic web client remain open. This slice does not complete Phase 5 or change any rollout gate.
...
| 2 — Project, Research, and Code Decision shell | Second conversational migration slice present; further integration open | `a56e51079` + Phase 5A integration + `13cc5d5e6` + `2f1b509da` + `b973356dd` | `test:code-question`, `test:offline`, `check`, `smoke`, and `git diff --check` pass; signed-in rendered localhost proof passes for the second slice | Pane identity, Project/decision isolation, hydration, and account-scoped cache behavior remain valid. The second slice adds actor-private durable pairing, exact start/resume, explicit versioned relink/unlink/replacement, and atomic Postgres enforcement of one current conversation per actor/Project/decision. Unlinked saved Research still cannot remain visually paired with an active decision. Old stage helpers remain only for migration/deep-link compatibility. Cold-reload offline, configured Postgres execution, device, and mixed-client proof remain open. |
| 3 — Governed Question and Project Inputs | Trust layer complete; explicit message capture added; automatic extraction open | `fcac63182` + Phase 5A integration + `b973356dd` | Existing Define/client-state contracts plus linked-Research/input-capture HTTP coverage; `test:code-question`, `test:offline`, `check`, and `smoke` pass; rendered fact/assumption/missing-information capture passes | Authorized users can explicitly classify and capture a user-authored Research message through the existing Question Input command using only canonical state combinations and server-derived provenance. Automatic proposals/extraction, assistant-output proposals, broader disposition UX, and explicit Professional Conclusion save/finalize remain open. Manual Define-stage presentation must continue to be repurposed into the Code Decision. |
| 4 — Conversational evidence discovery and approval | Trust layer complete; governed conversational acceptance still open | `8608305a8` + Phase 5A integration | Evidence/client-state/HTTP contracts; full suites; prior active-Project Saved candidate import | Approval, immutable snapshots/sets, and isolation remain valid underneath. Research selections may support exploratory Research but are not approved Code Decision evidence. Candidate/Reader/tray discovery must become contextual Research UI with explicit acceptance/rejection through the existing Evidence Set commands. |
| 5 — Conversational Research and Code Decision | **Redesign in progress; durable pairing and explicit input capture present; trust layer complete** | `11e129cb8` + `4bca8b2a7` + `13cc5d5e6` + `2f1b509da` + `b973356dd` | Existing analysis/server-adapter/HTTP contracts prove the underlying governed path; `test:code-question`, `test:offline`, `check`, and `smoke` pass for the second slice; signed-in rendered localhost proof passes | Research is the primary surface and now pairs durably with the caller's exact private conversation while the shared Code Decision remains canonical. The one-current-link guarantee is per actor, Project, and Code Decision; the shared record does not expose or merge members' private Research conversations. Ordinary Research enforces attached selected evidence, but its selections/answers are exploratory rather than approved Evidence Set/question-bound analysis. Automatic extraction/proposals, governed evidence acceptance, conversational question-bound analysis, Professional Conclusion save/finalize/Mark Final, contextual Review/Code Memo completion, and modular client extraction remain open. |
...
- Current working branch: `main`; the verified code commit leaves it eight commits ahead of `origin/main` before this plan-only ledger commit. The historical baseline before the August 9 conversational migration was `b9b8aa1fa7`, four commits ahead of `origin/main`.
...
- The first August 9 conversational Code Decision migration slice is committed in `13cc5d5e6`, with unlinked Research/decision context separation and server-summary cache hardening in `2f1b509da`. Continue incrementally without creating a second Research or governed-record store.
- The second slice is committed in `b973356dd`. It adds an actor-private durable Research Conversation ↔ shared Code Decision association, deterministic exact start/resume, conditional linked coexistence, explicit versioned relink/unlink/replacement, immutable decision-context snapshots for ordinary Research answers, explicit user-message capture through existing Question Input commands, an ordinary-Research evidence-required guard, and a partial-unique-index plus serializable/CAS Postgres replacement/removal path.
- Keep the terminology boundary explicit: attached/selected Research passages and immutable ordinary Research answers remain exploratory. They do not become approved Evidence Set entries or governed question-bound analysis merely because the conversation is linked to a Code Decision.
...
- The existing question-bound analysis path remains delivered: it binds exact server-owned versions and approved evidence, returns structured limitations/citations/assumptions/missing facts/conflicts, and uses request-bound idempotency and staleness checks. The existing Professional Conclusion remains separate, revisioned, attributable, and dependency/citation-bounded. This second slice does not invoke governed analysis from ordinary Research or implement Professional Conclusion save/finalize/Mark Final; retry/current-predecessor hardening required for that finalization UX remains part of a later slice.
...
- Phase 5A is complete in current history. The second Phase 5 slice implements durable actor-private linking, explicit versioned relink/unlink/replacement, immutable ordinary-Research decision-context snapshots, Postgres one-current-link enforcement, and explicit user-message-to-input capture, but automatic extraction/proposals, governed evidence acceptance/rejection, conversational question-bound analysis, Professional Conclusion save/finalize/Mark Final, contextual Review and end-to-end Code Memo UX, and modular extraction from `public/app.js` remain open. Configured Postgres execution and cold-reload offline/device/mixed-client proof are not yet recorded. Phase 9 device/mutation gates and Phase 10 external gates still require permissioned professional pilots, final policy/rights/retention/full-accessibility sign-off, deliberate push and deployment, active Production client/cache verification, a real signed-in Production lifecycle, and deployed rollback rehearsal. Capability remains default-disabled.
