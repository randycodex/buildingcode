...
export function definitionsForReader(registry, {bundle, codeSectionID, chapterNumber, chapterID, sectionNumber, includeSectionScoped = false}) {
  ...
  if (registry.books.some(book => book.excludeWholeChapter !== false && book.bundle === bundle && String(book.codeSectionID) === String(codeSectionID)
    && String(book.definitionChapter).toUpperCase() === chapter
    && (!Number.isSafeInteger(chapterID) || book.chapterID == null || book.chapterID === chapterID))) return [];
  const selected = registry.books.filter(book => book.bundle === bundle && String(book.codeSectionID) === String(codeSectionID)
    ...
    .flatMap(book => book.entries.filter(entry => entry.applicability === 'definition-chapter'
      ...
      && (!entry.applicableChapterIDs || (Number.isSafeInteger(chapterID) && entry.applicableChapterIDs.includes(chapterID)))
