import { definitionAppliesToSection } from './definition-matcher.js?v=20260916-definitions-v47';
import { definitionBundleID, definitionsForReader } from './reader-definition-registry.js?v=20260916-definitions-v47';
import { installDefinitionLinks } from './reader-definition-popover.js?v=20260916-definitions-v47';

const contexts=new WeakMap();
let registryPromise;
function loadRegistry() {
  if(!registryPromise)registryPromise=fetch('/web/reader-definition-registry.json?v=20260916-definitions-v47')
...
export function decorateReaderDefinitions(root,reader) {
  ...
  void loadRegistry().then(registry=>{
    ...
    const hasSectionScopes=entries.some(entry=>entry.applicableSections||entry.excludedSections);
    for(const block of root.querySelectorAll('.annotated-code-block > :first-child')) {
      ...
      for(const paragraph of prose.length?prose:[block])installDefinitionLinks(paragraph,scopedEntries);
    }
