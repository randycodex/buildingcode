import { definitionAppliesToSection } from './definition-matcher.js?v=20260917-definitions-v81';
import { definitionBundleID, definitionsForReader } from './reader-definition-registry.js?v=20260917-definitions-v81';
import { installDefinitionLinks } from './reader-definition-popover.js?v=20260917-definitions-v81';

const contexts=new WeakMap();
let registryPromise;
function loadRegistry() {
  if(!registryPromise)registryPromise=fetch('/web/reader-definition-registry.json?v=20260917-definitions-v81')
...
export function decorateReaderDefinitions(root,reader) {
  ...
  void loadRegistry().then(registry=>{
    ...
    const hasSectionScopes=entries.some(entry=>entry.applicableSections||entry.applicableExactSections||entry.excludedSections||entry.excludedExactSections||entry.excludedOccurrences);
