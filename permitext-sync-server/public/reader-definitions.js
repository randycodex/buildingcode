import { definitionAppliesToSection } from './definition-matcher.js?v=20260916-definitions-v69';
import { definitionBundleID, definitionsForReader } from './reader-definition-registry.js?v=20260916-definitions-v69';
import { installDefinitionLinks } from './reader-definition-popover.js?v=20260916-definitions-v69';

const contexts=new WeakMap();
let registryPromise;
function loadRegistry() {
  if(!registryPromise)registryPromise=fetch('/web/reader-definition-registry.json?v=20260916-definitions-v69')
...
export function decorateReaderDefinitions(root,reader) {
  ...
  void loadRegistry().then(registry=>{
    ...
    const hasSectionScopes=entries.some(entry=>entry.applicableSections||entry.excludedSections||entry.excludedOccurrences);
