import { createHash } from "node:crypto";
import { spawnSync } from "node:child_process";
import { readFile } from "node:fs/promises";
import { resolve } from "node:path";
import { fileURLToPath, pathToFileURL } from "node:url";
import {
  requireActiveZoningSuccessorPaidAuthorization,
  validateZoningSuccessorPaidAuthorization
} from "./zoning-successor-paid-authorization.mjs";
import {
  validateZoningRemediationSuccessor3V11ConfirmationPaidAuthorization,
  zoningRemediationSuccessor3V11ConfirmationAuthorizationPackageCommit,
  zoningRemediationSuccessor3V11ConfirmationConsumedAuthorizationSHA256,
  zoningRemediationSuccessor3V11ConfirmationExecutionCommit,
  zoningRemediationSuccessor3V11ConfirmationResultJSONFile,
  zoningRemediationSuccessor3V11ConfirmationResultJSONSHA256,
  zoningRemediationSuccessor3V11ConfirmationResultMarkdownFile,
  zoningRemediationSuccessor3V11ConfirmationResultMarkdownSHA256,
  zoningRemediationSuccessor3V11ConfirmationRunID
} from "./zoning-successor-remediation-3-v11-confirmation-paid-authorization.mjs";
import {
  zoningV11RunnerHandoffProtocol,
  zoningV11RunnerPublicKeyDERBase64
} from "./zoning-v11-paid-runner-handoff.mjs";

const defaultAuthorizationPath = fileURLToPath(new URL(
  "./zoning-successor-remediation-3-v12-confirmation-paid-authorization.json",
  import.meta.url
));
export const zoningRemediationSuccessor3V12ConfirmationLockedAuthorizationSHA256 =
  "7b3344947a454ed8821ae6b5edacc3d62b664ec678bee368b516ec6a19418174";
export const zoningRemediationSuccessor3V12ConfirmationAuthorizationPackageCommit =
  "67fbd6ca25d69b9f59d07dfb3b556ca16d134b39";
export const zoningRemediationSuccessor3V12ConfirmationExecutionCommit =
  "bf13a7128edc0dc9d53c62611eaa660a35e0cf73";
export const zoningRemediationSuccessor3V12ConfirmationConsumedAuthorizationSHA256 =
  "b6a7fdbb00f5a7b7f587cb3e9557fe2d673464df69962e4afd01a1df79c2af48";
export const zoningRemediationSuccessor3V12ConfirmationRunID =
  "6e370831-82c1-4480-9253-2ea8ceb908ec";
export const zoningRemediationSuccessor3V12ConfirmationResultJSONFile =
  "results/2026-08-31T22-37-43-979Z-6e370831-82c1-4480-9253-2ea8ceb908ec.json";
export const zoningRemediationSuccessor3V12ConfirmationResultJSONSHA256 =
  "9491fb2c50cddabe0592359453721ec6036218538181132c5099ac0abeb34cbb";
export const zoningRemediationSuccessor3V12ConfirmationResultMarkdownFile =
  "results/2026-08-31T22-37-43-979Z-6e370831-82c1-4480-9253-2ea8ceb908ec.md";
export const zoningRemediationSuccessor3V12ConfirmationResultMarkdownSHA256 =
  "772b65bd26a291ee9ea649162a73c48262ac26b10f73ff97807958e0c8f85429";
export const zoningRemediationSuccessor3V12ConfirmationPreparedFromCommit =
  "a96b922fadc2e8bea67decdd2b09cc9166dcc40a";
export const zoningRemediationSuccessor3V12ConfirmationSafetySHA256 =
  "f1a5edf73ae02fde6f77456d6b3bd59255d3d561a632055f12c3b653ff53f224";
export const zoningRemediationSuccessor3V12ConfirmationEconomicsSHA256 =
  "d4816da6162137e122355494a3f2954dca09fc9d8978b85eb682516d29ec5ae0";
export const zoningRemediationSuccessor3V12ConfirmationAppSHA256 =
  "1b907f5db72f65248489b80801904a2011b2df91ce5d739a7e6dc39cce702797";
export const zoningRemediationSuccessor3V12ConfirmationRunnerHandoffSHA256 =
  "e45975a2d028d5d9852032fe6c107aacf0d3e7d18586ba41ae7eac4a2b4df327";
export const zoningRemediationSuccessor3V12ConfirmationRunnerPublicKeySHA256 =
  "7830127ce97437dcb85971faecfac4ad031288d4f98608837fa5c22aa2c64918";

const expectedAuthorizationID = "31dd8aa9-3b2c-4fc8-8988-1b542735ac78";
const expectedCohortFile =
  "zoning-cases-expanded-batch-1-successor-remediation-3.json";
const expectedCohortSHA256 =
  "852e521f427a418eb18c1bd45e3e764736ae50cbb09d0d0a46ce64f8cad893fc";
const expectedSafetyVersion = "20260831-zoning-material-completeness-v12";
const expectedPriorAuthorizationFile =
  "zoning-successor-remediation-3-v11-confirmation-paid-authorization.json";
const repositoryRoot = fileURLToPath(new URL("../../", import.meta.url));

function sha256(value) {
  return createHash("sha256").update(value).digest("hex");
}

function assert(condition, message) {
  if (!condition) throw new Error(message);
}

async function assertFileHash(url, expectedHash, message) {
  assert(sha256(await readFile(fileURLToPath(url), "utf8")) === expectedHash,
    message);
}

function assertHistoricalFileHash(relativePath, expectedHash, message) {
  const blob = spawnSync("git", [
    "show",
    `${zoningRemediationSuccessor3V12ConfirmationPreparedFromCommit}:${relativePath}`
  ], {
    cwd: repositoryRoot,
    encoding: "utf8",
    maxBuffer: 4 * 1024 * 1024
  });
  assert(blob.status === 0,
    `Unable to read the reviewed v12 historical bytes for ${relativePath}.`);
  assert(sha256(blob.stdout) === expectedHash, message);
}

function validateHistoricalReviewedInputs() {
  assertHistoricalFileHash(
    "permitext-sync-server/research-zoning-safety.mjs",
    zoningRemediationSuccessor3V12ConfirmationSafetySHA256,
    "The reviewed v12 historical Zoning safety bytes changed."
  );
  assertHistoricalFileHash(
    "permitext-sync-server/research-economics.mjs",
    zoningRemediationSuccessor3V12ConfirmationEconomicsSHA256,
    "The reviewed v12 historical Research economics bytes changed."
  );
  assertHistoricalFileHash(
    "permitext-sync-server/app.mjs",
    zoningRemediationSuccessor3V12ConfirmationAppSHA256,
    "The reviewed v12 historical application bytes changed."
  );
}

async function validateRunnerHandoffInputs(authorization) {
  assert(authorization.lineage?.runnerHandoffProtocol ===
    zoningV11RunnerHandoffProtocol,
  "The v12 package names the wrong signed runner handoff protocol.");
  assert(authorization.lineage?.runnerHandoffSHA256 ===
    zoningRemediationSuccessor3V12ConfirmationRunnerHandoffSHA256,
  "The v12 package names the wrong signed runner handoff SHA.");
  assert(authorization.lineage?.runnerHandoffPublicKeySHA256 ===
    zoningRemediationSuccessor3V12ConfirmationRunnerPublicKeySHA256,
  "The v12 package names the wrong signed runner public-key SHA.");
  await assertFileHash(
    new URL("./zoning-v11-paid-runner-handoff.mjs", import.meta.url),
    zoningRemediationSuccessor3V12ConfirmationRunnerHandoffSHA256,
    "The signed v12 runner handoff implementation changed."
  );
  assert(
    sha256(Buffer.from(zoningV11RunnerPublicKeyDERBase64, "base64")) ===
      zoningRemediationSuccessor3V12ConfirmationRunnerPublicKeySHA256,
    "The signed v12 runner handoff public key changed."
  );
}

async function validateHistoricalV11Lineage(authorization) {
  const lineage = authorization.lineage;
  assert(lineage?.priorAuthorizationFile === expectedPriorAuthorizationFile,
    "The v12 package names the wrong historical v11 authorization.");
  assert(lineage?.priorAuthorizationSHA256 ===
    zoningRemediationSuccessor3V11ConfirmationConsumedAuthorizationSHA256,
  "The v12 package names the wrong historical v11 authorization SHA.");
  assert(lineage?.priorAuthorizationPackageCommit ===
    zoningRemediationSuccessor3V11ConfirmationAuthorizationPackageCommit,
  "The v12 package names the wrong historical v11 package commit.");
  assert(lineage?.priorExecutionCommit ===
    zoningRemediationSuccessor3V11ConfirmationExecutionCommit,
  "The v12 package names the wrong historical v11 execution commit.");
  assert(lineage?.priorRunID === zoningRemediationSuccessor3V11ConfirmationRunID,
    "The v12 package names the wrong historical v11 run.");

  await assertFileHash(
    new URL(`./${expectedPriorAuthorizationFile}`, import.meta.url),
    zoningRemediationSuccessor3V11ConfirmationConsumedAuthorizationSHA256,
    "The consumed historical v11 authorization changed."
  );
  await validateZoningRemediationSuccessor3V11ConfirmationPaidAuthorization();

  for (const [file, expectedHash, label] of [
    [zoningRemediationSuccessor3V11ConfirmationResultJSONFile,
      zoningRemediationSuccessor3V11ConfirmationResultJSONSHA256, "JSON"],
    [zoningRemediationSuccessor3V11ConfirmationResultMarkdownFile,
      zoningRemediationSuccessor3V11ConfirmationResultMarkdownSHA256,
      "Markdown"]
  ]) {
    assert(lineage?.[`priorResult${label}File`] === file,
      `The v12 package names the wrong historical v11 ${label} result.`);
    assert(lineage?.[`priorResult${label}SHA256`] === expectedHash,
      `The v12 package names the wrong historical v11 ${label} result SHA.`);
    await assertFileHash(
      new URL(`./${file}`, import.meta.url),
      expectedHash,
      `The retained historical v11 ${label} result changed.`
    );
  }
}

export async function validateZoningRemediationSuccessor3V12ConfirmationPaidAuthorization({
  authorizationPath = defaultAuthorizationPath
} = {}) {
  const validation = await validateZoningSuccessorPaidAuthorization({
    authorizationPath
  });
  const authorization = validation.authorization;
  assert(authorization.authorizationID === expectedAuthorizationID,
    "The v12 confirmation authorization has the wrong unique identity.");
  assert(authorization.cohort.file === expectedCohortFile,
    "The v12 confirmation authorization names the wrong cohort file.");
  assert(authorization.cohort.sha256 === expectedCohortSHA256,
    "The v12 confirmation authorization names the wrong cohort SHA.");
  assert(authorization.lineage?.preparedFromCommit ===
    zoningRemediationSuccessor3V12ConfirmationPreparedFromCommit,
  "The v12 confirmation package is not bound to the reviewed repair commit.");
  assert(authorization.lineage?.zoningSafetyVersion === expectedSafetyVersion,
    "The v12 confirmation package names the wrong Zoning safety version.");
  assert(authorization.lineage?.zoningSafetySHA256 ===
    zoningRemediationSuccessor3V12ConfirmationSafetySHA256,
  "The v12 confirmation package names the wrong Zoning safety SHA.");
  assert(authorization.lineage?.researchEconomicsSHA256 ===
    zoningRemediationSuccessor3V12ConfirmationEconomicsSHA256,
  "The v12 confirmation package names the wrong Research economics SHA.");
  assert(authorization.lineage?.appSHA256 ===
    zoningRemediationSuccessor3V12ConfirmationAppSHA256,
  "The v12 confirmation package names the wrong application SHA.");
  validateHistoricalReviewedInputs();
  await validateRunnerHandoffInputs(authorization);
  await validateHistoricalV11Lineage(authorization);

  assert(authorization.execution?.webSupportEnabled === false,
    "The v12 confirmation package may not enable unbudgeted web search.");
  assert(authorization.execution?.stopOnExecutionError === true,
    "The v12 confirmation package must stop on its first execution error.");
  const exactSpendingCapPhrase =
    authorization.ownerDecision?.exactSpendingCapPhrase;
  const authorizationPackageCommit =
    authorization.execution?.authorizationPackageCommit;
  const executionCommit = authorization.execution?.executionCommit;
  if (authorization.status === "locked") {
    assert(exactSpendingCapPhrase === null,
      "Locked v12 confirmation authorization may not record a spending-cap phrase.");
    assert(authorizationPackageCommit === null,
      "Locked v12 confirmation authorization may not name an authorized package commit.");
    assert(executionCommit === null,
      "Locked v12 confirmation authorization may not name an execution commit.");
  } else {
    assert(/^[0-9a-f]{40}$/i.test(authorizationPackageCommit || ""),
      "V12 confirmation authorization must bind the exact committed locked package.");
    assert(authorization.scope?.caseCount === 30 &&
      authorization.scope?.repetitions === 1 &&
      authorization.scope?.maximumCumulativeSpendUSD === 5,
    "V12 confirmation authorization must retain the exact 30-case, one-repetition, $5 scope.");
    const expectedOwnerPhrase =
      `authorize exactly package commit ${authorizationPackageCommit} for all 30 ` +
      "ordered cases, one repetition, with a maximum cumulative API spend of $5.";
    assert(authorization.ownerDecision?.exactAuthorizationPhrase ===
      expectedOwnerPhrase,
    "V12 confirmation authorization must bind the owner's exact phrase to the selected package and scope.");
    assert(exactSpendingCapPhrase === expectedOwnerPhrase,
      "V12 confirmation authorization must retain the same exact package-bound spending-cap phrase.");
  }
  if (authorization.status === "authorized") {
    assert(executionCommit === null,
      "Authorized v12 confirmation may not name an execution commit before dispatch.");
  }
  if (["running", "consumed"].includes(authorization.status)) {
    assert(typeof authorization.consumption?.attemptID === "string" &&
      authorization.consumption.attemptID.length > 0 &&
      typeof authorization.consumption?.startedAt === "string" &&
      authorization.consumption.startedAt.length > 0,
    "V12 confirmation execution must retain its durable attempt identity.");
    assert(/^[0-9a-f]{40}$/i.test(executionCommit || ""),
      "V12 confirmation execution must retain its exact clean execution commit.");
  }
  if (authorization.status === "consumed") {
    assert(authorization.consumption.attemptID === authorization.consumption.runID,
      "The consumed v12 confirmation result must match its pre-dispatch attempt identity.");
    assert(authorizationPackageCommit ===
      zoningRemediationSuccessor3V12ConfirmationAuthorizationPackageCommit,
    "The consumed v12 confirmation must retain its exact locked package commit.");
    assert(executionCommit ===
      zoningRemediationSuccessor3V12ConfirmationExecutionCommit,
    "The consumed v12 confirmation must retain its exact execution commit.");
    assert(authorization.consumption.runID ===
      zoningRemediationSuccessor3V12ConfirmationRunID,
    "The consumed v12 confirmation must retain its exact run ID.");
    for (const [file, expectedHash, label] of [
      [zoningRemediationSuccessor3V12ConfirmationResultJSONFile,
        zoningRemediationSuccessor3V12ConfirmationResultJSONSHA256, "JSON"],
      [zoningRemediationSuccessor3V12ConfirmationResultMarkdownFile,
        zoningRemediationSuccessor3V12ConfirmationResultMarkdownSHA256,
        "Markdown"]
    ]) {
      await assertFileHash(
        new URL(`./${file}`, import.meta.url),
        expectedHash,
        `The retained v12 ${label} result changed.`
      );
    }
  }
  return validation;
}

export function requireActiveZoningRemediationSuccessor3V12ConfirmationPaidAuthorization(
  validation
) {
  return requireActiveZoningSuccessorPaidAuthorization(validation);
}

if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  const validation =
    await validateZoningRemediationSuccessor3V12ConfirmationPaidAuthorization();
  if (process.argv.includes("--require-active")) {
    requireActiveZoningRemediationSuccessor3V12ConfirmationPaidAuthorization(
      validation
    );
  }
  console.log("Zoning remediation-successor-3 v12 confirmation authorization guard passed", {
    status: validation.authorization.status,
    cohortCases: validation.cohort.cases.length,
    cohortSHA256: validation.authorization.cohort.sha256,
    active: validation.active,
    authorizationPackageCommit:
      validation.authorization.execution.authorizationPackageCommit,
    maximumCumulativeSpendUSD:
      validation.authorization.scope.maximumCumulativeSpendUSD,
    publicResearchReleaseAuthorized:
      validation.authorization.publicResearchReleaseAuthorized
  });
}
