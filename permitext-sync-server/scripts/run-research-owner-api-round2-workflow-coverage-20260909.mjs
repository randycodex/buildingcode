// Confirm repaired source coverage and prompt v6 on two unchanged workflow questions once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-guidance-precision-2026-09-09.json";
...
assert.equal(roundPreviousConservativeUSD, 7.519989);
const profile = { promptVersion: "20260909-document-summary-v6", schema: "permitext-owner-api-round2-workflow-coverage-v1", sourceCommit,
  authorization: "Owner authorized continued API tests within a fresh $8 round, with notice before more is needed. Historical costs remain separate. This single-use confirmation tests the repaired stakeholder source coverage and subsequent-filing qualifications under guidance prompt v6 on unchanged authored DOBNOW-023 and DOBNOW-003 scenarios and questions, once each. The preceding conservative round total is $7.519989, leaving $0.480011. Maximum $0.40 for two unassigned Research HTTP turns, at most $0.20 and two provider requests per turn, four provider requests in total, no paid search, manual retry, separate judge, Project workflow or deployment.",
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner workflow coverage confirmation" } });
