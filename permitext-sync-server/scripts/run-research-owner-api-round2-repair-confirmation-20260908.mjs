// New owner-authorized API round: confirm five recent repairs after recorded citation and answer-scope failures through isolated Research HTTP.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-confirmation-2026-09-08.json";
const firstRoundBytes = await readFile(new URL(firstRoundFile, root));
const firstRound = JSON.parse(firstRoundBytes);
assert.equal(firstRound.status, "stopped");
assert.equal(firstRound.providerCalls.length, 3);
...
assert.equal(roundPreviousConservativeUSD, 0.124828);
...
const caseIDs = ["ZR-18", "MC-01", "PC-04", "PC-10", "MC-15"];
assert(caseIDs.every((id) => attempted.has(id)), "The initial batch confirms previously attempted cases after offline repairs.");
const profile = { schema: "permitext-owner-api-round2-repair-confirmation-v1", sourceCommit,
  authorization: "Owner explicitly authorized fresh API tests with $8 currently available and requested notice if more is needed. This is a new $8 round; historical costs stay separate. This single-use follow-up batch tests the scoped drafting repair plus the three previously skipped cases, with a $2.50 batch ceiling after $0.124828 conservatively recorded in the first batch. No Project workflow tests or deployment.",
  ...
  priorRoundResults: [{ file: firstRoundFile, sha256: hash(firstRoundBytes) }],
  ownerDatasetSHA256: hash(ownerBytes), roundID: "owner-api-round2-20260908",
  maximumCumulativeSpendUSD: 2.5, maximumTurnSpendUSD: 0.5, maximumTurns: 5, maximumProviderRequests: 25, maximumProviderRequestsPerTurn: 5,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-repair-confirmation-preflight-2026-09-08.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-repair-confirmation-2026-09-08.json", root);
const sourceFiles = ["app.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-repair-confirmation-20260908.mjs"];
