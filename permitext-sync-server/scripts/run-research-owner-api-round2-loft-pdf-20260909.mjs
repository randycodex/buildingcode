// Confirm the repaired official PDF path on authored DOBNOW-016 once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-mapped-review-2026-09-09.json";
...
assert.equal(roundPreviousConservativeUSD, 7.468798);
const profile = { schema: "permitext-owner-api-round2-loft-pdf-v1", sourceCommit,
  authorization: "Owner authorized continued API tests within a fresh $8 round, with notice before more is needed. Historical costs remain separate. This single-use confirmation tests the repaired official PDF path on the unchanged authored DOBNOW-016 scenario and question, once. The preceding conservative round total is $7.468798, leaving $0.531202. Maximum $0.50 for one unassigned Research HTTP turn, at most two provider requests, no paid search, manual retry, separate judge, Project workflow or deployment.",
...
const preflightURL = new URL("evals/results/research-owner-api-round2-loft-pdf-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-loft-pdf-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-presentation.mjs", "research-official-guidance-summary.mjs", "evals/research-answer-key-amendments.json", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-official-pdf-ranking.mjs", "research-official-pdf-worker.mjs", "research-dob-workflow-routing.mjs", "evals/research-answer-key-reconciliation.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "package.json", "package-lock.json", "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-loft-pdf-20260909.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "The authored Loft Board question must pass the no-API HTTP preflight before live dispatch.");
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner Loft PDF confirmation" } });
