// Confirm guidance prompt v6 on the unchanged authored DOBNOW-016 question once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-loft-pdf-2026-09-09.json";
...
const repairedCaseIDs = ["DOBNOW-016"];
const newCaseIDs = [];
...
assert.equal(roundPreviousConservativeUSD, 7.492540);
const profile = { promptVersion: "20260909-document-summary-v6", schema: "permitext-owner-api-round2-guidance-precision-v1", sourceCommit,
  authorization: "Owner authorized continued API tests within a fresh $8 round, with notice before more is needed. Historical costs remain separate. This single-use confirmation tests clearer application, explicit prerequisites and approval boundaries under guidance prompt v6 on the unchanged authored DOBNOW-016 scenario and question, once. The preceding conservative round total is $7.492540, leaving $0.507460. Maximum $0.20 for one unassigned Research HTTP turn, at most two provider requests, no paid search, manual retry, separate judge, Project workflow or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.2, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.2, maximumTurns: 1, maximumProviderRequests: 2, maximumProviderRequestsPerTurn: 2,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-guidance-precision-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-guidance-precision-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-presentation.mjs", "research-official-guidance-summary.mjs", "evals/research-answer-key-amendments.json", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-official-pdf-ranking.mjs", "research-official-pdf-worker.mjs", "research-dob-workflow-routing.mjs", "evals/research-answer-key-reconciliation.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "package.json", "package-lock.json", "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-guidance-precision-20260909.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "The unchanged authored Loft Board question must pass the no-API HTTP preflight before live dispatch.");
...
globalThis.fetch = async (url, options) => {
  ...
  if (!active.initialRequestSHA256) {
    ...
    assert.equal(active.id, "DOBNOW-016");
    const workflow = researchDOBWorkflowRoute(active.question);
    assert.equal(workflow?.directDocumentRetrieval, true, "The repaired direct PDF route must be active.");
    if (workflow?.directDocumentRetrieval) {
      ...
      assert(input.passages.some((passage) => passage.url.includes("26_lb_dn-sn.pdf") && passage.page === 1));
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner guidance precision confirmation" } });
