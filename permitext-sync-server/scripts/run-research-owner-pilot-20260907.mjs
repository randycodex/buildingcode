// One attempt at the eight-turn owner-authorized September 7 pilot.
...
const cases = [
  { id: "CC-01", conversation: "stairs", question: question("CC-01") },
  { id: "CC-03", conversation: "room", question: question("CC-03") },
  { id: "CC-04", conversation: "room", question: question("CC-04") },
  { id: "ZR-08", conversation: "far", question: question("ZR-08") },
  { id: "ZR-06", conversation: "missing-facts", question: question("ZR-06") },
  { id: "DOBNOW-019", conversation: "bpp", question: question("DOBNOW-019") },
  { id: "DOBNOW-018", conversation: "wetlands", question: question("DOBNOW-018") },
  { id: "PDF-BPP", conversation: "explicit-pdf", question: "According to this official DOB service notice, a new Builders Pavement Plan application is initiated after August 17, 2026. Where must it be filed, which review type applies, and what authorization step appears? Cite the PDF page supporting the answer: https://www.nyc.gov/assets/buildings/pdf/bpp_build-sn.pdf" }
];
const profile = {
  schema: "permitext-owner-live-research-pilot-20260907-v1",
  authorization: "Owner: There are about $8 dollars on API tokens available that can be used.",
  ...
  maximumCumulativeSpendUSD: 7,
  maximumTurnSpendUSD: 0.85,
  maximumTurns: 8,
  absoluteTurnSumCeilingUSD: 6.8,
  repetitions: 1,
...
const resultURL = new URL("evals/results/research-owner-live-pilot-2026-09-07.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-live-pilot-"));
...
Object.assign(process.env, {
  ...
  PERMITEXT_RESEARCH_EVAL_MAX_USD: "7",
  PERMITEXT_RESEARCH_MAX_REQUEST_USD: "0.85",
  PERMITEXT_RESEARCH_USER_DAILY_CAP_USD: "7",
  PERMITEXT_RESEARCH_USER_MONTHLY_CAP_USD: "7",
  PERMITEXT_RESEARCH_DAILY_CAP_USD: "7",
  PERMITEXT_RESEARCH_MONTHLY_CAP_USD: "7",
  PERMITEXT_RESEARCH_MONTHLY_REQUEST_LIMIT: "8",
...
try {
  ...
  for (const item of cases) {
    ...
    await persist();
    console.log(`Starting ${item.id} (${result.results.length}/8).`);
    const start = performance.now();
    try {
