// One bounded owner-authorized PDF summary confirmation.
...
const previousConservativeSpendUSD = Number((earlierPilotConservativeSpendUSD + [...confirmationOperations, ...farOperations, ...qualityOperations, ...scopeOperations].reduce((sum, item) => sum + item.conservativeProviderCostUSD, 0)).toFixed(6));
assert.equal(previousConservativeSpendUSD, 3.026380);
...
const profile = {
  schema: "permitext-owner-document-summary-confirmation-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. This package is limited to two official PDF summary turns after no-cost source-integrity and persistence checks; one attempt per case.",
...
const resultURL = new URL("evals/results/research-owner-live-document-summary-confirmation-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-document-summary-confirmation-"));
