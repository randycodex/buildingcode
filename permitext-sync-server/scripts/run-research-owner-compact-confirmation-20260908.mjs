// Bounded confirmation of compact instructions and repaired laundry citation binding.
...
const cases = ["PC-04", "PC-10"].map((id) => {
...
const previousPackages = [
  ...
  ["attribution-confirmation-2026-09-08", 3, 9], ["premise-expansion-2026-09-08", 13, 28], ["routing-confirmation-2026-09-08", 1, 3, "stopped"], ["routing-confirmation-v2-2026-09-08", 3, 6], ["fountain-expansion-2026-09-08", 7, 15, "stopped"], ["plumbing-repair-confirmation-2026-09-08", 1, 2, "stopped"], ["plumbing-repair-confirmation-v2-2026-09-08", 2, 2, "stopped"]
];
let previousConservativeSpendUSD = 0;
const previousResultHashes = [];
for (const [suffix, turns, calls, expectedStatus = "completed"] of previousPackages) {
...
assert.equal(previousConservativeSpendUSD, 7.597436);
const profile = {
  schema: "permitext-owner-compact-confirmation-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. One laundry-source repair confirmation and one previously undispatched water-heater-pan question using compact instructions, one attempt each, capped at the remaining $0.402564 conservative aggregate with no manual retries.",
  ...
  maximumCumulativeSpendUSD: 0.402564,
  maximumTurnSpendUSD: 0.402564,
  maximumTurns: 2, authorizationUSD: 8,
  turnBudgetPolicy: "Cap each complete provider pipeline at the lesser of $0.402564 and the remaining settled conservative $0.402564 batch budget. Missing or pending accounting stops all further requests.",
...
nextBudget();
const comparisonBytes = await readFile(new URL("evals/results/research-instruction-compaction-comparison-2026-09-08.json", serverRoot));
const comparison = JSON.parse(comparisonBytes);
assert.equal(comparison.summary.cases, 110);
assert.equal(comparison.summary.networkAttempts, 0);
assert.equal(comparison.summary.providerCalls, 0);
assert.equal(comparison.currentAppSHA256, createHash("sha256").update(await readFile(new URL("app.mjs", serverRoot))).digest("hex"));
profile.instructionComparisonSHA256 = createHash("sha256").update(comparisonBytes).digest("hex");
const preflightURL = new URL("evals/results/research-owner-compact-confirmation-preflight-2026-09-08.json", serverRoot);
if (!process.argv.includes("--run-live")) {
  ...
  const { researchDiscoveryNeedsAutomaticWebSupport, researchWebSupportTrigger } = await import("../research-source-policy.mjs");
  ...
  const { buildResearchRequestEnvelopeBuilders, researchRequestEnvelopeEnvironment } = await import("../tests/research-request-envelope-preflight.mjs");
  const { deterministicResearchEvidenceAnalysisForTurn } = await import("../app.mjs");
  ...
  const { beginResearchSpendReservation, reserveResearchProviderSpend, endResearchSpendReservation } = await import("../research-config.mjs");
  const envelopeEnvironment = { ...researchRequestEnvelopeEnvironment, PERMITEXT_RESEARCH_MAX_REQUEST_USD: String(profile.maximumTurnSpendUSD) };
  const { buildAnswerRequest } = await buildResearchRequestEnvelopeBuilders(envelopeEnvironment);
  const results = [];
  for (const item of cases) {
    ...
    const completeReferences = item.id === "PC-03" ? ["PC 410.1", "PC 410.3"] : references;
    ...
    const useWeb = researchWebSupportTrigger({ question: item.question,
    ...
    if (item.id === "PC-03") {
      for (const reference of ["PC 410.1", "PC 410.3"]) assert(requiredReferences.includes(reference));
      for (const reference of ["PC 403.1", "PC 410.2"]) assert(!requiredReferences.includes(reference));
      assert.match(assembled.sources.find((source) => source.codePrefix === "PC" && source.sectionNumber === "410.3").text,
        /dedicated plumbing fixtures[\s\S]*container at least 10 inches[\s\S]*adjacent to or readily visible/i);
      assert.equal(useWeb, false);
    }
    assert.equal(useWeb, false, "This package does not authorize a new web path.");
    const registry = createResearchCorpusRegistry();
    const requestBody = buildAnswerRequest(item.question, assembled.sources, "synthetic-compact-preflight", {
      responseStyle: "conversational", model: "gpt-5.6-terra",
      requiredClaims: requiredResearchClaimsFromEvidence(assembled.sources),
      codeBasis: resolveResearchCodeBasis({ availableCorpora: registry, corpusPlan: routeResearchCorpora({ question: item.question, registry }), resolvedAt: "2026-09-08T12:00:00.000Z" }),
      structuredEvidenceAnalysis: deterministicResearchEvidenceAnalysisForTurn(assembled.sources, [])
    });
    let initialDraftMaximumUSD;
    beginResearchSpendReservation({ id: `preflight-${item.id}` }, envelopeEnvironment);
    try { initialDraftMaximumUSD = reserveResearchProviderSpend(requestBody, envelopeEnvironment).maximumRequestUSD; }
    finally { endResearchSpendReservation(); }
    assert(initialDraftMaximumUSD <= profile.maximumCumulativeSpendUSD);
    results.push({ id: item.id, references, completeReferences, missingReferences, sources, requiredReferences, selectedCorpora, useWeb, initialDraftMaximumUSD });
  }
  assert.equal(networkAttempts, 0);
  await writeFile(preflightURL, `${JSON.stringify({ schema: "permitext-owner-compact-confirmation-preflight-v1", sourceCommit: profile.sourceCommit, cases, results, networkAttempts, providerCalls: 0 }, null, 2)}\n`, { flag: "wx" });
...
const resultURL = new URL("evals/results/research-owner-live-compact-confirmation-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-compact-confirmation-"));
...
try {
  ...
  for (const item of cases) {
    ...
    if (preflight.results.find((candidate) => candidate.id === item.id).initialDraftMaximumUSD > budget.maximumNextTurnUSD) throw new Error(`Remaining settled budget cannot reserve the initial draft for ${item.id}; no turn dispatched.`);
