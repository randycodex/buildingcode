// Bounded fountain confirmation and nine previously untested safety/application cases.
...
const cases = ["PC-03", "FGC-08", "FGC-09", "FGC-10", "FGC-13", "MC-08", "PC-04", "PC-10", "PC-11", "PC-13"].map((id) => {
...
const previousPackages = [
  ...
  ["attribution-confirmation-2026-09-08", 3, 9], ["premise-expansion-2026-09-08", 13, 28], ["routing-confirmation-2026-09-08", 1, 3, "stopped"], ["routing-confirmation-v2-2026-09-08", 3, 6]
];
...
assert.equal(previousConservativeSpendUSD, 6.780490);
const profile = {
  schema: "permitext-owner-fountain-expansion-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. One fountain qualification confirmation and nine previously untested safety/application cases, one attempt each, capped at $1.20 conservative aggregate with no manual retries.",
  ...
  maximumCumulativeSpendUSD: 1.20,
  maximumTurnSpendUSD: 0.50,
  maximumTurns: 10, authorizationUSD: 8,
  turnBudgetPolicy: "Cap each complete provider pipeline at the lesser of $0.50 and the remaining settled conservative $1.20 batch budget. Missing or pending accounting stops all further requests.",
...
nextBudget();
const preflightURL = new URL("evals/results/research-owner-fountain-expansion-preflight-2026-09-08.json", serverRoot);
if (!process.argv.includes("--run-live")) {
  ...
  await writeFile(preflightURL, `${JSON.stringify({ schema: "permitext-owner-fountain-expansion-preflight-v1", sourceCommit: profile.sourceCommit, cases, results, networkAttempts, providerCalls: 0 }, null, 2)}\n`, { flag: "wx" });
...
const resultURL = new URL("evals/results/research-owner-live-fountain-expansion-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-fountain-expansion-"));
