// One bounded owner-authorized FAR confirmation after the table-integrity and definition repairs.
...
const previousConservativeSpendUSD = Number((earlierPilotConservativeSpendUSD + confirmationOperations.reduce((sum, item) => sum + item.conservativeProviderCostUSD, 0)).toFixed(6));
assert(Number.isFinite(previousConservativeSpendUSD) && previousConservativeSpendUSD <= 2.163);
...
assert(previousConservativeSpendUSD + 0.85 <= 8, "Retain the owner's total authorization across runs.");
const profile = {
  schema: "permitext-owner-far-repair-confirmation-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. This package is limited to one FAR turn after verified local repairs, one attempt.",
  ...
  maximumCumulativeSpendUSD: 1,
  maximumTurnSpendUSD: 0.85,
  maximumTurns: 1,
  absoluteTurnSumCeilingUSD: 0.85,
  repetitions: 1,
...
const resultURL = new URL("evals/results/research-owner-live-far-confirmation-2026-09-08.json", serverRoot);
...
Object.assign(process.env, {
  ...
  PERMITEXT_RESEARCH_USER_MONTHLY_CAP_USD: "1",
  PERMITEXT_RESEARCH_DAILY_CAP_USD: "1",
  PERMITEXT_RESEARCH_MONTHLY_CAP_USD: "1",
