// Confirm four repaired DOB cases and attempt seven untested DOB cases, once each.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-dob-source-coverage-2026-09-09.json";
...
const repairedCaseIDs = ["DOBNOW-001", "DOBNOW-008", "DOBNOW-020", "DOBNOW-021"];
const newCaseIDs = ["DOBNOW-003", "DOBNOW-004", "DOBNOW-006", "DOBNOW-010", "DOBNOW-011", "DOBNOW-012", "DOBNOW-013"];
...
assert.equal(roundPreviousConservativeUSD, 5.417557);
const profile = { schema: "permitext-owner-api-round2-dob-safety-confirmation-v1", sourceCommit,
  authorization: "Owner explicitly authorized fresh API tests with $8 currently available and requested notice if more is needed. This is a new $8 round; historical costs stay separate. This single-use batch confirms four repaired DOB cases and attempts seven untested DOB workflow cases, once each; it retains all failures from earlier batches. Their original scenarios and questions are preserved with the packet's shared DOB NOW workflow context, explicitly recovered and validated independently of the answers. It dispatches only turns passing offline preflight. Maximum $1.50 for the batch within the remaining $8 round allowance. Each next turn requires a full $0.50 remaining conservative allowance. No Project workflow tests or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(1.50, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 11, maximumProviderRequests: 55, maximumProviderRequestsPerTurn: 5,
  ...
  scope: "Actual unassigned HTTP Research on isolated local accounts and storage. No Project is created or exercised. Each question includes the source packet's explicit DOB NOW workflow context plus its unchanged scenario and question. No answer key, rubric, required-citation hint or professional-review expectation is supplied to drafting or verification. Existing automatic official web search and public NYC document retrieval are enabled; no model or routing substitution is made. The no-API preflight intercepts every provider dispatch and permits only public NYC document GETs, needed to bind the direct-document summary request for the repaired site-safety case. Earlier no-context attempts and all 110 numbered cases remain in the cumulative scope; this eleven-case batch does not narrow acceptance. No live filing, saved-project workflow, phone, UI or deployment changes.",
...
const preflightURL = new URL("evals/results/research-owner-api-round2-dob-safety-confirmation-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-dob-safety-confirmation-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-presentation.mjs", "research-official-guidance-summary.mjs", "evals/research-answer-key-amendments.json", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-official-pdf-ranking.mjs", "research-official-pdf-worker.mjs", "research-dob-workflow-routing.mjs", "evals/research-answer-key-reconciliation.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-dob-safety-confirmation-20260909.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "All eleven DOB inputs must pass the no-API HTTP preflight before live dispatch.");
...
globalThis.fetch = async (url, options) => {
  ...
  if (!active || body.tools?.some((tool) => tool.type !== "web_search") || body.service_tier !== "default") throw stop("Unexpected provider request shape.");
  const phase = body.tools?.length ? "web_support" : body.text?.format?.name;
  if (!active.initialRequestSHA256) {
    assert(["web_support", "permitext_code_interpretation", "permitext_official_guidance_summary"].includes(phase));
    if (active.id === "DOBNOW-008") {
      assert.equal(phase, "permitext_official_guidance_summary", "The direct site-safety route must retrieve its catalog before drafting.");
      const input = JSON.parse(body.input);
      const urls = input.passages.map((passage) => passage.url);
      for (const document of ["dob_now_application_user_guide.pdf", "2022_code_changes_dobnow.pdf", "code_site_safety_1-3_family_sn.pdf"]) {
        assert(urls.some((url) => new URL(url).pathname.endsWith(`/${document}`)), `Missing direct site-safety source: ${document}`);
      }
      assert(input.passages.some((passage) => passage.url.includes("dob_now_application_user_guide.pdf") && passage.page === 23));
      assert(input.passages.some((passage) => passage.url.includes("2022_code_changes_dobnow.pdf") && passage.page === 17));
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner DOB safety diagnostic" } });
