// Confirm the preparation/attestation distinction on one unchanged authored question.
// Uses the remaining approved allowance, not the pending additional allowance.
...
assert(process.argv.slice(2).every((arg, index, args) => arg === "--run-live" || ["--output", "--capture-prefix"].includes(arg) || ["--output", "--capture-prefix"].includes(args[index - 1])), "Unknown argument.");
assert(!live || process.argv.length === 3, "Live mode accepts only --run-live.");
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-source-resolution-parts-2026-09-09.json";
...
const repairedCaseIDs = ["DOBNOW-023"];
const newCaseIDs = [];
...
assert.equal(roundPreviousConservativeUSD, 8.304417);
assert.equal(hash(firstRoundBytes), "7cc352badc9e6ed4ac022784ff022ccedbf7d508cc66bac7ef2820302a9a9d7a");
...
const draftComparisonFile = "evals/results/research-release-history-check-2026-09-09.json";
const draftComparisonBytes = await readFile(new URL(draftComparisonFile, root));
assert.equal(hash(draftComparisonBytes), "1a64c780c68ffb7959b64ff66c8ebc7b2b500737b56cdf90a0f2f0201e617b06");
...
assert.equal(draftComparison.summary.numberedRouteChecks, 110);
assert.equal(draftComparison.summary.requestComparisons, 4);
assert.equal(draftComparison.summary.originalPassagesRemovedOrRewritten, 0);
assert.equal(draftComparison.summary.providerCalls, 0);
const profile = { promptVersion: "20260909-document-summary-v15", schema: "permitext-owner-api-round2-attestation-preparation-v1", sourceCommit,
  draftComparisonFile, draftComparisonSHA256: hash(draftComparisonBytes),
  authorization: "The owner authorized continued API testing toward the full Research goal within an $8 allowance, then approved an additional $0.50. Settled conservative usage is $8.304417, leaving $0.195583. This single-use test uses at most $0.19 for DOBNOW-023 once, with at most two provider calls. The aggregate authorization remains $8.50. The separately requested additional $0.50 is still unapproved and is not used. The v15 draft retains the preparation versus attestation/submission distinction; the ordinary independent verifier remains mandatory. The hard spending gate may stop the turn if actual settled use leaves insufficient room to reserve verification; this test never overrides that gate or delivers an unverified answer. No paid search, manual retry, separate judge, Project workflow, deployment or price change. All 110 questions remain in the acceptance scope.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.19, Number((authorizationAmendment.authorizationUSD - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.19, turnSpendCapsUSD: { "DOBNOW-023": 0.19 }, maximumTurns: 1, maximumProviderRequests: 2, maximumProviderRequestsPerTurn: 2,
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "The unchanged authored attestation question must pass the no-API HTTP preflight before live dispatch.");
...
globalThis.fetch = async (url, options) => {
  ...
  if (!active.initialRequestSHA256) {
    ...
    if (workflow?.directDocumentRetrieval) {
      ...
      assert.deepEqual(input.sourceResolutionPacket, guidanceSourceResolutionPacket(input));
      assert.equal(Object.keys(body.text.format.schema.properties)[0], "sourceResolutions");
      assert(body.text.format.schema.required.includes("sourceResolutions"));
      const paragraphSchema = body.text.format.schema.properties.paragraphs.items;
      assert.deepEqual(paragraphSchema.required, ["parts", "sourceUses"]);
      assert(paragraphSchema.properties.parts.items.required.includes("relationshipIndex"));
      assert(!Object.hasOwn(body.text.format.schema.properties.sourceResolutions.properties.relationships.items.properties, "paragraphIndex"));
      assert.equal(input.sourceRelationships.length, active.id === "DOBNOW-003" ? 2 : 1, "All fetched source relationships must reach drafting.");
      ...
      assert.match(body.instructions, /permitted preparation/i);
      ...
      } else if (active.id === "DOBNOW-023") {
        assert.equal(input.sourceRelationships[0].kind, "conditional_stakeholder");
        assert.match(text, /required Stakeholder/i);
        assert.match(text, /attestations?[^.!?]*before[^.!?]*filing/i);
      } else {
  ...
  if (earlier.length) {
    ...
    assert.deepEqual(input.sourceResolutionPacket, JSON.parse(initialBody.input).sourceResolutionPacket);
    validateGuidanceSourceResolutions(input, input.proposedAnswer);
    const rawText = earlier[0].output.filter(item => item.type === "message")
      .flatMap(item => item.content || []).filter(item => item.type === "output_text").map(item => item.text).join("");
    const rendered = materializeGuidanceSourceResolutions(JSON.parse(initialBody.input), JSON.parse(rawText));
    const { answerText, ...proposedDraft } = input.proposedAnswer;
    assert.deepEqual(proposedDraft, rendered, "The verifier must receive all model-authored prose and precisely the referenced findings.");
    const compact = text => text.replace(/\s+/g, " ").trim();
    assert(rendered.paragraphs.every(paragraph => compact(answerText).includes(compact(paragraph.text))),
      "All rendered paragraphs must remain in the verified answer text.");
    assert.deepEqual(input.sourceRelationships, guidanceSourceRelationships(input));
    assert.match(body.instructions, /Resolve each material relationship/);
    assert(body.instructions.includes(researchClaimScopeInstruction));
    assert.deepEqual(input.qualificationReviewPacket, guidanceQualificationReviewPacket(input));
    assert.match(body.instructions, /answer text actually says/i);
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner attestation preparation confirmation" } });
