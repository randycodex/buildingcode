// One bounded owner-authorized fixture follow-up confirmation after governing source completion.
...
const cases = [
  ...
  { id: "CC-04", conversation: "assembly", question: question("CC-04") }
];
...
const previousConservativeSpendUSD = Number((earlierPilotConservativeSpendUSD + [...confirmationOperations, ...farOperations, ...qualityOperations, ...scopeOperations, ...documentOperations].reduce((sum, item) => sum + item.conservativeProviderCostUSD, 0)).toFixed(6));
assert.equal(previousConservativeSpendUSD, 3.050704);
...
assert(previousConservativeSpendUSD + 2 * 0.85 <= 8, "Retain the owner's total authorization across runs.");
const profile = {
  schema: "permitext-owner-fixture-confirmation-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. This package is limited to a two-turn assembly and fixture follow-up conversation after no-cost proof that the full governing fixture section reaches Research; one attempt per case.",
  ...
  maximumCumulativeSpendUSD: 1.8,
  maximumTurnSpendUSD: 0.85,
  maximumTurns: 2,
  absoluteTurnSumCeilingUSD: 1.7,
  repetitions: 1,
...
const resultURL = new URL("evals/results/research-owner-live-fixture-confirmation-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-fixture-confirmation-"));
...
Object.assign(process.env, {
  ...
  PERMITEXT_RESEARCH_EVAL_MAX_USD: "1.8",
  PERMITEXT_RESEARCH_MAX_REQUEST_USD: "0.85",
  PERMITEXT_RESEARCH_USER_DAILY_CAP_USD: "1.8",
  PERMITEXT_RESEARCH_USER_MONTHLY_CAP_USD: "1.8",
  PERMITEXT_RESEARCH_DAILY_CAP_USD: "1.8",
  PERMITEXT_RESEARCH_MONTHLY_CAP_USD: "1.8",
  PERMITEXT_RESEARCH_MONTHLY_REQUEST_LIMIT: "2",
