// New owner-authorized API round: expand to 21 unattempted Construction Code questions through isolated Research HTTP.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-repair-confirmation-2026-09-08.json";
...
const caseIDs = ["FGC-11", "FGC-12", "FGC-14", "FGC-15", "MC-11", "MC-12", "MC-14", "PC-06", "PC-07", "PC-09", "PC-11", "PC-13", "PC-14", "GAP-06", "GAP-07", "GAP-09", "GAP-10", "GAP-12", "GAP-13", "GAP-14", "GAP-15"];
assert(caseIDs.every((id) => !attempted.has(id)), "This batch expands to previously unattempted numbered cases.");
const profile = { schema: "permitext-owner-api-round2-code-coverage-v1", sourceCommit,
  authorization: "Owner explicitly authorized fresh API tests with $8 currently available and requested notice if more is needed. This is a new $8 round; historical costs stay separate. This single-use batch expands to 21 previously unattempted Construction Code questions; maximum $3.50 for the batch within the remaining $8 round allowance. Each next turn requires a full $0.50 remaining conservative allowance. No Project workflow tests or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(3.5, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 21, maximumProviderRequests: 105, maximumProviderRequestsPerTurn: 5,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-code-coverage-preflight-2026-09-08.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-code-coverage-2026-09-08.json", root);
const sourceFiles = ["app.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-code-coverage-20260908.mjs"];
