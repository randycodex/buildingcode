// One bounded owner-authorized technical expansion after source, PDF, and revision-verification repairs.
...
const cases = ["MC-05", "GAP-04", "PC-03", "PC-08", "MC-10"].map((id) => {
...
const previousConservativeSpendUSD = Number((earlierPilotConservativeSpendUSD + [...confirmationOperations, ...farOperations, ...qualityOperations, ...scopeOperations, ...documentOperations, ...fixtureOperations].reduce((sum, item) => sum + item.conservativeProviderCostUSD, 0)).toFixed(6));
assert.equal(previousConservativeSpendUSD, 3.492894);
...
assert(previousConservativeSpendUSD + 5 * 0.85 <= 8, "Retain the owner's total authorization across runs.");
const profile = {
  schema: "permitext-owner-technical-expansion-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. This package is limited to five previously untested technical questions after source review and no-cost retrieval proof; one attempt per question; one attempt per case.",
  ...
  maximumCumulativeSpendUSD: 4.3,
  maximumTurnSpendUSD: 0.85,
  maximumTurns: 5,
  absoluteTurnSumCeilingUSD: 4.25,
  repetitions: 1,
...
const resultURL = new URL("evals/results/research-owner-live-technical-expansion-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-technical-expansion-"));
...
Object.assign(process.env, {
  ...
  PERMITEXT_RESEARCH_EVAL_MAX_USD: "4.3",
  PERMITEXT_RESEARCH_MAX_REQUEST_USD: "0.85",
  PERMITEXT_RESEARCH_USER_DAILY_CAP_USD: "4.3",
  PERMITEXT_RESEARCH_USER_MONTHLY_CAP_USD: "4.3",
  PERMITEXT_RESEARCH_DAILY_CAP_USD: "4.3",
  PERMITEXT_RESEARCH_MONTHLY_CAP_USD: "4.3",
  PERMITEXT_RESEARCH_MONTHLY_REQUEST_LIMIT: "5",
