// Two previously unattempted owner questions; isolated HTTP, existing routing.
...
assert.equal(previousResultHashes.length, 19);
...
assert.equal(previousConservativeSpendUSD, 7.853484);
...
const caseIDs = ["ZR-01", "ZR-14"];
assert(caseIDs.every((id) => !attempted.has(id)));
const profile = { schema: "permitext-owner-zoning-small-confirmation-v1", sourceCommit,
  authorization: "Owner authorized approximately $8 of API tests and repeated testing toward the 110-case answer target. This new package attempts ZR-01 and ZR-14 once each within the existing remaining authorization; it cannot replay a consumed package.",
  previousResultHashes, previousConservativeSpendUSD, authorizationUSD: 8,
  maximumCumulativeSpendUSD: 0.14, maximumTurnSpendUSD: 0.07, maximumTurns: 2, maximumProviderRequests: 6,
...
const preflightURL = new URL("evals/results/research-owner-zoning-small-confirmation-preflight-2026-09-08.json", root);
const resultURL = new URL("evals/results/research-owner-live-zoning-small-confirmation-2026-09-08.json", root);
const sourceFiles = ["app.mjs", "research-config.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-safety.mjs",
  "research-evidence-assembly.mjs", "research-corpus-registry.mjs", "research-model-routing.mjs", "evals/research-owner-scope-input.mjs",
  "evals/research-reconciled-answer-key.json", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-zoning-small-confirmation-20260908.mjs"];
...
const scratch = await mkdtemp(join(tmpdir(), live ? "permitext-zoning-small-live-" : "permitext-zoning-small-preflight-"));
...
Object.assign(process.env, {
  ...
  PERMITEXT_RESEARCH_USER_DAILY_CAP_USD: "0.14", PERMITEXT_RESEARCH_USER_MONTHLY_CAP_USD: "0.14",
  PERMITEXT_RESEARCH_DAILY_CAP_USD: "0.14", PERMITEXT_RESEARCH_MONTHLY_CAP_USD: "0.14", PERMITEXT_RESEARCH_MONTHLY_REQUEST_LIMIT: "2",
