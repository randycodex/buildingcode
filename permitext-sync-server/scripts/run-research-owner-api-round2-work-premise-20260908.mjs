// Confirm a repaired negative work-commencement premise and the skipped stop-work question.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-code-coverage-2026-09-08.json";
...
const caseIDs = ["GAP-06", "GAP-15"];
assert(!attempted.has("GAP-15"));
const profile = { schema: "permitext-owner-api-round2-work-premise-v1", sourceCommit,
  authorization: "Owner explicitly authorized fresh API tests with $8 currently available and requested notice if more is needed. This is a new $8 round; historical costs stay separate. This single-use batch confirms the negative-commencement fact repair on GAP-06 and tests the previously skipped GAP-15. Maximum $1.00 for the batch within the remaining $8 round allowance. Each next turn requires a full $0.50 remaining conservative allowance. No Project workflow tests or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(1, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 2, maximumProviderRequests: 10, maximumProviderRequestsPerTurn: 5,
  ...
  scope: "Actual automatic-retrieval HTTP Research flow on isolated local accounts and storage. Expected answers are not supplied to drafting or verification. Local diagnostic Zoning eligibility only; no public eligibility, deployment, or phone changes.",
  cases: caseIDs };
...
const preflightURL = new URL("evals/results/research-owner-api-round2-work-premise-preflight-2026-09-08.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-work-premise-2026-09-08.json", root);
const sourceFiles = ["app.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-work-premise-20260908.mjs"];
