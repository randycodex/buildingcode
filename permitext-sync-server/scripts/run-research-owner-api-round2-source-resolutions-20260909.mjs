// Confirm source-resolution drafting on three unchanged workflow questions once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-draft-focus-2026-09-09.json";
...
const repairedCaseIDs = ["DOBNOW-003", "DOBNOW-004", "DOBNOW-023"];
const newCaseIDs = [];
...
assert.equal(roundPreviousConservativeUSD, 8.094222);
...
assert.equal(researchOfficialGuidanceSummaryPromptVersion, "20260909-document-summary-v13");
const draftComparisonFile = "evals/results/research-source-resolution-request-comparison-2026-09-09.json";
const draftComparisonBytes = await readFile(new URL(draftComparisonFile, root));
assert.equal(hash(draftComparisonBytes), "bb18d2d5362451ecb918531fd401201a17538823a45b0298887e79d953fea5c4");
...
assert.equal(draftComparison.summary.requestComparisons, 6);
assert.equal(draftComparison.summary.changedSourcePassages, 0);
assert.equal(draftComparison.summary.providerCalls, 0);
const profile = { promptVersion: "20260909-document-summary-v13", schema: "permitext-owner-api-round2-source-resolutions-v1", sourceCommit,
  draftComparisonFile, draftComparisonSHA256: hash(draftComparisonBytes),
  authorization: "The owner authorized continued API testing toward the full Research goal within an $8 allowance, then approved an additional $0.50 to the testing allowance. The prior scoped comparison and amendment remain immutable in the ledger. This single-use continuation uses at most $0.40 of the $0.405778 aggregate remaining allowance; the total round cap stays $8.50. It tests the unchanged DOBNOW-003, DOBNOW-004 and DOBNOW-023 authored questions once each on prompt v13. Each draft must resolve detected source relationships within the existing drafting call and carry its finding into the cited main text; independent semantic verification remains required. Three unassigned Research HTTP turns, at most $0.22 each, two provider calls per turn and six total. The batch stops if the remaining cap cannot reserve a complete next turn. No paid search, manual retry, separate judge, Project workflow, deployment or price change.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.40, Number((authorizationAmendment.authorizationUSD - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.22, turnSpendCapsUSD: { "DOBNOW-003": 0.22, "DOBNOW-004": 0.22, "DOBNOW-023": 0.22 }, maximumTurns: 3, maximumProviderRequests: 6, maximumProviderRequestsPerTurn: 2,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-source-resolutions-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-source-resolutions-2026-09-09.json", root);
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "All three unchanged authored workflow questions must pass the no-API HTTP preflight before live dispatch.");
