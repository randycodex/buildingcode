// Confirm source-derived applicability questions and direct submission evidence on two unchanged workflow questions once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-source-span-2026-09-09.json";
...
assert.equal(roundPreviousConservativeUSD, 7.745497);
assert.equal(researchOfficialGuidanceSummaryPromptVersion, "20260909-document-summary-v9");
const profile = { promptVersion: "20260909-document-summary-v9", schema: "permitext-owner-api-round2-source-relationships-v1", sourceCommit,
  authorization: "Owner authorized continued API tests within a fresh $8 round, with notice before more is needed. Historical costs remain separate. This single-use comparison tests source-derived applicability questions and direct applicant-submission evidence under guidance prompt v9 on unchanged authored DOBNOW-023 and DOBNOW-003 scenarios and questions, once each. The preceding conservative round total is $7.745497, leaving $0.254503. Maximum $0.24 for two unassigned Research HTTP turns: at most $0.20 for DOBNOW-023 and $0.18 for DOBNOW-003, two provider requests per turn and four total. No paid search, manual retry, separate judge, Project workflow or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.24, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.2, turnSpendCapsUSD: { "DOBNOW-023": 0.2, "DOBNOW-003": 0.18 }, maximumTurns: 2, maximumProviderRequests: 4, maximumProviderRequestsPerTurn: 2,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-source-relationships-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-source-relationships-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-presentation.mjs", "research-official-guidance-summary.mjs", "research-guidance-qualification-review.mjs", "research-guidance-source-relationships.mjs", "evals/research-answer-key-amendments.json", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-official-pdf-ranking.mjs", "research-official-pdf-worker.mjs", "research-dob-workflow-routing.mjs", "evals/research-answer-key-reconciliation.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "package.json", "package-lock.json", "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-source-relationships-20260909.mjs"];
...
globalThis.fetch = async (url, options) => {
  ...
  if (!active.initialRequestSHA256) {
    ...
    if (workflow?.directDocumentRetrieval) {
      ...
      assert.equal(input.sourceRelationships.length, 1, "The fetched source relationship must reach drafting.");
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner source relationship confirmation" } });
