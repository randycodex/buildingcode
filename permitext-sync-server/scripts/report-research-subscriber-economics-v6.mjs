...
export function renderV6SubscriberEconomicsMarkdown(report) {
  ...
  return `# Permitext Research subscriber economics — V6\n\n` +
    ...
    `- Tax: Permitext now has a dormant automatic-tax Checkout path with required billing-address collection and a fail-closed Production guard. The live USD $20 Price uses Stripe's \`Default (inferred by currency)\` behavior, which Stripe currently documents as exclusive; the verifier resolves that provider behavior before comparing it with the required owner decision. Certificate receipt, product-code review, registration, Production configuration, and a real taxed Checkout remain open. The model already includes Stripe Tax Basic's 0.5% web fee separately from the 5% unresolved downside reserve, so this implementation evidence does not change the $15.84 p90 web projection. The 5% line is not a factual tax rate. [Stripe Tax setup](https://docs.stripe.com/tax/set-up) · [Stripe Tax pricing](https://stripe.com/tax/pricing) · [New York computer-software guidance](https://www.tax.ny.gov/pubs_and_bulls/tg_bulletins/st/computer_software.htm)\n` +
