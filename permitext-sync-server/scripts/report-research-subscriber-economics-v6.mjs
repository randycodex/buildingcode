...
export function renderV6SubscriberEconomicsMarkdown(report) {
  ...
  return `# Permitext Research subscriber economics — V6\n\n` +
    ...
    `- Tax: Permitext now has a dormant automatic-tax Checkout path with required billing-address collection and a fail-closed Production guard. The owner approved \`automatic\` + \`exclusive\` on August 30, 2026, with the exact web disclosure \`$20/month plus applicable taxes shown by Stripe.\` The live USD $20 Price uses Stripe's \`Default (inferred by currency)\` behavior, which Stripe currently documents as exclusive; the verifier resolves that provider behavior before comparing it with the approved decision. Certificate receipt, Product tax-code review, registration, Production configuration, and a real taxed Checkout remain open. The model already includes Stripe Tax Basic's 0.5% web fee separately from the 5% unresolved downside reserve, so this implementation evidence does not change the $15.84 p90 web projection. The 5% line is not a factual tax rate. [Stripe Tax setup](https://docs.stripe.com/tax/set-up) · [Stripe Tax pricing](https://stripe.com/tax/pricing) · [New York computer-software guidance](https://www.tax.ny.gov/pubs_and_bulls/tg_bulletins/st/computer_software.htm)\n` +
    ...
    `- Infrastructure: Vercel currently lists Pro at $20 per month with $20 of usage credit. The owner configured a $20 on-demand amount, for approximately $40 plus tax and possible small metering overrun of initial exposure. The retained $45 p90 case remains a deliberately higher sensitivity, not measured marginal subscriber usage or the current live spend setting. [Vercel Pro plan](https://vercel.com/docs/plans/pro-plan)\n` +
    `- App Store: the confirmed decision case uses the 15% Small Business Program commission. Apple documents the customer price as inclusive of applicable taxes it collects and remits, and Partner Share as customer price minus applicable taxes and commission. Stripe automatic tax therefore stays web-only. A later read-only inspection verified the parent app category as \`App Store software\` and the subscription as \`Match to parent app\`; the owner accepted leaving that classification unchanged. The iOS tax reserve remains until actual financial reports provide measured tax and proceeds evidence. [Apple Small Business Program](https://developer.apple.com/app-store/small-business-program/) · [Apple tax categories](https://developer.apple.com/help/app-store-connect/manage-app-information/set-a-tax-category) · [Apple financial report fields](https://developer.apple.com/help/app-store-connect/reference/reporting/financial-report-fields/)\n\n` +
