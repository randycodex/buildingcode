// Confirm explicit semantic map-scope review, historical branch coverage and source binding on authored ZR-06 once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-storage-scope-2026-09-09.json";
...
assert.equal(roundPreviousConservativeUSD, 7.375051);
const profile = { schema: "permitext-owner-api-round2-mapped-review-v1", sourceCommit,
  authorization: "Owner authorized continued API testing within a fresh $8 round and requested notice before more is needed. Historical spending stays separate. This single-use batch checks the explicit semantic map-scope review, historical-branch coverage and source-binding repairs on ZR-06 once, preserving its authored question and section references. Batch maximum $0.50 within the round's $0.624949 conservative remainder. The turn requires a full $0.50 conservative reservation. No manual retry, extra judge, Project workflow test or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.5, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 1, maximumProviderRequests: 2, maximumProviderRequestsPerTurn: 2,
  ...
  scope: "Actual unassigned HTTP Research on isolated accounts and storage. Author-specified section references use the explicit section_reference API mode; no highlight, visual review or parcel fact is fabricated. Canonical source retrieval and ordinary verification remain active. Answer keys and review expectations are excluded from planning, drafting and verification. Web support remains configured normally, but these enacted-source questions must not request web tools or external documents. The authored input must pass the zero-API preflight. Earlier failures and all 110 numbered cases remain in the cumulative acceptance scope. No UI, phone, Project workflow or deployment testing.",
  requestComparison: "The existing request binding normalizes opaque run-specific identifiers only; authored text, selected sources, runtime instructions and schemas remain bound. Raw request hashes are also recorded.",
...
const preflightURL = new URL("evals/results/research-owner-api-round2-mapped-review-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-mapped-review-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-presentation.mjs", "research-official-guidance-summary.mjs", "evals/research-answer-key-amendments.json", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-official-pdf-ranking.mjs", "research-official-pdf-worker.mjs", "research-dob-workflow-routing.mjs", "evals/research-answer-key-reconciliation.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  "research-evidence-assembly.mjs", "research-definition-excerpts.mjs", "research-zoning-context-excerpts.mjs", "research-zoning-metadata.mjs", "research-zoning-conditional-explanation.mjs", "project-foundation-contract.mjs", "research-corpus-registry.mjs", "research-model-routing.mjs", "evals/research-owner-scope-input.mjs",
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "research-zoning-lot-history.mjs", "research-project-fact-coverage.mjs", "research-zoning-attribution.mjs", "research-zoning-mapped-review.mjs", "research-zoning-storage-branches.mjs", "scripts/run-research-owner-api-round2-mapped-review-20260909.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "The authored ZR-06 input must pass the no-API HTTP preflight before live dispatch.");
...
globalThis.fetch = async (url, options) => {
  if (String(url) !== "https://api.openai.com/v1/responses") {
    throw stop("This enacted-source package must not request external documents.");
  }
  ...
  if (!active.initialRequestSHA256) {
    ...
    assert.equal(active.id, "ZR-06");
    assert.match(String(body.input), /Each supported point must include the supplied source IDs/);
    const contextLine = String(body.input).split("\n").find((line) => line.startsWith("DETERMINISTIC_CONTEXT: "));
    assert(contextLine);
    ...
    for (const id of ["storage_documented_reconstruction_branch", "storage_undocumented_nonconforming_branch"]) {
      assert(context.answerObligations.some((obligation) => obligation.id === id), "The actual draft must receive the source-derived historical alternatives.");
    }
    const missingLine = String(body.input).split("\n").find((line) => line.startsWith("MISSING_PROJECT_FACTS: "));
    assert(missingLine);
    const missing = JSON.parse(missingLine.slice("MISSING_PROJECT_FACTS: ".length));
    for (const id of ["special_district_status", "zoning_lot_area", "property_identifier", "official_mapped_status"]) {
      assert(missing.some((fact) => fact.id === id));
    }
  ...
  const mappedScopeReviewRequested = phase === "permitext_research_verification" && String(body.input).includes("MAPPED SCOPE REVIEW\n");
  if (mappedScopeReviewRequested) assert(body.text.format.schema.required.includes("mappedScopeReview"));
  const entry = { caseID: active.id, phase, model: body.model, mappedScopeReviewRequested, requestBytes: Buffer.byteLength(options.body), maxOutputTokens: body.max_output_tokens,
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner mapped scope diagnostic" } });
  ...
  for (const id of caseIDs) {
    ...
    assert(input.pinnedEvidence.length > 0);
    ...
    active = { id, question: input.question, inputSHA256: hash(JSON.stringify(input)), requestedCodeVersion: input.projectCodeVersion,
      ...
      selectionCharacterCounts: selections.map((selection) => ({ sectionID: selection.sectionID, characters: selection.selectedText?.length || 0, selectionMode: selection.selectionMode })),
    ...
    const created = await request("/research/conversations/create", { auth, ...(selections.length ? { selections } : {}) }, token);
