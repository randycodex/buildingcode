...
const previousConservativeSpendUSD = Number((earlierPilotConservativeSpendUSD + [...confirmationOperations, ...farOperations, ...qualityOperations].reduce((sum, item) => sum + item.conservativeProviderCostUSD, 0)).toFixed(6));
assert(Number.isFinite(previousConservativeSpendUSD) && previousConservativeSpendUSD <= 2.833);
...
const profile = {
  schema: "permitext-owner-answer-scope-confirmation-20260908-v2",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. This package is limited to two turns after verified descendant-scope and applicability repairs, one attempt per case.",
...
const resultURL = new URL("evals/results/research-owner-live-scope-confirmation-v2-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-scope-confirmation-v2-"));
