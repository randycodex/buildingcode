...
const previousPackages = [
  ...
  ["attribution-confirmation-2026-09-08", 3, 9], ["premise-expansion-2026-09-08", 13, 28], ["routing-confirmation-2026-09-08", 1, 3, "stopped"]
];
...
assert.equal(previousConservativeSpendUSD, 6.457678);
const profile = {
  schema: "permitext-owner-routing-confirmation-v2-20260908-v1",
...
nextBudget();
const preflightURL = new URL("evals/results/research-owner-routing-confirmation-v2-preflight-2026-09-08.json", serverRoot);
if (!process.argv.includes("--run-live")) {
  ...
  for (const item of cases) {
    ...
    if (item.id === "MC-02") {
      assert.deepEqual(selectedCorpora, ["nyc-2022-construction-codes"]);
      assert.equal(useWeb, false, "Contextual lot terminology must not trigger outside-Zoning web support.");
      assert(requiredReferences.includes("MC 401.4"));
      assert.match(assembled.sources.find((source) => source.codePrefix === "MC" && source.sectionNumber === "401.4").text,
        /Group R-3 occupancies are not required to comply/i);
    }
    if (item.id === "PC-01") {
...
const resultURL = new URL("evals/results/research-owner-live-routing-confirmation-v2-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-routing-confirmation-v2-"));
