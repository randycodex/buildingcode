// One bounded attempt at the eight-turn owner-authorized source-selection confirmation after the zero-call guard stop.
...
const ownerIDs = ["FGC-03", "FGC-04", "MC-06", "MC-15", "GAP-05", "PC-05", "PC-15"];
const cases = [
  ...ownerIDs.map((id) => {
...
const previousConservativeSpendUSD = previousOperations.reduce((sum, item) => sum + item.conservativeProviderCostUSD, 0);
assert(Number.isFinite(previousConservativeSpendUSD) && previousConservativeSpendUSD <= 1.066);
...
assert(previousConservativeSpendUSD + 8 * 0.85 <= 8, "Retain the owner's total authorization across runs.");
const profile = {
  schema: "permitext-owner-live-source-confirmation-20260908-v2",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. This package is limited to eight turns, one attempt each.",
...
const resultURL = new URL("evals/results/research-owner-live-source-confirmation-v2-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-source-confirmation-"));
