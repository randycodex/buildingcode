// Attempt the last five numbered cases with faithful section references, once each.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-dob-companion-confirmation-2026-09-09.json";
...
const repairedCaseIDs = [];
const newCaseIDs = ["ZR-03", "ZR-06", "ZR-09", "ZR-19", "ZR-20"];
...
assert.equal(roundPreviousConservativeUSD, 6.838767);
const profile = { schema: "permitext-owner-api-round2-section-reference-v1", sourceCommit,
  authorization: "Owner authorized continued API testing within a fresh $8 round and requested notice before more is needed. Historical spending stays separate. This single-use batch attempts the five numbered cases with no preceding provider dispatch, once each, preserving their authored questions and section references. Batch maximum $1.161233, the conservative remainder of the round. Each next turn requires a full $0.50 conservative reservation. No manual retry, extra judge, Project workflow test or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(1.161233, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 5, maximumProviderRequests: 25, maximumProviderRequestsPerTurn: 5,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-section-reference-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-section-reference-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-presentation.mjs", "research-official-guidance-summary.mjs", "evals/research-answer-key-amendments.json", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-official-pdf-ranking.mjs", "research-official-pdf-worker.mjs", "research-dob-workflow-routing.mjs", "evals/research-answer-key-reconciliation.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-section-reference-20260909.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "All five authored section-reference inputs must pass the no-API HTTP preflight before live dispatch.");
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner section reference diagnostic" } });
