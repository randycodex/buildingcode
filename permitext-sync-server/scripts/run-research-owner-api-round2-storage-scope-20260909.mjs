// Confirm mapped-source scope, declared missing facts and source attribution on authored ZR-06 once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-lot-history-2026-09-09.json";
...
assert.equal(roundPreviousConservativeUSD, 7.289668);
const profile = { schema: "permitext-owner-api-round2-storage-scope-v1", sourceCommit,
  authorization: "Owner authorized continued API testing within a fresh $8 round and requested notice before more is needed. Historical spending stays separate. This single-use batch checks the mapped-source scope, explicit missing-fact and claim-attribution repairs on ZR-06 once, preserving its authored question and section references. Batch maximum $0.50 within the round's $0.710332 conservative remainder. The turn requires a full $0.50 conservative reservation. No manual retry, extra judge, Project workflow test or deployment.",
...
const preflightURL = new URL("evals/results/research-owner-api-round2-storage-scope-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-storage-scope-2026-09-09.json", root);
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner historical premise diagnostic" } });
