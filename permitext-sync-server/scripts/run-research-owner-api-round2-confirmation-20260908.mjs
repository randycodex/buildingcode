// New owner-authorized API round: confirm five recent repairs through isolated Research HTTP.
...
const profile = { schema: "permitext-owner-api-round2-confirmation-v1", sourceCommit,
  authorization: "Owner explicitly authorized fresh API tests with $8 currently available and requested notice if more is needed. This is a new $8 round; historical costs stay separate. This single-use first batch attempts five recently repaired cases once with a $2.50 ceiling. No Project workflow tests or deployment.",
  previousResultHashes, historicalConservativeSpendUSD: previousConservativeSpendUSD, previousConservativeSpendUSD: 0, authorizationUSD: 8,
  ...
  cases: caseIDs };
assert(profile.maximumCumulativeSpendUSD <= profile.authorizationUSD);
assert(profile.maximumTurnSpendUSD * profile.maximumTurns <= profile.maximumCumulativeSpendUSD);
const preflightURL = new URL("evals/results/research-owner-api-round2-confirmation-preflight-2026-09-08.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-confirmation-2026-09-08.json", root);
const sourceFiles = ["app.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-confirmation-20260908.mjs"];
...
try {
  ...
  for (const id of caseIDs) {
    ...
    result.cumulativeConservativeSpendUSD = Number((result.results.flatMap((item) => item.operations || []).reduce((sum, operation) => sum + operation.conservativeProviderCostUSD, 0)).toFixed(6));
    assert(result.cumulativeConservativeSpendUSD <= profile.maximumCumulativeSpendUSD);
