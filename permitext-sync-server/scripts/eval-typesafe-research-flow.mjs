...
import { configureEvidenceFocus, operationTokenCost } from '../evals/typesafe-evidence-focus.mjs';
const serverRoot = new URL('../', import.meta.url);
const key = JSON.parse(await readFile(new URL('evals/research-reconciled-answer-key.json',serverRoot)));
const candidatePruning=process.argv.includes('--candidate-pruning');
const cases = (candidatePruning ? ['CC-01'] : ['CC-01','CC-03','CC-04']).map(id=>{
...
for(let repetition=1;repetition<=2;repetition++) for(const [i,c] of cases.entries())
 for(const mode of (i+repetition)%2 ? ['baseline','jev'] : ['jev','baseline']) plan.push({caseID:c.id,repetition,mode,question:c.question});
...
const replacement=`    const originalAssembledEvidence = boundedCitationLookup
      ? boundedCitationEvidence
      : evidencePackage.sources || [];
    const assembledEvidence = await focusResearchEvidence({ question, evidence: originalAssembledEvidence,
      requiredClaims: requiredResearchClaimsFromEvidence(originalAssembledEvidence), disabled: boundedCitationLookup || zoningTurn });`;
const variant='import { focusResearchEvidence } from "./evals/typesafe-evidence-focus.mjs";\n'+original.replace(needle,replacement);
const focusModuleBytes=await readFile(new URL('evals/typesafe-evidence-focus.mjs',serverRoot));
const profile={schemaVersion:1,candidatePruning,focusModuleSHA256:createHash('sha256').update(focusModuleBytes).digest('hex'),experiment:'full local HTTP Research flow, baseline vs Jev relevance preparation',
 sourceSHA256:createHash('sha256').update(original).digest('hex'),variantSHA256:createHash('sha256').update(variant).digest('hex'),
 plan,cases,configuration:'Terra single-model, medium answer reasoning, existing verification/repairs; identical for both arms',
 limitations:['Three construction-code scenarios, two repetitions; not a production workload sample.','No UI/network-to-production timing; measured from local HTTP request through completed verified answer.','Jev may reorder all passages but can omit only unprotected optional passages at confidence >=0.95.','Current preparation is deterministic in both arms; no artificial baseline model-analysis call.']};
if(!process.argv.includes('--live')) {console.log(JSON.stringify({status:'offline-preflight',...profile},null,2)); process.exit(0);}
assert.equal(process.env.PERMITEXT_TYPESAFE_FLOW_LIVE,'1','Explicit live authorization required');
...
const resultURL=new URL(`${Date.now()}-research-flow.json`,dir);
...
try {
 ...
 for(const [i,turn] of plan.entries()){
  ...
  configureEvidenceFocus({mode:turn.mode,allowCandidatePruning:candidatePruning,apiKey:local.TYPESAFE_API_KEY,fetch:nativeFetch,record:r=>{active.preparation=r;}});
  ...
  if(!active.answer) active.error={code:response.body.code,message:response.body.error};
  ...
  console.log(`${active.id}: ${active.status}, ${active.durationMs} ms, sources ${active.preparation?.before?.length} -> ${active.preparation?.afterIDs?.length ?? active.preparation?.before?.length}`);
  ...
  if(active.preparation?.status==='fallback') throw new Error('Jev preparation failed; current turn used baseline; stop experiment');
 }
 result.status='completed';
} catch(error){result.status='stopped';result.stopReason=error.message;}
finally{
 configureEvidenceFocus(null); globalThis.fetch=nativeFetch;
