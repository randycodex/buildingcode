...
const terminalFile = "evals/results/research-owner-live-decision-fact-verifier-v2-2026-09-08.json";
...
assert.equal(previousResultHashes.length, 18);
assert.equal(new Set(previousResultHashes.map((entry) => entry.file)).size, 18);
...
assert.equal(previousConservativeSpendUSD, 7.827715);
const sourceCommit = execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }).trim();
const profile = { schema: "permitext-decision-fact-verifier-diagnostic-v3", sourceCommit,
  authorization: "Owner authorized repeated Research testing within approximately $8 total. This package uses at most $0.15 of the remaining $0.172285 conservative allowance for four production-verifier checks of the exact delivered laundry answer, its fact correction, a broken point binding and an invented standpipe exception. The instruction explicitly separates supported application from unsupported inference.",
  previousResultHashes, previousConservativeSpendUSD, authorizationUSD: 8, maximumCumulativeSpendUSD: .15,
  maximumTurnSpendUSD: .15, maximumChecks: 4, maximumProviderRequests: 8,
  ...
  originalFullHTTPTurnAttempts: 66, originalProviderCalls: 152, originalVerifierOnlyChecks: 13,
...
const nextBudget = (operations) => ownerResearchNextTurnBudget({ previousConservativeUSD: previousConservativeSpendUSD, operations,
  authorizationUSD: 8, maximumBatchUSD: .15, maximumTurnUSD: .15 });
nextBudget([]);
const environment = { ...researchRequestEnvelopeEnvironment, PERMITEXT_RESEARCH_MAX_REQUEST_USD: ".15",
...
const preflightURL = new URL("evals/results/research-decision-fact-verifier-v3-preflight-2026-09-08.json", root);
...
for (const fixture of fixtures) {
  ...
  preflightEntries.push({ id: fixture.id, expectedPass: fixture.expectedPass, expectedIssueTypes: fixture.expectedIssueTypes, acceptableSubstantiveIssueTypes: fixture.acceptableSubstantiveIssueTypes || null, expectedRepairApplied: fixture.expectedRepairApplied,
...
assert(preflightEntries.reduce((sum, item) => sum + item.maximumRequestUSD, 0) < .15);
const preflightPayload = { ...profile, fixtureSourceSHA256: hash(await readFile(new URL("evals/research-decision-fact-fixtures-v3.mjs", root))),
...
if (!process.argv.includes("--run-live")) {
  ...
  console.log(JSON.stringify({ status: "preflight-passed-no-provider", sourceCommit, entries: preflightEntries, capUSD: .15 }));
  process.exit(0);
}
...
const resultURL = new URL("evals/results/research-owner-live-decision-fact-verifier-v3-2026-09-08.json", root);
...
Object.assign(process.env, environment, { OPENAI_API_KEY: local.OPENAI_API_KEY, PERMITEXT_RUN_PAID_RESEARCH_EVALS: "1", PERMITEXT_RESEARCH_EVAL_MAX_USD: ".15" });
...
try {
  for (const fixture of fixtures) {
    ...
    const entry = { id: fixture.id, sourceCaseID: fixture.sourceCaseID, scope: "verifier-only", expectedPass: fixture.expectedPass,
      expectedIssueTypes: fixture.expectedIssueTypes, acceptableSubstantiveIssueTypes: fixture.acceptableSubstantiveIssueTypes || null, expectedRepairApplied: fixture.expectedRepairApplied, expectedMissingFactIndices: fixture.expectedMissingFactIndices || [], purpose: fixture.purpose, status: "running", operations: [] };
    ...
    try {
      ...
      Object.assign(entry, assessDecisionFactVerifierResultV3(fixture, output.result));
