// Confirm two retained answer defects after complete-source and obligation repairs; isolated HTTP, existing routing.
...
assert.equal(previousResultHashes.length, 20);
...
assert.equal(previousConservativeSpendUSD, 7.864152);
...
// Dispatch the larger initial request first so the aggregate evaluation
// reservation can fit while preserving the existing production guards.
const caseIDs = ["ZR-14", "ZR-01"];
assert(caseIDs.every((id) => attempted.has(id)), "This package confirms the two retained misses; it adds no new numbered cases.");
const profile = { schema: "permitext-owner-zoning-direct-rule-confirmation-v1", sourceCommit,
  authorization: "Owner authorized approximately $8 of API tests and repeated testing toward the 110-case answer target. This new package confirms ZR-01 and ZR-14 once each after source/obligation repairs, within the existing remaining authorization; it cannot replay a consumed package.",
  previousResultHashes, previousConservativeSpendUSD, authorizationUSD: 8,
  maximumCumulativeSpendUSD: 0.135848, maximumTurnSpendUSD: 0.06, maximumTurns: 2, maximumProviderRequests: 6,
...
const preflightURL = new URL("evals/results/research-owner-zoning-direct-rule-confirmation-preflight-2026-09-08.json", root);
const resultURL = new URL("evals/results/research-owner-live-zoning-direct-rule-confirmation-2026-09-08.json", root);
const sourceFiles = ["app.mjs", "research-config.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-zoning-direct-rule-confirmation-20260908.mjs"];
...
const scratch = await mkdtemp(join(tmpdir(), live ? "permitext-zoning-direct-rule-live-" : "permitext-zoning-direct-rule-preflight-"));
...
Object.assign(process.env, {
  ...
  PERMITEXT_RESEARCH_DAILY_CAP_USD: String(profile.maximumCumulativeSpendUSD), PERMITEXT_RESEARCH_MONTHLY_CAP_USD: String(profile.maximumCumulativeSpendUSD), PERMITEXT_RESEARCH_MONTHLY_REQUEST_LIMIT: "2",
...
if (!live) {
  assert.equal(result.status, "preflight-passed", result.stopReason);
