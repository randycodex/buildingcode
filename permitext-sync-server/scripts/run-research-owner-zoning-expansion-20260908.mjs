// Expand live owner coverage across discontinuance and divided-lot use rules; isolated HTTP, existing routing.
...
assert.equal(previousResultHashes.length, 21);
...
assert.equal(previousConservativeSpendUSD, 7.876233);
...
const caseIDs = ["ZR-21", "ZR-18"];
assert(caseIDs.every((id) => !attempted.has(id)), "This package attempts two previously unattempted numbered owner cases.");
const profile = { schema: "permitext-owner-zoning-expansion-v1", sourceCommit,
  authorization: "Owner authorized approximately $8 of API tests and repeated testing toward the 110-case answer target. This new package attempts ZR-21 and ZR-18 once each to expand coverage after prompt compaction, within the existing remaining authorization; it cannot replay a consumed package.",
  previousResultHashes, previousConservativeSpendUSD, authorizationUSD: 8,
  maximumCumulativeSpendUSD: 0.123767, maximumTurnSpendUSD: 0.06, maximumTurns: 2, maximumProviderRequests: 6,
...
const preflightURL = new URL("evals/results/research-owner-zoning-expansion-preflight-2026-09-08.json", root);
const resultURL = new URL("evals/results/research-owner-live-zoning-expansion-2026-09-08.json", root);
const sourceFiles = ["app.mjs", "research-config.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-zoning-expansion-20260908.mjs"];
...
const scratch = await mkdtemp(join(tmpdir(), live ? "permitext-zoning-expansion-live-" : "permitext-zoning-expansion-preflight-"));
