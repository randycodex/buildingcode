// Check the four remaining owner code cases; dispatch only preflight-eligible turns.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-work-premise-2026-09-08.json";
...
const caseIDs = ["GAP-03", "GAP-08", "MC-09", "MC-13"];
for (const id of caseIDs) assert(!attempted.has(id));
const profile = { schema: "permitext-owner-api-round2-remaining-code-v1", sourceCommit,
  authorization: "Owner explicitly authorized fresh API tests with $8 currently available and requested notice if more is needed. This is a new $8 round; historical costs stay separate. This single-use batch checks the four remaining owner code cases and dispatches only turns that fit the offline preflight. Maximum $2.00 for the batch within the remaining $8 round allowance. Each next turn requires a full $0.50 remaining conservative allowance. No Project workflow tests or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(2, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 4, maximumProviderRequests: 20, maximumProviderRequestsPerTurn: 5,
  ...
  scope: "Actual automatic-retrieval HTTP Research flow on isolated local accounts and storage. Expected answers are not supplied to drafting or verification. Existing automatic web support and official-document validation remain enabled. Offline preflight intercepts every provider call and forbids document network access. Live document reads are limited to HTTPS GETs on the configured official domains. Preflight-blocked turns are skipped without spending. No public eligibility, deployment, Project workflow or phone changes.",
  cases: caseIDs };
...
const preflightURL = new URL("evals/results/research-owner-api-round2-remaining-code-preflight-2026-09-08.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-remaining-code-2026-09-08.json", root);
const sourceFiles = ["app.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-remaining-code-20260908.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-blocked" || (item.initialRequestSHA256 && item.initialMaximumUSD <= profile.maximumTurnSpendUSD)));
...
globalThis.fetch = async (url, options) => {
  if (String(url) !== "https://api.openai.com/v1/responses") {
    assert(live, "Offline preflight forbids document network requests.");
    const documentURL = new URL(String(url));
    assert(documentURL.protocol === "https:" && !documentURL.username && !documentURL.password);
    assert(["nyc.gov", "ny.gov", "ada.gov", "rules.cityofnewyork.us"].some((host) => documentURL.hostname === host || documentURL.hostname.endsWith(`.${host}`)));
    assert(!options?.method || options.method === "GET");
    assert(!new Headers(options?.headers).has("authorization"));
    result.documentRequests.push({ caseID: active?.id, url: String(url), requestedAt: new Date().toISOString() });
    await persist();
...
try {
  ...
  for (const id of caseIDs) {
    if (live && preflight.results.find((item) => item.id === id).status === "preflight-blocked") continue;
    ...
    if (!live) {
      if (/SPEND_CAP/.test(response.body.code || "")) {
