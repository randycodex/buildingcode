// Confirm decision-scoped verification and complete filing consequences on two unchanged workflow questions once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-source-relationships-2026-09-09.json";
...
const repairedCaseIDs = ["DOBNOW-023", "DOBNOW-003"];
const newCaseIDs = [];
...
assert.equal(roundPreviousConservativeUSD, 7.833214);
...
assert.equal(researchOfficialGuidanceSummaryPromptVersion, "20260909-document-summary-v10");
const profile = { promptVersion: "20260909-document-summary-v10", schema: "permitext-owner-api-round2-decision-scope-v1", sourceCommit,
  authorization: "The owner explicitly approved an additional $0.50 for the next capped comparison, raising the fresh-round authorization from $8 to $8.50. Historical costs remain separate. This single-use comparison tests prompt v10 decision scope, compact answers and cross-source filing-completion consequences on unchanged authored DOBNOW-023 and DOBNOW-003 scenarios and questions, once each. The preceding conservative round total is $7.833214, leaving $0.666786. Maximum $0.40 for two unassigned Research HTTP turns: at most $0.22 for DOBNOW-023 and $0.20 for DOBNOW-003, two provider requests per turn and four total. No paid search, manual retry, separate judge, Project workflow or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.4, Number((authorizationAmendment.authorizationUSD - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.22, turnSpendCapsUSD: { "DOBNOW-023": 0.22, "DOBNOW-003": 0.2 }, maximumTurns: 2, maximumProviderRequests: 4, maximumProviderRequestsPerTurn: 2,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-decision-scope-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-decision-scope-2026-09-09.json", root);
...
globalThis.fetch = async (url, options) => {
  ...
  if (!active.initialRequestSHA256) {
    ...
    if (workflow?.directDocumentRetrieval) {
      ...
      assert.equal(input.sourceRelationships.length, active.id === "DOBNOW-023" ? 1 : 2, "All fetched source relationships must reach drafting.");
      ...
      if (active.id === "DOBNOW-023") {
        const roles = input.passages.filter((passage) => passage.sourceID === "dob-stakeholder-faq").map((passage) => passage.text).join(" ");
        ...
        const steps = input.passages.filter((passage) => passage.sourceID === "dob-filing-submission-steps").map((passage) => passage.text).join(" ");
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner decision scope confirmation" } });
