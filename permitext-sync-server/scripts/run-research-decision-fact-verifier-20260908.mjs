...
const terminalFile = "evals/results/research-owner-live-compact-confirmation-2026-09-08.json";
...
assert.equal(previousResultHashes.length, 16);
assert.equal(new Set(previousResultHashes.map((entry) => entry.file)).size, 16);
...
assert.equal(previousConservativeSpendUSD, 7.749843);
const sourceCommit = execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }).trim();
const profile = { schema: "permitext-decision-fact-verifier-diagnostic-v1", sourceCommit,
  authorization: "Owner authorized repeated Research testing within approximately $8 total. This package uses at most $0.24 of the remaining $0.250157 conservative allowance for seven production-verifier checks, including contrasting applicability and requested-design controls.",
  previousResultHashes, previousConservativeSpendUSD, authorizationUSD: 8, maximumCumulativeSpendUSD: .24,
  maximumTurnSpendUSD: .24, maximumChecks: 7, maximumProviderRequests: 14,
  ...
  originalFullHTTPTurnAttempts: 66, originalProviderCalls: 139,
...
const nextBudget = (operations) => ownerResearchNextTurnBudget({ previousConservativeUSD: previousConservativeSpendUSD, operations,
  authorizationUSD: 8, maximumBatchUSD: .24, maximumTurnUSD: .24 });
nextBudget([]);
const environment = { ...researchRequestEnvelopeEnvironment, PERMITEXT_RESEARCH_MAX_REQUEST_USD: ".24",
...
const preflightURL = new URL("evals/results/research-decision-fact-verifier-preflight-2026-09-08.json", root);
...
for (const fixture of fixtures) {
  ...
  preflightEntries.push({ id: fixture.id, expectedPass: fixture.expectedPass, expectedIssue: fixture.expectedIssue || null,
...
assert(preflightEntries.reduce((sum, item) => sum + item.maximumRequestUSD, 0) < .24);
const preflightPayload = { ...profile, fixtureSourceSHA256: hash(await readFile(new URL("evals/research-decision-fact-fixtures.mjs", root))),
...
if (!process.argv.includes("--run-live")) {
  ...
  console.log(JSON.stringify({ status: "preflight-passed-no-provider", sourceCommit, entries: preflightEntries, capUSD: .24 }));
  process.exit(0);
}
...
const resultURL = new URL("evals/results/research-owner-live-decision-fact-verifier-2026-09-08.json", root);
...
Object.assign(process.env, environment, { OPENAI_API_KEY: local.OPENAI_API_KEY, PERMITEXT_RUN_PAID_RESEARCH_EVALS: "1", PERMITEXT_RESEARCH_EVAL_MAX_USD: ".24" });
...
try {
  for (const fixture of fixtures) {
    ...
    const entry = { id: fixture.id, sourceCaseID: fixture.sourceCaseID, scope: "verifier-only", expectedPass: fixture.expectedPass,
      expectedIssue: fixture.expectedIssue || null, expectedMissingFactIndices: fixture.expectedMissingFactIndices || [], purpose: fixture.purpose, status: "running", operations: [] };
    ...
    try {
      ...
      entry.expectationMatched = output.result.pass === fixture.expectedPass &&
        (!fixture.expectedIssue || output.result.issues.some((issue) => issue.type === fixture.expectedIssue)) &&
        JSON.stringify([...(output.result.unnecessaryMissingFactIndices || [])].sort((a, b) => a - b)) === JSON.stringify(entry.expectedMissingFactIndices);
