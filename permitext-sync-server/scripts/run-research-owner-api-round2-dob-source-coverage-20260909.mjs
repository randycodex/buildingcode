// Attempt eight untested DOB workflow cases once after offline source and HTTP preflight checks.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-zoning-repair-preservation-2026-09-09.json";
...
const caseIDs = ["DOBNOW-001", "DOBNOW-002", "DOBNOW-005", "DOBNOW-007", "DOBNOW-008", "DOBNOW-009", "DOBNOW-020", "DOBNOW-021"];
assert(caseIDs.every((id) => !attempted.has(id)));
const profile = { schema: "permitext-owner-api-round2-dob-source-coverage-v1", sourceCommit,
  authorization: "Owner explicitly authorized fresh API tests with $8 currently available and requested notice if more is needed. This is a new $8 round; historical costs stay separate. This single-use batch attempts eight previously untested DOB workflow cases. Their original scenarios and questions are preserved with the packet's shared DOB NOW workflow context, explicitly recovered and validated independently of the answers. It dispatches only turns passing offline preflight. Maximum $1.75 for the batch within the remaining $8 round allowance. Each next turn requires a full $0.50 remaining conservative allowance. No Project workflow tests or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(1.75, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 8, maximumProviderRequests: 40, maximumProviderRequestsPerTurn: 5,
  ...
  scope: "Actual unassigned HTTP Research on isolated local accounts and storage. No Project is created or exercised. Each question includes the source packet's explicit DOB NOW workflow context plus its unchanged scenario and question. No answer key, rubric, required-citation hint or professional-review expectation is supplied to drafting or verification. Existing automatic official web search and public NYC document retrieval are enabled; no model or routing substitution is made. The offline preflight intercepts provider dispatch and forbids document network calls. Earlier no-context attempts and all 110 numbered cases remain in the cumulative scope; this eight-case batch does not narrow acceptance. No live filing, saved-project workflow, phone, UI or deployment changes.",
  cases: caseIDs };
...
const preflightURL = new URL("evals/results/research-owner-api-round2-dob-source-coverage-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-dob-source-coverage-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-official-pdf-ranking.mjs", "research-official-pdf-worker.mjs", "research-dob-workflow-routing.mjs", "evals/research-answer-key-reconciliation.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-dob-source-coverage-20260909.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "All eight DOB inputs must pass the offline HTTP preflight before live dispatch.");
...
globalThis.fetch = async (url, options) => {
  if (String(url) !== "https://api.openai.com/v1/responses") {
    ...
    if (!live || parsed.protocol !== "https:" || !(parsed.hostname === "nyc.gov" || parsed.hostname.endsWith(".nyc.gov")) || options?.method !== "GET") {
      throw stop("This package permits only public NYC document GET requests during live Research.");
    }
    const documentRequest = { caseID: active?.id, url: String(url), startedAt: new Date().toISOString() };
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner DOB diagnostic" } });
