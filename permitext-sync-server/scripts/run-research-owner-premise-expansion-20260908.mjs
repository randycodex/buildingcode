// One bounded owner-authorized premise check and expansion to twelve new cases.
...
const cases = ["PC-03", "PC-02", "FGC-01", "FGC-05", "FGC-06", "FGC-07", "MC-02", "MC-03", "MC-04", "MC-07", "GAP-01", "GAP-02", "PC-01"].map((id) => {
...
const attributionBytes = await readFile(new URL("evals/results/research-owner-live-attribution-confirmation-2026-09-08.json", serverRoot));
const attributionRun = JSON.parse(attributionBytes);
assert.equal(attributionRun.status, "completed");
assert.equal(attributionRun.spend.pendingRequestCount, 0);
assert.equal(attributionRun.providerCalls.length, 9);
const attributionOperations = attributionRun.results.flatMap((item) => item.operations || []);
assert.equal(attributionOperations.length, 3);
assert(attributionOperations.every((item) => item.pendingProviderRequestCount === 0));
const previousConservativeSpendUSD = Number((earlierPilotConservativeSpendUSD + [...confirmationOperations, ...farOperations, ...qualityOperations, ...scopeOperations, ...documentOperations, ...fixtureOperations, ...technicalOperations, ...attributionOperations].reduce((sum, item) => sum + item.conservativeProviderCostUSD, 0)).toFixed(6));
assert.equal(previousConservativeSpendUSD, 4.559495);
...
assert(previousConservativeSpendUSD + 3.2 <= 8, "Retain the owner's total authorization across runs.");
const profile = {
  schema: "permitext-owner-premise-expansion-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. This package is limited to one premise-verification confirmation and twelve previously untested technical cases; one new attempt per case, with a sequential $3.20 conservative aggregate cap.",
  previousConservativeSpendUSD,
  priorAttributionResultSHA256: createHash("sha256").update(attributionBytes).digest("hex"),
  ...
  maximumCumulativeSpendUSD: 3.2,
  maximumTurnSpendUSD: 0.85,
  maximumTurns: 13,
  authorizationUSD: 8,
  turnBudgetPolicy: "Before every turn, cap its complete provider pipeline at the lesser of $0.85 and the remaining settled conservative $3.20 batch budget; never continue after missing or pending accounting.",
  repetitions: 1,
...
nextBudget();
const preflightURL = new URL("evals/results/research-owner-premise-expansion-preflight-2026-09-08.json", serverRoot);
if (!process.argv.includes("--run-live")) {
  ...
  for (const item of cases) {
    ...
    results.push({ id: item.id, references, missingReferences, sources });
  }
  assert.equal(networkAttempts, 0);
  await writeFile(preflightURL, `${JSON.stringify({ schema: "permitext-owner-premise-preflight-v1", sourceCommit: profile.sourceCommit, cases, results, networkAttempts, providerCalls: 0 }, null, 2)}\n`, { flag: "wx" });
...
const resultURL = new URL("evals/results/research-owner-live-premise-expansion-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-premise-expansion-"));
...
Object.assign(process.env, {
  ...
  PERMITEXT_RESEARCH_EVAL_MAX_USD: "3.2",
  PERMITEXT_RESEARCH_MAX_REQUEST_USD: "0.85",
  PERMITEXT_RESEARCH_USER_DAILY_CAP_USD: "3.2",
  PERMITEXT_RESEARCH_USER_MONTHLY_CAP_USD: "3.2",
  PERMITEXT_RESEARCH_DAILY_CAP_USD: "3.2",
  PERMITEXT_RESEARCH_MONTHLY_CAP_USD: "3.2",
  PERMITEXT_RESEARCH_MONTHLY_REQUEST_LIMIT: "13",
