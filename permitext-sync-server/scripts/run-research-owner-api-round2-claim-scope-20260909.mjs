// Confirm claim-scoped verification and complete filing consequences on two unchanged workflow questions once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-decision-scope-2026-09-09.json";
...
assert.equal(roundPreviousConservativeUSD, 7.921699);
...
assert.equal(researchOfficialGuidanceSummaryPromptVersion, "20260909-document-summary-v11");
const profile = { promptVersion: "20260909-document-summary-v11", schema: "permitext-owner-api-round2-claim-scope-v1", sourceCommit,
  authorization: "The owner authorized continued API testing toward the full Research goal within an $8 allowance, then approved an additional $0.50 to the testing allowance. The prior scoped comparison and amendment remain immutable in the ledger. This single-use continuation uses at most $0.50 of the $0.578301 aggregate remaining allowance; the total round cap stays $8.50. It tests the unchanged DOBNOW-003 and DOBNOW-004 authored questions once each on prompt v11. Two unassigned Research HTTP turns, at most $0.22 for DOBNOW-003 and $0.28 for DOBNOW-004, two provider calls per turn and four total. No paid search, manual retry, separate judge, Project workflow, deployment or price change.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.5, Number((authorizationAmendment.authorizationUSD - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.28, turnSpendCapsUSD: { "DOBNOW-003": 0.22, "DOBNOW-004": 0.28 }, maximumTurns: 2, maximumProviderRequests: 4, maximumProviderRequestsPerTurn: 2,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-claim-scope-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-claim-scope-2026-09-09.json", root);
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner claim scope confirmation" } });
