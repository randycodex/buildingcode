// Confirm three repaired Zoning paths once after offline contract and HTTP preflight checks.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-zoning-source-repair-v2-2026-09-09.json";
...
const caseIDs = ["ZR-05", "ZR-07", "ZR-11"];
const profile = { schema: "permitext-owner-api-round2-zoning-repair-preservation-v1", sourceCommit,
  authorization: "Owner explicitly authorized fresh API tests with $8 currently available and requested notice if more is needed. This is a new $8 round; historical costs stay separate. This single-use batch confirms three repaired Zoning cases, preserving their authored questions, selected passages and section references, and dispatches only turns that pass offline preflight. Maximum $1.00 for the batch within the remaining $8 round allowance. Each next turn requires a full $0.50 remaining conservative allowance. No Project workflow tests or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(1, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 3, maximumProviderRequests: 15, maximumProviderRequestsPerTurn: 5,
  ...
  scope: "Actual selected-passage HTTP Research flow on isolated local accounts and storage. No Project is created or exercised. This confirmation tests complete amendment repair sources and source-basis labels, unresolved-FAR assertion classification, and lot-coverage numeric relationships. Amendment-history observations use the identified corpus metadata snapshot; no live refresh or historical enacted-rule completeness is claimed. Oversized section selections and visual-review requirements are recorded, never bypassed. Expected answers are not supplied to drafting or verification. Automatic web support remains enabled in configuration; each selected-code question must avoid it. The driver rejects web tools and document reads. Offline preflight intercepts every provider call. Preflight-blocked turns are skipped without spending. No public eligibility, deployment, Project workflow or phone changes.",
  cases: caseIDs };
...
const preflightURL = new URL("evals/results/research-owner-api-round2-zoning-repair-preservation-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-zoning-repair-preservation-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "scripts/run-research-owner-api-round2-zoning-repair-preservation-20260909.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "All three repaired paths must pass the offline HTTP preflight before live dispatch.");
