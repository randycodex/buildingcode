...
const previousPackages = [
  ...
  ["attribution-confirmation-2026-09-08", 3, 9], ["premise-expansion-2026-09-08", 13, 28], ["routing-confirmation-2026-09-08", 1, 3, "stopped"], ["routing-confirmation-v2-2026-09-08", 3, 6], ["fountain-expansion-2026-09-08", 7, 15, "stopped"]
];
...
assert.equal(previousConservativeSpendUSD, 7.376368);
const profile = {
  schema: "permitext-owner-plumbing-repair-confirmation-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. Two plumbing source repair confirmations and three previously undispatched plumbing cases, one attempt each, capped at $0.60 conservative aggregate with no manual retries.",
  ...
  maximumCumulativeSpendUSD: 0.60,
  maximumTurnSpendUSD: 0.50,
  maximumTurns: 5, authorizationUSD: 8,
  turnBudgetPolicy: "Cap each complete provider pipeline at the lesser of $0.50 and the remaining settled conservative $0.60 batch budget. Missing or pending accounting stops all further requests.",
...
nextBudget();
const preflightURL = new URL("evals/results/research-owner-plumbing-repair-confirmation-preflight-2026-09-08.json", serverRoot);
if (!process.argv.includes("--run-live")) {
  ...
  await writeFile(preflightURL, `${JSON.stringify({ schema: "permitext-owner-plumbing-repair-confirmation-preflight-v1", sourceCommit: profile.sourceCommit, cases, results, networkAttempts, providerCalls: 0 }, null, 2)}\n`, { flag: "wx" });
...
const resultURL = new URL("evals/results/research-owner-live-plumbing-repair-confirmation-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-plumbing-repair-confirmation-"));
