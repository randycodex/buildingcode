// Confirm the stated historical premise repair on the authored ZR-19 case once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-section-reference-2026-09-09.json";
...
assert.equal(roundPreviousConservativeUSD, 7.283466);
const profile = { schema: "permitext-owner-api-round2-lot-history-v1", sourceCommit,
  authorization: "Owner authorized continued API testing within a fresh $8 round and requested notice before more is needed. Historical spending stays separate. This single-use batch confirms the historical-premise repair on ZR-19 once, preserving its authored question and section reference. Batch maximum $0.50 within the round's $0.716534 conservative remainder. The turn requires a full $0.50 conservative reservation. No manual retry, extra judge, Project workflow test or deployment.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.5, Number((8 - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.5, maximumTurns: 1, maximumProviderRequests: 5, maximumProviderRequestsPerTurn: 5,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-lot-history-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-lot-history-2026-09-09.json", root);
const sourceFiles = ["app.mjs", "research-answer-presentation.mjs", "research-official-guidance-summary.mjs", "evals/research-answer-key-amendments.json", "research-answer-quality.mjs", "research-focused-technical-scope.mjs", "research-conversation-topic.mjs", "research-technical-topic-routes.mjs", "evidence-discovery.mjs", "research-conversation-facts.mjs", "research-fact-qualification.mjs", "research-source-policy.mjs", "research-official-html-attribution.mjs", "research-official-pdf-attribution.mjs", "research-official-pdf-ranking.mjs", "research-official-pdf-worker.mjs", "research-dob-workflow-routing.mjs", "evals/research-answer-key-reconciliation.mjs", "research-config.mjs", "research-cost-usage.mjs", "research-provider-client.mjs", "research-zoning-planner.mjs", "research-zoning-temporal-application.mjs", "research-zoning-safety.mjs",
  ...
  "evals/research-reconciled-answer-key.json", "evals/research-owner-code-candidates.json", "evals/research-owner-code-review.mjs", "evals/research-owner-http-request-binding.mjs", "research-zoning-lot-history.mjs", "research-project-fact-coverage.mjs", "scripts/run-research-owner-api-round2-lot-history-20260909.mjs"];
...
if (live) {
  ...
  assert(preflight.results.every((item) => item.status === "preflight-intercepted"), "The authored ZR-19 input must pass the no-API HTTP preflight before live dispatch.");
...
globalThis.fetch = async (url, options) => {
  ...
  if (!active.initialRequestSHA256) {
    ...
    assert.equal(active.id, "ZR-19");
    assert.match(String(body.input), /Do not reopen/);
    const premiseLine = String(body.input).split("\n").find((line) => line.startsWith("DETERMINISTIC_CONTEXT: "));
    assert(premiseLine);
    const history = JSON.parse(premiseLine.slice("DETERMINISTIC_CONTEXT: ".length)).answerObligations.find((item) => item.id === "definition_historical_branches");
