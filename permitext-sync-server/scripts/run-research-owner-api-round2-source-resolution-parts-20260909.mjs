// Confirm source-resolution drafting on two unchanged workflow questions with explicit rendered references once.
...
const firstRoundFile = "evals/results/research-owner-api-round2-live-source-resolutions-2026-09-09.json";
...
const repairedCaseIDs = ["DOBNOW-003", "DOBNOW-023"];
const newCaseIDs = [];
...
assert.equal(roundPreviousConservativeUSD, 8.214661);
...
const draftComparisonFile = "evals/results/research-source-resolution-parts-replay-2026-09-09.json";
const draftComparisonBytes = await readFile(new URL(draftComparisonFile, root));
assert.equal(hash(draftComparisonBytes), "f765f4696e3bd7356b88c44eca211003c3b8990aa5325c70f140f6f676468446");
...
assert.equal(draftComparison.summary.syntheticFixtures, 3);
assert.equal(draftComparison.summary.relationshipsRendered, 4);
assert.equal(draftComparison.summary.freeTextClaimsRemovedOrRewritten, 0);
assert.equal(draftComparison.summary.providerCalls, 0);
const profile = { promptVersion: "20260909-document-summary-v14", schema: "permitext-owner-api-round2-source-resolution-parts-v1", sourceCommit,
  draftComparisonFile, draftComparisonSHA256: hash(draftComparisonBytes),
  authorization: "The owner authorized continued API testing toward the full Research goal within an $8 allowance, then approved an additional $0.50 to the testing allowance. The prior scoped comparison and amendment remain immutable in the ledger. This single-use continuation uses at most $0.28 of the $0.285339 aggregate remaining allowance; the total round cap stays $8.50. It tests the unchanged DOBNOW-003 and DOBNOW-023 authored questions once each on prompt v14. DOBNOW-004 and all other numbered cases remain pending in the full 110-question acceptance scope. Each draft must provide findings and explicit paragraph references within the existing drafting call; the renderer inserts those model-authored findings, preserves ordinary prose, and binds all required citations; independent semantic verification remains required. Two unassigned Research HTTP turns, at most $0.20 each, two provider calls per turn and four total. The batch stops if the remaining cap cannot reserve a complete next turn. No paid search, manual retry, separate judge, Project workflow, deployment or price change.",
  ...
  maximumCumulativeSpendUSD: Math.min(0.28, Number((authorizationAmendment.authorizationUSD - roundPreviousConservativeUSD).toFixed(6))), maximumTurnSpendUSD: 0.20, turnSpendCapsUSD: { "DOBNOW-003": 0.20, "DOBNOW-023": 0.20 }, maximumTurns: 2, maximumProviderRequests: 4, maximumProviderRequestsPerTurn: 2,
...
const preflightURL = new URL("evals/results/research-owner-api-round2-source-resolution-parts-preflight-2026-09-09.json", root);
const resultURL = new URL("evals/results/research-owner-api-round2-live-source-resolution-parts-2026-09-09.json", root);
...
try {
  const signed = await request("/account/sign-in", { credential: { provider: "web", providerUserID: randomUUID(), displayName: "Isolated owner source resolution confirmation" } });
