// Bounded plumbing repair confirmation and three previously undispatched cases.
...
const cases = ["PC-03", "PC-04", "PC-10", "PC-11", "PC-13"].map((id) => {
...
const previousPackages = [
  ...
  ["attribution-confirmation-2026-09-08", 3, 9], ["premise-expansion-2026-09-08", 13, 28], ["routing-confirmation-2026-09-08", 1, 3, "stopped"], ["routing-confirmation-v2-2026-09-08", 3, 6], ["fountain-expansion-2026-09-08", 7, 15, "stopped"], ["plumbing-repair-confirmation-2026-09-08", 1, 2, "stopped"]
];
...
assert.equal(previousConservativeSpendUSD, 7.488284);
const profile = {
  schema: "permitext-owner-plumbing-repair-confirmation-v2-20260908-v1",
  authorization: "Owner: approximately $8 available for API testing; test as many times as needed. Two plumbing source repair confirmations and three previously undispatched plumbing cases, one attempt each, capped at $0.50 conservative aggregate with no manual retries.",
  ...
  maximumCumulativeSpendUSD: 0.50,
  maximumTurnSpendUSD: 0.50,
  maximumTurns: 5, authorizationUSD: 8,
  turnBudgetPolicy: "Cap each complete provider pipeline at the lesser of $0.50 and the remaining settled conservative $0.50 batch budget. Missing or pending accounting stops all further requests.",
...
nextBudget();
const preflightURL = new URL("evals/results/research-owner-plumbing-repair-confirmation-v2-preflight-2026-09-08.json", serverRoot);
if (!process.argv.includes("--run-live")) {
  ...
  for (const item of cases) {
    ...
    results.push({ id: item.id, references, completeReferences, missingReferences, sources, requiredReferences, selectedCorpora, useWeb });
  }
  assert.equal(networkAttempts, 0);
  await writeFile(preflightURL, `${JSON.stringify({ schema: "permitext-owner-plumbing-repair-confirmation-v2-preflight-v1", sourceCommit: profile.sourceCommit, cases, results, networkAttempts, providerCalls: 0 }, null, 2)}\n`, { flag: "wx" });
...
const resultURL = new URL("evals/results/research-owner-live-plumbing-repair-confirmation-v2-2026-09-08.json", serverRoot);
const lock = await open(resultURL, "wx", 0o600);
await lock.close();
const scratch = await mkdtemp(join(tmpdir(), "permitext-owner-plumbing-repair-confirmation-v2-"));
