...
const directory = await mkdtemp(join(tmpdir(), "permitext-trash-test-"));
...
try {
  ...
  const trash = async (action = "list", id, confirmation, accountID=userID) => post("/content/trash", {auth:{accountUserID:accountID},action,id,confirmation}, account.backendSessionToken);
  ...
  assert.equal((await push([deletion])).rejectedMutationIDs.length, 0);
  let list = await trash();
  assert.equal(list.status,200,JSON.stringify(list.body));
  assert.equal(list.body.entries.length,1);
  const id = list.body.entries[0].id;
  assert([401,403].includes((await trash("list",null,null,"not-owner")).status));
  assert.equal((await post("/content/trash",{})).status,400);
  assert.equal((await trash("empty")).status,400);
  assert.equal((await trash("purge",id,"delete")).status,400);
  await push([deletion]);
  assert.equal((await trash()).body.entries.length,1,"Retry cannot duplicate recovery");
  const restored = await trash("restore",id);
  assert.equal(restored.status,200,JSON.stringify(restored.body));
  assert.equal(restored.body.restoredCount,1,"Free can recover existing content");
  assert.equal(restored.body.entries.length,0);
  const pull = await post("/sync/pull",{auth:{accountUserID:userID}},account.backendSessionToken);
  const restoredSave=pull.body.mutations.find(m=>m.savedItem?.sectionID===545).savedItem;
  assert(!restoredSave.deletedAt);
  assert(Date.parse(restoredSave.updatedAt)>Date.parse(deletion.savedItem.updatedAt));
  assert.equal((await trash("restore",id)).status,409);
  const clear = {codeVersionClear:{userID,codeVersion,updatedAt:new Date(Date.now()+4000).toISOString(),values:{scope:"notes"}}};
  await push([clear]);
  list=await trash();
  assert.equal(list.body.entries.length,1);
  assert.equal(list.body.entries[0].count,1);
  assert.equal((await trash("restore",list.body.entries[0].id)).body.restoredCount,1);
  const deleteAgain={savedItem:{...restoredSave,updatedAt:new Date(Date.now()+6000).toISOString(),deletedAt:new Date().toISOString()}};
  await push([deleteAgain]);
  const purgeID=(await trash()).body.entries[0].id;
  assert.equal((await trash("purge",purgeID,"DELETE")).status,200);
  assert.equal((await trash("restore",purgeID)).status,409);
  assert.equal((await trash()).body.entries.length,0);
  console.log("Trash HTTP: deletion, retry, cross-client pull, Free restoration, bulk notes, typed confirmation, ownership, permanent removal passed.");
