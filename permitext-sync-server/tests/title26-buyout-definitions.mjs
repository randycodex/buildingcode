...
test('extracts exactly three complete source bodies with immutable provenance and inactive scope',()=>{
 ...
 for(const term of terms){assert.equal(term.anchor,binding.anchor);assert.equal(term.sectionNumber,binding.section);assert.equal(term.referenceOnly,false);assert.equal(term.applicability,'review-required');assert.deepEqual(term.applicableChapters,['24']);assert.ok(!term.text.includes('L.L.'));}
});
