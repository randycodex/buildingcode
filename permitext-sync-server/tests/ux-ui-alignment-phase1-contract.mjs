...
assert.match(clientSource, /async function resumeClerkSignInReturn\(\)[\s\S]*?searchParams\.delete\("clerk_return"\)[\s\S]*?state\.utilities\.settings = true;[\s\S]*?catch \(error\)[\s\S]*?Sign-in could not be completed\./);
assert.doesNotMatch(clientSource, /async function resumeClerkSignInReturn\(\)[\s\S]*?if \(!config\.available\) throw new Error\("Secure sign-in is not configured\."\)/);
assert.match(clientSource, /if \(clerkSignInReturnNotice\) \{[\s\S]*?setStatus\(clerkSignInReturnNotice\.message, clerkSignInReturnNotice\.isError\)/);
