...
const authorizationPath = new URL(
  "../evals/zoning-successor-remediation-3-v17-confirmation-paid-authorization.json",
  import.meta.url
);
...
const exactAuthorizationID = "49f15f55-dfa5-41e8-a938-789531076d8a";
...
for (const requiredGuard of [
  ...
  assert(runnerSource.includes(requiredGuard),
    `The v17 confirmation runner is missing guard: ${requiredGuard}`);
}
...
for (const requiredGuard of [
  ...
  assert(evaluatorSource.includes(requiredGuard),
    `The v17 confirmation evaluator is missing guard: ${requiredGuard}`);
}

const resultsBefore = (await readdir(resultsPath)).sort();
for (const args of [
  ["tests/research-evals.mjs",
    "--zoning-successor-remediation-3-v17-confirmation", "--run-live"],
  ["tests/research-evals.mjs",
    "--zoning-successor-remediation-3-v17-confirmation", "--run-live",
    "--repeat", "2"],
  ["tests/research-evals.mjs",
...
assert.deepEqual((await readdir(resultsPath)).sort(), resultsBefore,
  "A locked or malformed v17 attempt created a result file.");
...
assert.match(combinedOutput(mockPreflight), /30\/30/,
  "The no-cost v17 confirmation preflight did not cover the full frozen cohort.");
assert.deepEqual((await readdir(resultsPath)).sort(), resultsBefore,
  "The no-cost v17 confirmation preflight created a paid result file.");

const temporaryDirectory = await mkdtemp(
  join(tmpdir(), "permitext-zoning-remediation-3-v17-confirmation-")
);
try {
  ...
  for (const [mutate, expected] of [
    ...
    }, /wrong historical v16 authorization SHA/],
    [(fixture) => {
...
assert.equal(sha256(await readFile(authorizationPath, "utf8")),
  exactLockedAuthorizationSHA256);
assert.deepEqual((await readdir(resultsPath)).sort(), resultsBefore,
  "The v17 authorization contract created a paid result file.");
console.log(
  "Zoning remediation-successor-3 v17 confirmation authorization contract passed; locked scope, consumed v16 lineage, hostile-runtime scrubbing, paid-dispatch blocks, and 30/30 no-cost preflight remain intact."
);
