...
assert.match(styleSource, /\.research-message\.is-assistant\.is-pending\s*\{\s*min-block-size: min-content;/);
assert.match(styleSource, /\.research-progress-loading\s*\{[\s\S]*?grid-template-columns: auto minmax\(0, auto\) auto;[\s\S]*?justify-content: end;/);
assert.match(styleSource, /\.research-progress-elapsed\s*\{[\s\S]*?grid-column: 3;[\s\S]*?justify-self: end;/);
