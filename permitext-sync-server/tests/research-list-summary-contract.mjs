...
assert.match(stylesSource, /\.research-dialogue-pane > \.research-compact-composer \{[^}]*grid-template-columns: minmax\(0, 1fr\) auto;[^}]*padding-bottom: var\(--space-3\);[\s\S]*?\.research-dialogue-pane > \.research-compact-composer \.research-verification-note \{[^}]*grid-row: 1;[\s\S]*?\.research-dialogue-pane > \.research-compact-composer \.research-composer-box \{[^}]*grid-row: 2;/, "Conversation composer must share the New chat bottom position with its verification note above the input.");
...
assert.match(functionSource(clientSource, "renderNewResearchComposer"), /runResearchProgressSession\(progress, recoveredResearchProgressCallbacks\(conversationID\)\)/,
