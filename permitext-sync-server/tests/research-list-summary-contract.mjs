...
assert.match(clientSource, /function openResearchSourceInSectionDetail\(item, anchorPaneID, projectID = "", evidenceAnchor = null\)[\s\S]*?resolveInlineCodeSection\(codePrefix, sectionNumber\)[\s\S]*?newUtilityInstance\("sdc"\)[\s\S]*?openSectionDetail\(detailOwner\.id, searchResultDetail\(navigationItem\), \{[\s\S]*?anchorPaneID,[\s\S]*?evidenceAnchor[\s\S]*?\}\)/, "Answer sources do not resolve and open the compact detail column beside Research with optional passage anchoring.");
...
assert.match(clientSource, /reportButton\.textContent = "Add to Report"[\s\S]*?promoteNotebookCardToReport\(identity, activeCard\)/, "Notebook Notes do not expose explicit Report promotion.");
assert.match(clientSource, /function promoteNotebookCardToReport\(project, card\)[\s\S]*?kind: "paragraph"[\s\S]*?text: String\(card\.plainText \|\| ""\)\.trim\(\)[\s\S]*?derivedFrom:[\s\S]*?kind: "notebookCard"[\s\S]*?sourceSnapshotAt:[\s\S]*?evidenceLinks: structuredClone\(card\.evidenceLinks \|\| \[\]\)/, "Report promotion does not create an independent editable snapshot with Note and evidence provenance.");
...
assert.match(clientSource, /function selectReaderSectionForResearch\(sectionWrapper\)[\s\S]*?showResearchSelectionMenu\(\{[\s\S]*?\.\.\.passage,[\s\S]*?passages: \[passage\],[\s\S]*?projectID: sectionWrapper\.closest\("\.workspace-panel"\)\?\.dataset\.projectId \|\| "",[\s\S]*?rect[\s\S]*?\}, \{ pinned: true \}\)/, "Add to Research does not open the same Project-aware, unassigned-capable selection menu as highlighted Reader text.");
