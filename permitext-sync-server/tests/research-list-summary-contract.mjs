...
assert.match(clientSource, /const researchPaneScrollTop = track\.querySelector\([\s\S]*?utility:analysis[\s\S]*?\)\?\.scrollTop \?\? 0;[\s\S]*?refreshedResearchPane\.scrollTop = researchPaneScrollTop;/, "Opening a previous chat should preserve the Research list scroll position.");
