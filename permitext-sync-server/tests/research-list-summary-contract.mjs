...
assert.match(clientSource, /const citation = `\$\{notebookReferenceCodeTitle\(codePrefix\)\} · § \$\{source\.sectionNumber \|\| "Section"\}`;/, "Direct Reader evidence does not use the full code title in Notebook references.");
...
assert.doesNotMatch(clientSource, /research-selection-context-summary|updateSelectionContextSummary/, "The selected-passage menu still repeats the verbose Using context summary.");
...
assert.match(clientSource, /function unassignedSavedEvidenceKeys\(savedItems, projectSections, projects = \[\]\)[\s\S]*?projectRecords = \(projects \|\| \[\]\)\.filter\(folderIsProject\)[\s\S]*?!linkedSectionIDs\.has\(sectionID\)/, "Unassigned Saved must include legacy collection evidence while excluding evidence assigned to a real Project.");
...
assert.match(functionSource(clientSource, "refreshProjectSourceConsumers"), /notebook\.refreshReferenceSources\(\)[\s\S]*?report\.refreshSources\(\)/, "Project evidence changes must invalidate mounted Notebook and Report source choices together.");
