...
assert.match(clientSource, /reference\.placeholder = "Supporting section or official source \(optional\)"[\s\S]*?supportingReference: reference\.value/, "Detailed Research feedback must preserve the optional supporting-source reference added for tester review.");
...
assert.match(clientSource, /meta\.textContent = researchConversationDate\(conversation\.updatedAt \|\| conversation\.createdAt\);/, "Previous chats do not show their latest conversation date with a creation-date fallback.");
...
assert.match(newResearchComposerSource, /const currentProject = workspaceProject\(\);[\s\S]*?const initialProjectID = currentProject \? projectDetailKey\(currentProject\) : "";[\s\S]*?projectID: initialProjectID/, "New Research does not inherit the active workspace Project context.");
assert.doesNotMatch(newResearchComposerSource, /createResearchProjectSelect|research-start-project/, "New Research still exposes the removed per-composer Project selector instead of following the active workspace.");
...
assert.match(clientSource, /newChatButton\.className = "ghost-button research-new-chat"[\s\S]*?panel\.classList\.add\("has-research-history-footer"\);[\s\S]*?panel\.append\(newChatButton\);/, "New chat must mount as a fixed Research history footer rather than inside the scrolling conversation list.");
...
assert.equal((clientSource.match(/AI-assisted · Verify cited text/g) || []).length, 1, "Only the empty chat should expose the consolidated verification disclosure.");
...
assert.deepEqual(
  ...
  "Internal retrieval diagnostics must not become model-visible legal evidence limitations."
);
assert.doesNotThrow(
...
assert.doesNotMatch(clientSource, /notebook-add-to-report|reportButton\.textContent = reportBlock \? "Update in Report" : "Add to Report"/, "Notebook Notes should be added from the Report column.");
...
assert.match(clientSource, /function unassignedSavedEvidenceKeys\(savedItems, projectSections, projects = \[\], annotations = \[\]\)[\s\S]*?projectRecords = activeFolderRecords\(projects \|\| \[\]\)[\s\S]*?!linkedSectionIDs\.has\(sectionID\)/, "Unassigned saves must exclude evidence assigned to a Project or saved collection.");
...
assert.match(functionSource(clientSource, "renderSectionDetail"), /persistSectionBookmark\(sectionPayload, false, \{ undoPaneID: panel\.dataset\.paneId \}\)[\s\S]*?refreshVisibleSyncedDerivedState\(\)/, "Section-detail deletion must retain Undo in its originating column and update bookmark state without replacing unrelated panes.");
...
assert.match(functionSource(clientSource, "renderNewResearchComposer"), /runResearchProgressSession\(progress, recoveredResearchProgressCallbacks\(conversationID, \{ supplemental: Boolean\(instance\) \}\)\)/,
  "New Research must use the shared answer propagation handler.");
assert.match(functionSource(clientSource, "renderResearchConversation"), /pendingProgress\.retry = \(\) => void runResearchProgressSession\([\s\S]*?callbacks[\s\S]*?await runResearchProgressSession\(progress, recoveredResearchProgressCallbacks/,
  "Existing and recovered Research must use the shared answer propagation handler.");
// Execute that handler's propagation and navigation assertions; don't require
// three duplicated source implementations of the same callback.
await import("./research-client-context-recovery-contract.mjs");
...
assert.match(clientSource, /unassignedHeading\.textContent = "Unassigned saves"[\s\S]*?unassignedCountLabel\.textContent = String\(unassignedCount\)[\s\S]*?unassignedCount === 1 \? "1 unassigned save" : `\$\{unassignedCount\} unassigned saves`[\s\S]*?const organizeUnassigned = !instance\.organizeUnassigned;[\s\S]*?requestProjectSelection\(paneID, instance\.id,[\s\S]*?kind: organizeUnassigned \? "unassigned" : "none"/, "The Projects menu does not expose a counted Unassigned saves destination through the serialized Project-selection controller.");
assert.match(clientSource, /Saved passages and notes that are not assigned to a Project\./, "Unassigned saves explains that the collection is saved evidence outside a Project.");
assert.match(clientSource, /No unassigned saves match[\s\S]*?Clear the search to see every unassigned save\./, "A search that hides Unassigned saves must say the collection is unchanged.");
...
assert.match(clientSource, /panel\.classList\.add\("analysis-panel", "research-list-panel"\)[\s\S]*?if \(draftShowing && activeAccount\(\) && hasCapability\("research"\)\) \{[\s\S]*?panel\.classList\.add\("has-research-composer", "is-new-research-chat"\)/, "Only the dedicated New chat destination must reserve a fixed composer row.");
assert.match(clientSource, /function renderNewResearchComposer\(container, researchEnabled, instance = null\)[\s\S]*?input\.rows = 1;[\s\S]*?container\.append\(form\);/, "The dedicated New chat composer does not mount directly in its panel.");
assert.match(clientSource, /renderNewResearchComposer\(panel, true, instance\)/, "The dedicated New chat composer still scrolls inside history content.");
assert.match(clientSource, /newChatButton\.className = "ghost-button research-new-chat"[\s\S]*?panel\.classList\.add\("has-research-history-footer"\)[\s\S]*?panel\.append\(newChatButton\)/, "Research history does not end with the dedicated New chat action.");
assert.match(stylesSource, /\.analysis-panel\.has-research-composer > \.research-composer \{[\s\S]*?padding: var\(--space-3\) 0 var\(--panel-padding\);/, "The New chat composer does not retain its bottom position.");
...
assert.match(clientSource, /function openMobileMoreSheet\(\)[\s\S]*?workspace\.name[\s\S]*?"New Project"[\s\S]*?"Rename workspace"[\s\S]*?"Account"/, "The mobile More sheet does not contain workspace switching and management controls.");
