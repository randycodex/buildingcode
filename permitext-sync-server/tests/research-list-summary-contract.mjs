...
assert.match(clientSource, /researchButton\.onclick = \(\) => selectReaderSectionForResearch\(sectionWrapper\)/, "The Reader comment card does not use the direct Research flow.");
assert.match(clientSource, /async function selectReaderSectionForResearch\(sectionWrapper\)[\s\S]*?selectedOpenProjectID\(\)[\s\S]*?startNewResearchFromSelection\(\{[\s\S]*?\.\.\.passage,[\s\S]*?passages: \[passage\],[\s\S]*?projectID[\s\S]*?\}\)/, "Add to Research does not directly start a Project-aware, unassigned-capable Research conversation.");
