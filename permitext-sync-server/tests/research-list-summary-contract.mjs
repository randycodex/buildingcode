...
assert.match(stylesSource, /\.research-list-panel \.research-conversation-list \{[\s\S]*?width: calc\(100% \+ \(2 \* var\(--panel-padding\)\)\);[\s\S]*?margin-inline: calc\(-1 \* var\(--panel-padding\)\);/, "Research history highlights and row dividers must span the full column width.");
