...
assert.match(clientSource, /linkButton\.textContent = noteOnly \? "Link passage" : "Link to Note"[\s\S]*?\["context", "Context"\][\s\S]*?\["supports", "Supports"\][\s\S]*?\["conflicts", "Conflicts"\][\s\S]*?\["unresolved", "Unresolved"\]/, "The enacted-text selection menu does not expose lightweight Note linking and evidence relationships.");
...
assert.match(clientSource, /function researchActionIconSVG\(options = \{\}\)[\s\S]*?M12\.983 21\.186a1 1 0 0 1-1\.966 0/, "The Reader Research action does not use Lucide's Astroid icon.");
assert.match(clientSource, /function openReaderPassageNoteLinker\(panel, target, anchorElement\)[\s\S]*?readerPassageResearchSelection\(wrapper, target, anchorElement\)[\s\S]*?showResearchSelectionMenu\(selection, \{ pinned: true, noteOnly: true \}\)/, "The direct Reader Note action does not open the Note-linking workflow.");
assert.match(clientSource, /const selection = readerPassageResearchSelection\(button\.closest\("\.annotated-code-block"\), target, button\)[\s\S]*?await saveReaderPassage\(panel, section, reader, target, \{ silent: true \}\)[\s\S]*?showResearchSelectionMenu\(selection, \{ pinned: true, noteOnly: true \}\)/, "The Reader Note action does not save and link the passage without opening the legacy comment card.");
