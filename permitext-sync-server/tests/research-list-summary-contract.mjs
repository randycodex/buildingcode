...
assert.match(stylesSource, /\.research-list-panel\.has-research-history-footer \{[^}]*grid-template-rows: auto minmax\(0, 1fr\) auto;[^}]*\}[\s\S]*?\.research-new-chat \{[^}]*width: 100%;[^}]*min-height: 48px;[^}]*height: 48px;[^}]*margin: 0 0 calc\(var\(--panel-padding\) \+ var\(--space-2\) \+ 1\.2em \+ 4px\);/, "New chat must match the conversation box dimensions and align above its verification-note footer space.");
