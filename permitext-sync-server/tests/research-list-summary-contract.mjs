...
assert.match(clientSource, /authorship\.textContent = "Your professional analysis · Work in any order; linked material remains identified as a reference\."/, "Notebook does not preserve the professional-authorship and free-order workflow boundary.");
...
assert.match(clientSource, /currentResearchConversationLabel\(\)[\s\S]*?researchButton\.textContent = currentResearchLabel \? `Add to: \$\{currentResearchLabel\}` : "Research"[\s\S]*?addToCurrent: Boolean\(currentResearchLabel\)[\s\S]*?newResearchButton\.textContent = "New Research"/, "Reader passage actions do not identify the current Research destination and preserve a new-conversation action.");
