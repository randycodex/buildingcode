...
assert.match(stylesSource, /\.research-list-panel\.has-research-history-footer \{[^}]*grid-template-rows: auto minmax\(0, 1fr\) auto;[^}]*\}[\s\S]*?\.research-new-chat \{[^}]*width: 100%;[^}]*min-height: 48px;[^}]*height: 48px;/, "New chat must occupy the bottom row and match the 48px conversation box dimensions.");
