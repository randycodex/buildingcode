...
test('full registry Hotel proposal links only the ordinary plural application',async()=>{
 ...
 assert.equal(original.text,'A hotel is an inn having thirty or more sleeping rooms.');assert.equal(original.id,'43ebeca3182a8fe2e536');assert.equal(original.applicability,'review-required');
