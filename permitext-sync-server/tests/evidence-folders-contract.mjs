...
assert.match(savedFolderContextSource, /"Address"[\s\S]*?"Description"[\s\S]*?"Notebook"[\s\S]*?"Report Draft"[\s\S]*?"Coordination"[\s\S]*?context\.append\(controls\)[\s\S]*?populateSavedEvidenceSection[\s\S]*?title: "Blocknotes"[\s\S]*?appendProjectResearchHistory/);
...
assert.match(appSource, /\["Code Decisions", "project-code-decisions-button", projectHasOpenCodeDecisions, openProjectCodeDecisions, closeProjectCodeDecisions\]/);
