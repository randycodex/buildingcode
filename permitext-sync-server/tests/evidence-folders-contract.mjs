...
assert.match(savedFolderContextSource, /"Address"[\s\S]*?"Description"[\s\S]*?"Notebook"[\s\S]*?"Report Draft"[\s\S]*?"Coordination"[\s\S]*?context\.append\(controls\)[\s\S]*?populateSavedEvidenceSection/);
