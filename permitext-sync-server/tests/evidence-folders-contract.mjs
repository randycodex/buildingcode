...
assert.match(stylesSource, /\.saved-folder-context\.is-project \.saved-project-facts-section:not\(\.is-open\) > \.saved-project-facts-heading,[\s\S]*?\.saved-project-research-answers:not\(\.is-open\) > \.project-studio-section-heading \{[^}]*height: 40px;[^}]*min-height: 40px;/);
assert.match(stylesSource, /\.saved-folder-context\.is-project \.saved-project-facts-section:not\(\.is-open\)[\s\S]*?\.saved-project-research-toggle \{[^}]*height: 40px;[^}]*min-height: 40px;/);
assert.match(stylesSource, /\.saved-folder-context\.is-project \.project-section-motion:not\(\.is-open\)[^}]*\.project-section-toggle-chevron \{[^}]*width: 40px;[^}]*height: 40px;[^}]*flex-basis: 40px;/);
...
assert.match(appSource, /function renderAnnotationProjectEditor[\s\S]*?projectListToggle\.textContent = "Projects"[\s\S]*?chips\.inert = !open;/);
