...
assert.match(stylesSource, /\.is-saved-selecting \.saved-row-actions \{[\s\S]*?visibility: hidden;[\s\S]*?pointer-events: none;/, "Saved Evidence row actions must be hidden without changing row height during bulk selection.");
assert.doesNotMatch(stylesSource, /\.saved-panel\.is-saved-selecting \.saved-content[\s\S]*?padding-bottom:/, "Saved Evidence selection must not expand the evidence section with obsolete bulk-action padding.");
