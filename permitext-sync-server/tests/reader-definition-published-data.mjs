...
test('Housing Maintenance preserves complete inventory and reviewed general and Article 14 scopes',()=>{
 ...
 assert.equal(book.entries.find(entry=>entry.term==='Person').applicability,'definition-chapter');
 assert.ok(!definitionsForReader(registry,{bundle:book.bundle,codeSectionID:book.codeSectionID,chapterNumber:'3',sectionNumber:'27-2081'}).some(entry=>entry.term==='Person'));
 assert.ok(definitionsForReader(registry,{bundle:book.bundle,codeSectionID:book.codeSectionID,chapterNumber:'3',sectionNumber:'27-2075'}).some(entry=>entry.term==='Person'));
...
test('Housing Maintenance device-section private dwelling meaning stays in its section',()=>{
 ...
 assert.equal(select('27-2046').length,1);
 assert.equal(select('27-2046')[0].source.sectionNumber,'27-2004');
 assert.ok(select('27-2046')[0].text.startsWith('A private dwelling is any building or structure'));
 assert.notEqual(select('27-2046')[0].id,entries[0].id);
