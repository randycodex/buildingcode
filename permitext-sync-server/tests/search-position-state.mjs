...
vm.runInContext(['normalizeSearchResultSources','newUtilityInstance','normalizeUtilityInstances','searchPositionState','searchResultPositionKey'].map(extract).join('\n'),c);
const original={id:'search-a',key:'search',query:'concrete',codeFilters:['BC'],historyScrollTop:275,searchEdition:'2014',expandedResultSources:['BC|2014','MC|2022']};
...
assert.equal(restored.searchEdition,'2014');assert.deepEqual(Array.from(restored.expandedResultSources),['BC|2014','MC|2022']);
const legacy=c.normalizeUtilityInstances({utilityInstances:[{id:'legacy-search',key:'search',expandedResultSource:'BC|2014'}]})[0];
assert.deepEqual(Array.from(legacy.expandedResultSources),['BC|2014']);
