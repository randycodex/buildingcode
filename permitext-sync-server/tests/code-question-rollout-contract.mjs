...
assert.match(functionSource(workspaceScript, "replaceActiveAccount"), /accountRuntimeGeneration \+= 1/);
assert.match(functionSource(workspaceScript, "replaceActiveAccount"), /clearResearchAccountRuntime\(\)/);
assert.match(functionSource(workspaceScript, "storeSignedInAccount"), /replaceActiveAccount\(nextAccount\)/);
assert.match(functionSource(workspaceScript, "clearExpiredAccountSession"), /replaceActiveAccount\(null\)/);
...
assert.match(researchInterpretationSource, /Research details/);
...
assert.match(researchInterpretationSource, /appendResearchUnresolved\(evidenceReviewedBody, result\)/);
...
assert.match(workspaceHTML, /styles\.css\?v=[^"']+/);
...
assert.match(workspaceScript, /detailsBody\.append\(metadata\)/);
