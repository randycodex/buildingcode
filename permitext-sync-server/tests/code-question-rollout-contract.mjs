...
assert.match(workspaceHTML, /styles\.css\?v=20260812-research-history-selection-v41/);
assert.match(serviceWorker, /permitext-pro-shell-v622/);
assert.match(workspaceScript, /paneID\?\.startsWith\("research:conversation:"\)/);
assert.match(serviceWorker, /styles\.css\?v=20260812-research-history-selection-v41/);
...
assert.match(workspaceScript, /const researchSurfaceIDs = state\.utilities\.analysis[\s\S]*?\["utility:analysis", paneIDForResearchConversation\(\)\]\.filter\(Boolean\)/);
assert.match(functionSource(workspaceScript, "defaultActivePaneIDs"), /paneIDForResearchConversation/);
