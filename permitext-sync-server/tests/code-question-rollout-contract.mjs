...
assert.match(workspaceHTML, /styles\.css\?v=20260809-conversation-answer-surface-v2/);
assert.match(serviceWorker, /permitext-pro-shell-v557/);
assert.match(serviceWorker, /styles\.css\?v=20260809-conversation-answer-surface-v2/);
...
assert.match(workspaceScript, /const dockedComposer = conversation\.querySelector\("\.research-composer"\)[\s\S]*?panel\.classList\.add\("has-research-composer"\)[\s\S]*?panel\.append\(dockedComposer\)/);
