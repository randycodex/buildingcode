...
assert.match(workspaceHTML, /styles\.css\?v=20260809-evidence-action-cleanup-v1/);
assert.match(serviceWorker, /permitext-pro-shell-v519/);
assert.match(serviceWorker, /styles\.css\?v=20260809-evidence-action-cleanup-v1/);
...
assert.match(workspaceStyles, /\.evidence-candidate-controls \{[\s\S]*?position: sticky;[\s\S]*?top: 0;[\s\S]*?z-index: 4;[\s\S]*?background: var\(--surface\);/);
