...
## Approved allowance amendment and live v10 comparison

The owner explicitly approved adding $0.50 to the testing allowance for the next capped comparison. The authorization record is `evals/research-owner-api-round2-authorization-20260909.json`, raising this round's cap to $8.50. Historical results and their $8 caps remain unchanged. The accounting report validates the amendment hash before accepting the higher cap. Offline accounting probes confirmed unchanged prior spending, a $0.666786 pre-run remainder, and rejection of a wrong cap or forged amendment hash; no provider calls were made by those probes.

At `f917e3e57fa5f9348924b4b7b118a19b8dd3e08e`, the fresh decision-scope driver completed both unchanged questions once under a $0.40 package cap. Its no-cost preflight intercepted both requests, with initial reservations of $0.183840 and $0.156730. Live request hashes matched; all original passages, cross-source relationships and source-span catalogues reached verification intact. Both review receipts were structurally valid. The four provider calls settled with no paid search, retry or separate judge. This driver is consumed and must not be replayed.

| Case | HTTP result | Full duration | Draft | Verifier | Usage-derived estimate | Conservative cost |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| DOBNOW-023 | 502, verification rejected | 22.839 s | 5.865 s | 14.308 s | $0.02563355 | $0.047757 |
| DOBNOW-003 | 502, verification rejected | 17.333 s | 7.239 s | 7.431 s | $0.02233570 | $0.040728 |

The representative draft fits two paragraphs and 149 words. Its expected substantive concepts are present, including the conditional Board attestation and applicant submission steps. The verifier no longer demands the DPL-1 detail, but now rejects omission of the matching-email login procedure. That is an overbroad operational-detail demand for the authored authority question. The negative verdict is preserved. The draft also repeats the representative prohibition, volunteers Preview to File instructions and repeats a cited condition in `missingFacts`.

The plumbing draft restores the complete conditional LOC versus initial-CO completion treatment and identifies the unknown initial job type. It nevertheless makes an unconditional initiation-after-I1-submission claim in its main paragraph while acknowledging unresolved earlier creation in `evidenceLimitations`. The verifier correctly rejects that contradiction. A qualification in a separate gap field does not repair an overbroad main claim. Its two paragraphs contain 206 words and retain an unasked menu instruction. Neither case is a complete delivered benchmark pass.

The full manual assessment, drafts, verdicts and request hashes are retained in `evals/results/research-owner-decision-scope-answer-assessment-2026-09-09.json`, alongside the preflight, live result and cost audit. This package cost **$0.04796925 by recorded usage**, or **$0.088485 conservatively**. The amended round totals **$7.921699 conservatively against $8.50**, leaving **$0.578301**. Its cumulative usage-derived estimate is **$4.18561632**. These are test-accounting figures; no provider account balance or invoice was checked. No aggregate speed or cost improvement is established.

The next repair must address question-specific source scope for actor-authority answers without discarding material actor conditions, and ensure timing qualifications constrain the main claim. More prompt text and a valid review structure have not established consistent semantic accuracy. Do not weaken the failed-verification gate to increase delivery.

## No-cost review of additional failed workflows

A fresh fetch of the application guide and release notes reproduced the existing retained source fixture exactly. In a direct-source ranking probe, the rent-regulation question selects the complete owner-attestation page 38. That suggests a narrow known-source route could avoid the paid source-discovery step, but no runtime route was changed and no live quality or speed gain is claimed.

For the ADU-document question, the same three-page ranking selects release-note pages 10, 28 and 30. It omits the adjacent field-options page 29; a cellar variant also omits the conditional filing restriction on page 32. This coverage gap must be addressed before enabling a broad direct ADU-document route. The authored question was not changed: the cellar variant is a separate source-coverage probe. Evidence is retained in `evals/results/research-owner-dob-field-source-coverage-2026-09-09.json`. These checks made two public-document GETs and zero paid provider calls. All 110 numbered cases remain in scope, with Project workflow, UI, phone, push and deployment excluded.

## Local source-selection repair: routing v11 and PDF section groups v2

Narrow filing-representative authority questions now select complete permission and responsibility FAQ pairs from the declared stakeholder sections. Questions that also ask for login help, procedures, readiness or navigation retain ordinary FAQ selection. A shared preamble or non-FAQ section still keeps whole-section handling. The application guide, conditional Board attestation release note and explicit applicant review/signature/submission steps remain in the authority source packet. No failed verification verdict is overridden.

Fresh public-source validation retained three complete FAQ pairs for DOBNOW-023, reducing that FAQ's model-visible text from **3,166 to 1,073 characters**. The answer text alone is 606 characters, a different metric. Every selected full passage is unchanged from the prior live packet. This is a source-component reduction, not measured token savings, answer acceptance or faster Research.

The source catalogue now routes DOB NOW rent-regulation attestations directly to the application guide. Narrow ADU radon/vapor or newly erected single-family certificate/document questions route directly to three declared release-note headings. General ADU-document questions retain generic discovery. Explicit requested URLs, internet opt-out and the enacted-evidence boundary retain priority.

PDF section groups preserve all complete pages matching each declared heading, including repeated-heading continuation pages. The current ADU group contains pages **10, 28, 29 and 32**, preserving the certificate conditions, both form-option pages and the conditional cellar restriction. Selection follows heading text rather than page numbers. Invalid or missing headings and groups exceeding six pages fail validation instead of truncating. This establishes coverage for the declared headings; it is not a general proof that every relevant provision in the document was found. Ordinary PDF keyword ranking retains its existing three-page limit.

A negative HTTP test exposed a missing-group fallback to AI search that discarded the declared section requirement. The direct-source path now returns that unavailable-guidance result without provider dispatch when the required group cannot be retrieved. Other generic discovery behavior remains unchanged. Source-validation telemetry also now reports the actual attempted source count for the four-source authority route.

The fresh validation artifact is `evals/results/research-owner-focused-source-validation-2026-09-09.json`: six public-document GETs, zero paid provider calls. It records all bound sources, claims and per-case source counts. DOBNOW-014 selects the complete guide page 38; DOBNOW-017 selects the four-page release-note group. Both authored questions and the reconciled answer key remain unchanged.

Focused HTML attribution, routing, PDF ranking and mocked HTTP contracts passed, followed by the complete offline `npm run test:research-chat` suite with the API key removed (exit 0; `/tmp/permitext-focused-source-suite-v2-20260909.log`). Tests cover retained actor conditions, mixed procedural questions, explicit source and legal boundaries, renumbered PDF pages, missing/oversized section groups, both model-stage inputs and zero provider dispatch when the ADU group is unavailable. The two new document routes use two mocked provider calls instead of the historical three-call discovery path; this has not been confirmed in live model use. The initial full-suite failure is retained in `/tmp/permitext-focused-source-suite-20260909.log` and was resolved by the missing-group fallback repair.

No paid calls were made for this repair. The amended round remains **$7.921699 conservatively against $8.50**, leaving **$0.578301**; its usage-derived estimate remains **$4.18561632**. No account balance or invoice was checked. Live semantic quality, cost and speed remain unconfirmed for these changes, and DOBNOW-003's main-claim timing contradiction remains open. All 110 numbered cases remain in scope. No Project workflow, UI, phone, push or deployment was exercised for this repair.
