...
2. Both two-case comparisons are complete. Draft-only wording changes did not establish reliable scope handling and were reverted. Next investigate how source conditions and requested claims are represented, using retained failures and negative controls before another paid comparison. Preserve complete sources, the restored review-type permission as an acceptance requirement, verifier materiality review and all other repair groups. Do not solve the fee detour by expanding the reference into an exhaustive filing manual or overriding failed results.
