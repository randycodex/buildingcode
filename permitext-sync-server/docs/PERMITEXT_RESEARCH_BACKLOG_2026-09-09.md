...
Inventory revision 4: September 9, 2026. Latest two-case live comparison runtime: `3688bd6503736ed91dd48ffe06bf9ddca33aae31` (draft-only prompt v12). That experiment failed to improve delivery, so the prior v11 summary source was restored. The full cohort has not been rerun on one current baseline.
...
Source-resolution preparation: prompt v13 adds a structured result for each detected source relationship within the existing drafting call. The model supplies a resolved, conditional, unresolved or not-material finding. Material statements must be carried verbatim into a main paragraph citing all sources of that relationship. The [structural validator](../research-guidance-source-resolutions.mjs) rejects missing/stale records, secondary-only statements and one-sided citations before a verifier call. It does not decide whether a finding or not-material explanation is true; the independent semantic verifier still checks every full source passage and every claim, including earlier contradictions. No extra model call, source trimming, expected-answer text or automatic semantic override is introduced.

Local validation: the new structural contract and HTTP negative controls passed, followed by `npm run test:research-chat` on the final runtime. The HTTP cases confirm one draft call for invalid plans and two calls for valid plans rejected semantically; no retries occur. The [fixed-source comparison](../evals/results/research-source-resolution-request-comparison-2026-09-09.json) covers timing/completion, PAA field editability and conditional actor relationships. It preserves all source text, facts, historical candidate prose, models and output limits; six no-relationship control requests remain identical to v11. The structure adds 2,161–2,402 bytes to these draft requests and 890–1,129 bytes to fixed-draft verifier requests. Reservation estimates increase accordingly; these are not actual usage or speed measurements. Historical drafts in that comparison lack the new plan and are not dispatchable v13 candidates. Full suite log: `/tmp/permitext-source-resolutions-suite-20260909.log`.

The newly prepared live package covers DOBNOW-003, DOBNOW-004 and DOBNOW-023 once each, at most six provider calls and $0.40 total from the existing $0.405778 allowance. Each turn is capped at $0.22 and the package stops if it cannot reserve the next turn. A fresh, committed-source HTTP preflight is required before dispatch; all 110 cases remain in scope and no acceptance count changes from these local checks.
...
2. Both two-case comparisons are complete. Draft-only wording changes did not establish reliable scope handling and were reverted. The source-resolution structure is now locally implemented and tested; next evaluate it through the new capped three-case package after its preflight. Preserve complete sources, the restored review-type permission as an acceptance requirement, verifier materiality review and all other repair groups. Do not solve the fee detour by expanding the reference into an exhaustive filing manual or overriding failed results.
...
The latest draft-focus comparison made **four paid provider calls across two HTTP Research turns**, with no paid search, retries or separate judge. Its conservative charge is **$0.085572** and usage estimate is $0.04623475. Together with the preceding claim-scope comparison, the two packages used $0.172523 conservatively. The unchanged aggregate authorization is $8.50; retained conservative spending is **$8.094222**, leaving **$0.405778**. Recorded usage estimates total $4.27905217. These are ledger figures; the API account balance and invoice were not checked. The older historical campaign remains separate. Inventory generation and manual answer review themselves made zero provider or network calls. Both consumed live drivers must not be replayed.
...
| DOBNOW-003 | Subsequent plumbing filing | No delivered answer in latest attempt | R1, R2, R7 | [result](../evals/results/research-owner-api-round2-live-draft-focus-2026-09-09.json) · [review](../evals/results/research-owner-draft-focus-answer-assessment-2026-09-09.json) |
| DOBNOW-004 | PAA for an approved-scope change | No delivered answer in latest attempt | R1, R5, R7 | [result](../evals/results/research-owner-api-round2-live-draft-focus-2026-09-09.json) · [review](../evals/results/research-owner-draft-focus-answer-assessment-2026-09-09.json) |
...
The [revision 4 inventory](../evals/results/research-owner-backlog-v4-2026-09-09.json) retains all 110 case definitions, latest-attempt bindings, reviewed findings and input hashes. The [report script](../scripts/report-research-owner-backlog-20260909.mjs) checks retained ledger hashes, rejects ambiguous review bindings and refuses live execution. The supplemental and both two-case reviews are pinned by their hashes and contain explicit manual dispositions; untriaged reviews are not automatically accepted. These evaluation-only findings never enter model prompts.
