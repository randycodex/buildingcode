...
export const researchAnswerPresentationVersion = "20260921-guided-next-question-v16";
...
export const researchGuidedNextStepInstruction =
  "Make the next step easy for the user. When a project conclusion is blocked by facts, ask ONE short, plain-language question in followUpQuestions: choose the unresolved fact that most narrows the applicable rule. At most combine two closely related facts, such as existing/new building and proposed work. Use facts already provided; never ask for them again. Explain briefly in answerText why that fact matters, without ending with an exhaustive project-facts or records checklist. Omit optional downstream design and room-exception details unless they affect the requested decision or the user asks for them. Preserve all material conditions and uncertainty, but keep the full necessary fact inventory in missingFacts for expandable details. Do not ask for certificates, approved plans, or uploads before establishing which fact they must confirm. If the user says they do not know or asks what to check first, do not repeat the unanswered question in either prose or followUpQuestions. Explain the distinction in everyday terms and give one concrete way to find that specific fact (for example, inspect the project description on their application or ask the person preparing the work). If no materially different question would help, leave followUpQuestions empty. Never invent the fact. Put any next question only in followUpQuestions, not also in answerText. Missing enacted provisions belong in additionalEvidenceNeeded/evidenceLimitations, never missingFacts. Treat retrieval of available library provisions as Permitext's work, not a user assignment. State honestly if governing text is unavailable; do not promise a completed lookup or background search. Do not make unsupported claims while awaiting evidence. If the question is already answered, leave followUpQuestions empty. If the user expressly requests a complete checklist, provide it; still identify the first useful next step.";
