...
export const guidanceQualificationReviewVersion = "20260909-guidance-qualifications-v1";
...
export const guidanceQualificationReviewInstruction = [
  ...
  "Record finding addressed only when every material qualification from that passage is preserved, with short exact conditionQuotes and answerReferences (paragraph:0, missing_fact:0 or evidence_limitation:0, zero-based). A missing-fact question alone does not preserve a known conditional rule. Generic disclaimers do not address source-specific conditions.",
  "Use missing_or_misstated if any material condition is missing, contradicted or universalized; quote the source condition and explain the omission. Report the corresponding issue and fail verification. Reconcile both sides of a material source conflict, rather than crediting one cited passage alone.",
  "Use not_material with a specific reason only when the passage adds no material condition to the requested action or any volunteered claim; do not require unrelated page topics. Keep each reason brief. This review is internal: it must not add a checklist to the user-facing answer. Return the exact packetSHA256 and all passage indices."
].join(" ");
          ...
          passages: {
            ...
            items: {
              ...
              properties: {
                ...
                conditionQuotes: { type: "array", maxItems: 8, items: { type: "string", maxLength: 400 } },
                answerReferences: { type: "array", maxItems: 12, items: { type: "string" } },
                reason: { type: "string", maxLength: 400 }
              }, required: ["passageIndex", "finding", "conditionQuotes", "answerReferences", "reason"]
            }
          }
  ...
  const complete = input.qualificationReviewPacket?.packetSHA256 === packet.packetSHA256 &&
    ...
    records.every((record) => {
      ...
      if (!Number.isSafeInteger(record?.passageIndex) || !passage || !findings.includes(record.finding) ||
          ...
          !Array.isArray(record.conditionQuotes) || record.conditionQuotes.length > 8 ||
          !Array.isArray(record.answerReferences) || record.answerReferences.length > 12) return false;
      const sourceText = compact(passage.text);
      if (record.conditionQuotes.some((quote) => !compact(quote) || quote.length > 400 ||
          compact(quote).length < Math.min(12, sourceText.length) || !sourceText.includes(compact(quote))) ||
      ...
      return record.conditionQuotes.length > 0 &&
  ...
  return {
    ...
    qualificationReview: {
      ...
      passages: complete ? records.map((record) => ({
        ...packet.passages[record.passageIndex], finding: record.finding,
        conditionQuotes: [...record.conditionQuotes], answerReferences: [...record.answerReferences], reason: compact(record.reason)
      })) : []
    }
  };
}
