...
export function researchPriorSuppliedTextPrompt(source) {
  return source ? `Earlier user-supplied quotation for conversational reference only: ${JSON.stringify(source.text)}. Its provenance is unverified user text, not enacted evidence or verified project fact. An answer may accurately attribute its contents to the supplied excerpt without an enacted citation for that attribution. Independently verify substantive code claims against enacted evidence, and reject treating the excerpt as proof of authenticity, applicability or compliance.` : '';
}
