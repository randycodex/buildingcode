...
- npm's security bulk endpoint timed out, including a bounded standalone retry. Do **not** report a clean full npm audit. The exact advisory, patched release, lock inventory, module behavior, build and browser evidence independently verify this correction. Dependabot closure still requires publication to the default branch and a refreshed GitHub result.
