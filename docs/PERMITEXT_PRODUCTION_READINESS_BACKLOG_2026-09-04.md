...
| P2-4 | Reader chrome inconsistently identifies edition. | Native/web edition labels and citation identity repairs are implemented. Production and physical saved-citation paths show correct 2014/2022 labels. A later web code-switch race is repaired with immediate text clearing and navigation-generation guards; actual-function delay/race/retry tests and rendered local failure/recovery pass. See the Reader repair record below. | Publish and verify the navigation repair; complete broader source, scroll-position and assistive-technology coverage. |
| P2-5 | Tablet toolbars collide and phone-web scope is unclear. | Root checked 320/375/390/430/768/1024/1280/1440 widths with no observed overlap, and 44-pixel tablet targets. The owner confirmed build 60 cannot pan Fuel Gas Table 504.2(1). The single-viewport repair passes Simulator gestures with and without search highlighting, plus historical figure/table compatibility checks. | Verify the replacement TestFlight build on the physical phone; browser and Simulator checks do not replace physical touch or VoiceOver acceptance. |
| P2-6 | Exact-match search behavior is insufficiently explained. | Exact-match disclosure and term-search recovery are implemented and tested. Production `bc4230fe68f9` Reader no-match/clear/retry and global no-match/shorter-term/result-opening paths passed in the actual browser, with the code group retained. | Carry these checks forward to the final candidate and complete accessibility acceptance. |
...
The [September 5 Reader repair record](./PERMITEXT_READER_ACCEPTANCE_REPAIRS_2026-09-05.md)
retains the new findings, exact local evidence, and separate publication/physical
acceptance boundaries. These checks continue the original 17 findings.
