...
- Fresh-account Microsoft sign-in: passed September 7 on current Production web after repairing Clerk's invalid Microsoft client secret. Clerk's before search had no user; afterward the authorized personal Microsoft identity had a new verified account and Permitext showed Free/Synced with an empty workspace. Physical iOS and final shared-release binding remain open. See the [Microsoft repair and receipt](./PERMITEXT_MICROSOFT_SIGN_IN_2026-09-07.md).
...
- Existing-account Microsoft sign-in retained the correct Permitext account/data: passed September 7 on current Production web with one synthetic saved collection. Explicit sign-out cleared it; returning Microsoft sign-in restored the same account and byte-equivalent server mutation, with all 24 content/usage groups unchanged. The final replacement credential also passed real returning sign-in. This does not establish physical iOS, new-provider account linking, publisher verification or final shared-release binding. See the [Microsoft receipt](./PERMITEXT_MICROSOFT_SIGN_IN_2026-09-07.md).
...
- Web purchase consent displays and records those exact versions: bounded pass on Production `5f1afb414fdd`, September 7. The unchecked gate, canonical document links, one server-dated acceptance and same-policy retry passed with independent account exports; hosted Checkout was opened and left unpaid. A later real Google sign-in on `4048aa28e65b` removed the stored acceptance. The PostgreSQL non-Apple sign-in overwrite is identified; repair and live durability retest remain open. The Stripe description and retained acknowledgment passes remain valid. [Recording evidence](./PERMITEXT_WEB_POLICY_CONSENT_2026-09-07.md), [sign-in durability failure](./PERMITEXT_SIGN_IN_POLICY_PERSISTENCE_2026-09-07.md).
