...
## Locked post-v8 confirmation preparation

A new authorization package was prepared after the v8 repair without resetting or editing the consumed remediation-successor-3 authorization. Authorization ID `151aa121-7962-48d1-80b3-56728e62fc75` remains `locked`. It is bound to this unchanged 30-case cohort SHA, repaired source commit `747887054e1bba16578a44477720f813a55fc357`, safety version `20260830-zoning-material-completeness-v8`, safety SHA `62bb5459c2ea22f981b4b2b0367d25b7086c7d86bf0d0cb92d582ae1d817dc94`, and the retained prior authorization/result hashes. Its authorized-scope case count and repetitions, cap, owner phrases, authorized package commit, execution commit, attempt ID, and run ID are all unset.

All six historical successor live entry points are now retired and fail closed before dispatch even if an old consumed record is edited; only this distinct v8-confirmation path can become active after a future exact owner decision. Its dedicated runner and evaluator are separately namespaced from the historical runner. A future activation must bind the exact commit containing this locked package, and only the new authorization JSON may differ afterward. Before dispatch, the runner requires all 30 ordered cases, one repetition, a new cap no higher than `$5`, committed and clean server inputs, the repaired commit as an ancestor, matching global and package-specific locks, disabled provider web support, and a durable one-time `running` attempt. Any retained result must match the exact cohort order, run ID, clean execution commit, cap, ordered unique operation prefix, zero pending paid requests, and no operation-level web request or search. The guard and complete 30-case no-cost conversation preflight pass with zero provider calls. This preparation is not owner authorization and does not enable the 24,000-character candidate, public Research, deployment, pricing, or allowance changes.
