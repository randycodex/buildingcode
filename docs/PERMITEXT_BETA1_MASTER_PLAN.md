...
The product foundation is substantially built, but Beta 1 is **not ready for public paid release**. The post-v23 immutable v6 Research cohort completed all 20 production turns without a provider or charging failure, and all 20 answers passed every fatal evaluation gate. Subscriber-level aggregation places a fully used 100-turn month at $5.74 p50 and $6.06 p90 in model cost. With the confirmed 15% App Store commission, ten support minutes at $30/hour, and explicit tax, refund, and infrastructure planning reserves, the 25-subscriber p90 full-service cost is $15.84 on web and $17.86 on iOS. Allocating the full $45 infrastructure budget across only ten fully utilized subscribers lowers contribution to $1.46 on web and -$0.56 on iOS; the accepted $2 floor first passes at 12 web or 24 iOS subscribers. The owner retained 100 turns for Beta with a later $4–$6 target. Local signed-route Stripe and Apple lifecycle simulations now pass, and a provider-backed Stripe sandbox lifecycle also passes; those exercises found and retained regressions for Stripe's current refund shape plus Apple's failed-renewal, delayed-notification, and stale post-refund verification behavior. Apple staging has a dedicated Vercel project, separate free Neon database and private Blob store, all 17 readiness checks passing, a public healthy staging endpoint, and a dedicated App Store Connect Sandbox notification URL while the Production URL remains unset. Physical-iPhone build 43 completed a no-charge Apple Sandbox purchase but exposed a missing account-owner binding; build 44 repaired that boundary, completed processing for Internal Testers, and restored the existing purchase with HTTP 200 backend verification. Purchase, local Pro activation, the 100-turn allowance, account ownership, Restore, later notification delivery, post-cancellation notification delivery, cancellation-period access retention, and canceled-period expiration now pass. The earlier classification of that later Sandbox delivery as a failed-delivery retry is withdrawn: Apple documents that automatic retries are Production-only, while Sandbox attempts a notification once. Build 45's refund button did not present Apple's sheet; build 46 repaired presentation and completed Apple's no-charge TestFlight refund form through `Your request has been submitted`. The device nevertheless remained `Pro (Test)` after relaunch. Permitext now prevents an older transaction JWS from overriding a newer refund notification and clears cached iOS Pro when the server denies the entitlement; the complete billing suite and all 112 iOS entitlement/sync tests pass. The repair is live only on isolated staging; build 47 uploaded, became available in TestFlight, was installed, and displayed Free without another purchase or Restore. Refund revocation therefore passes on the physical iPhone. A separate five-minute Sandbox tester then completed a new no-charge purchase and renewal. That exercise exposed and repaired a PostgreSQL signed-date overflow; after the isolated repair, Apple transaction verification and a server notification returned HTTP 200 and the relaunched iPhone remained `Pro (Test)`. Renewal therefore passes. Disabling purchases and renewals then moved the device to Free, and re-enabling them restored Pro access, completing billing failure/recovery. The recovery exposed a transient-verification display defect that could degrade `Pro (Test)` to plain `Pro`; commit `04aec83126fe21ac9086527fb3911acd9f2aad95` repairs that narrow continuity case while retaining fail-closed ownership and expiration behavior, and all 113 entitlement/sync tests pass. Staging-targeted build 48 uploaded, completed processing, was installed, and a fresh no-charge hourly-tester purchase produced three HTTP 200 backend verifications plus the owner's explicit physical-device confirmation of `Pro (Test)`. Its initial server notification received HTTP 503 before ownership was bound. Apple's Sandbox Notification History returned the exact failed `SUBSCRIBED / INITIAL_BUY` payload; replaying it after ownership binding returned HTTP 200 with `changed: true`, so Apple-created delayed recovery passes. The identical duplicate returned `changed: false` and created no second notification-state row, but the strict database check found that it still rewrote the entitlement timestamp. A local atomic-CTE repair and regression contract pass with zero paid provider calls, but the repair is not deployed and provider-backed strict duplicate idempotency remains open. Web and iOS require explicit acceptance of the exact displayed policy versions before starting a purchase, the server rejects checkout without current acceptance, and web Checkout returns through a printable subscription acknowledgment; these controls remain dormant until owner-approved versions are configured. The owner elected to operate as a sole proprietor and not retain an attorney; a documented official-source self-review replaces the counsel gate without claiming legal approval. Controlled Production billing and the remaining Apple-created Sandbox/TestFlight lifecycle evidence remain open. New York sales-tax registration/configuration, actual refund/infrastructure incidence, provider, operations, Production, and final TestFlight gates remain open.

Current development branch: `codex/research-commercialization`

## Status dashboard

Checklist snapshot: **63 completed, 25 open**. Eleven of the open items are the final merge, deployment, TestFlight, and release sequence, which has not been authorized on this branch.
...
| Billing and paid continuation | Partially complete | Server billing contracts, local Stripe/Apple exercises, the provider-backed Stripe sandbox lifecycle, isolated Apple staging, and physical-iPhone purchase/account-restore/later-notification/cancellation-retention/expiration/refund/renewal/billing-recovery evidence pass. Build 46 completed Apple's refund form and submission; build 47 proved the refund repair and five-minute renewal/billing-recovery sequence; build 48 proved the transient Sandbox identity-label repair and Apple-created delayed recovery. The duplicate replay exposed a PostgreSQL no-op entitlement rewrite; its local repair passes but needs a separately authorized isolated-staging deployment and exact-payload replay. Controlled Production billing and final serving-release verification also remain. Additional-turn sales stay disabled. |
...
- [ ] Complete Apple-created Sandbox and TestFlight subscription lifecycle evidence against the compatible staging backend. Purchase, no-charge confirmation, local Pro/100-turn activation, account ownership, Restore, later notification delivery, post-cancellation notification delivery, cancellation-period access retention, canceled-period expiration, refund submission, refund revocation, renewal, billing failure/recovery, build 48 Sandbox identity continuity, and Apple-created delayed recovery pass. The exact duplicate replay exposed an entitlement-row no-op write; deploy the passing local atomic-CTE repair only with separate authorization, then repeat the retained signed payload and prove strict database idempotency.
