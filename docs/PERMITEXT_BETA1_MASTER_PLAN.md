...
The product foundation is substantially built, but Beta 1 is **not ready for public paid release**. The post-v23 immutable v6 Research cohort completed all 20 production turns without a provider or charging failure, and all 20 answers passed every fatal evaluation gate. Subscriber-level aggregation places a fully used 100-turn month at $5.74 p50 and $6.06 p90 in model cost. With the confirmed 15% App Store commission, ten support minutes at $30/hour, and explicit tax, refund, and infrastructure planning reserves, the 25-subscriber p90 full-service cost is $15.84 on web and $17.86 on iOS. Allocating the full $45 infrastructure budget across only ten fully utilized subscribers lowers contribution to $1.46 on web and -$0.56 on iOS; the accepted $2 floor first passes at 12 web or 24 iOS subscribers. The owner retained 100 turns for Beta with a later $4–$6 target. Local signed-route Stripe and Apple lifecycle simulations now pass, and a provider-backed Stripe sandbox lifecycle also passes; those exercises found and retained regressions for Stripe's current refund shape plus Apple's failed-renewal and delayed-notification behavior. Apple staging now has a dedicated Vercel project, separate free Neon database and private Blob store, all 17 readiness checks passing, a public healthy staging endpoint, and a dedicated App Store Connect Sandbox notification URL while the Production URL remains unset. Staging-targeted build 42 completed processing, is `Ready to Submit`, and is associated with Internal Testers; no Apple Sandbox transaction has yet been created. Web and iOS now require explicit acceptance of the exact displayed policy versions before starting a purchase, the server rejects checkout without current acceptance, and web Checkout returns through a printable subscription acknowledgment; these controls remain dormant until owner-approved versions are configured. The owner elected to operate as a sole proprietor and not retain an attorney; a documented official-source self-review replaces the counsel gate without claiming legal approval. Controlled Production billing and Apple-created Sandbox/TestFlight lifecycle evidence remain open. New York sales-tax registration/configuration, actual refund/infrastructure incidence, provider, operations, Production, and final TestFlight gates remain open.
...
| Billing and paid continuation | Partially complete | Server billing contracts, local Stripe/Apple exercises, the provider-backed Stripe sandbox lifecycle, isolated Apple Sandbox staging, the dedicated Sandbox notification URL, and processed staging-targeted build 42 pass. The physical-iPhone Apple lifecycle, controlled Production billing, and final serving-release verification remain. Additional-turn sales stay disabled. |
...
- [x] Create an isolated Apple Sandbox Vercel project with separate free Neon and private Blob resources, pass all staging-readiness checks, configure only the Sandbox notification URL, and upload staging-targeted build 42.
