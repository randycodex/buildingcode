...
Checklist snapshot: **96 completed, 14 open**. Eleven of the open items are the final merge, deployment, TestFlight, and release sequence, which has not been authorized on this branch.
...
| Zoning Research Beta 1 expansion | Successor semantic, reliability, and cost gates failed | The exact owner-approved 30-case successor passed 30/30 no-cost checks when frozen, then its one authorized $5-capped semantic run spent $3.333192, completed 27/30 production turns, failed three without a user charge, and passed 18/27 graded answers. Stronger bare-section parsing plus the later applicability re-audit expose three frozen answer-key/evidence blockers, so current preflight now fails closed at 27/30. The corrected no-cost ready-case advisory reduced average stored evidence 28.8% while preserving every deterministic required section, but the retained 24,000-character candidate remains disabled. Failed-work-amortized cost is $10.01 per 100 all-Zoning turns; mixed-month p90 is $10.41 at 100% Zoning, with negative $0.19 web and negative $2.21 iOS contribution under current reserves. The authorization is consumed. Cases 1, 3, and 11 remain held. A dedicated fail-closed public-release gate still requires a clean semantic result, accepted cost, and exact-release web/TestFlight physical-iPhone acceptance. Public Zoning Research stays disabled. |
...
- [ ] Resolve the successor's three execution failures and nine graded fatal-gate failures, three answer-key/evidence blockers, reduce cost, and confirm the repaired cohort in a newly authorized clean semantic run before enablement. The [no-cost failure triage](./PERMITEXT_ZONING_SUCCESSOR_FAILURE_TRIAGE_2026-08-30.md) now separates three evaluator defects, five product-completeness defects covered by new safeguards, three answer-key/evidence blockers requiring owner dispositions, and three execution paths. The [official-source applicability re-audit](./PERMITEXT_ZONING_CASE23_APPLICABILITY_AUDIT_2026-08-30.md) confirms selected ZR 23-343 as the residential Case 23 branch while recording community-facility status as a missing material fact; current successor preflight correctly blocks at 27/30 before another paid run.
...
- the exact owner-approved Zoning successor's three execution failures, nine graded quality failures, and above-target cost are repaired and a newly authorized clean semantic run plus exact-release web/TestFlight physical-iPhone acceptance passes;
