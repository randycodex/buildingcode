...
A distinct post-v8 confirmation package is now prepared and locked under authorization ID `151aa121-7962-48d1-80b3-56728e62fc75`. It reuses only the immutable 30-case remediation-successor-3 cohort SHA, pins repaired source commit `747887054e1bba16578a44477720f813a55fc357` and the reviewed v8 safety bytes, and proves the three historical authorizations and retained remediation-3 result remain unchanged. The fresh record cannot run while locked: authorized-scope case count and repetitions, cap, owner phrases, package and execution commits, attempt ID, and run ID remain unset. If separately authorized later, its exact committed package must permit all 30 ordered cases, one repetition, a new cap no higher than `$5`, disabled web support, first-execution-error stop, matching global/cohort locks, and an exact clean execution commit. Preparation and its 30/30 no-cost path made zero provider calls and did not authorize execution, public access, deployment, pricing, or allowance changes.

Current development branch: `codex/zoning-research-beta1`

## Status dashboard

Checklist snapshot: **105 completed, 14 open**. Eleven of the open items are the final merge, deployment, TestFlight, and release sequence, which has not been authorized on this branch.
...
| Zoning Research Beta 1 expansion | Post-v8 confirmation package locked; semantic, reliability, and cost gates open | The exact remediation-3 run spent `$0.287113`, completed 2/30 operations with both passing at 4.00/4, then failed closed at the Appendix J case and left 27 unattempted. Its authorization is consumed, and no user was charged for the failed operation. [No-cost classification and v8 repair](./PERMITEXT_ZONING_REMEDIATION_3_SEMANTIC_RESULT_2026-08-30.md) now accept the compliant source-level Appendix J rule plus separate map/location boundary while keeping the no-boundary and specific-site counterexamples failed. A distinct fresh package now locks that repaired path to authorization ID `151aa121-7962-48d1-80b3-56728e62fc75`, the same exact 30-case cohort, and repaired commit `747887054e1bba16578a44477720f813a55fc357`; it is not authorized and made zero provider calls. The two changed successor cases were not reached. The `$13.04` two-turn projection is not sample-ready, so remediation 2's `$20.18` per 100 all-Zoning risk projection and `$20.72` 100%-Zoning mixed-month p90 remain controlling. Cases 1, 3, and 11 remain held. A dedicated fail-closed public-release gate still requires a clean newly authorized semantic result, accepted cost and reliability, and exact-release web/TestFlight physical-iPhone acceptance. Public Zoning Research stays disabled. |
...
- [ ] Use the repaired safety path to resolve the remaining unconfirmed remediation-successor-2 paths/outcomes, reduce and accept cost, and confirm all 30 remediation-successor-3 cases in a newly authorized clean semantic run before enablement. The [remediation-3 result](./PERMITEXT_ZONING_REMEDIATION_3_SEMANTIC_RESULT_2026-08-30.md) is not sample-ready; the [remediation-2 result](./PERMITEXT_ZONING_REMEDIATION_2_SEMANTIC_RESULT_2026-08-30.md) keeps the `$20.18` failed-work-amortized 100-turn projection controlling. Any later paid confirmation requires independent review, a new explicit exact-SHA one-run authorization, and a new cumulative cap.
...
- the five execution-failure paths, two graded quality-failure paths, and above-target cost retained from remediation successor 2 are repaired or cleanly resolved in a later exact frozen successor, and a newly authorized clean semantic run plus exact-release web/TestFlight physical-iPhone acceptance passes;
