...
| Zoning Research Beta 1 expansion | Remediation-2 partial result retained; semantic, reliability, and cost gates open | The exact remediation-2 run spent $3.357895, completed 15/30 production operations, failed 15 without a user charge, and passed 13/15 graded answers. The authorization is consumed. Its failed-work-amortized cost is $20.18 per 100 all-Zoning turns; mixed-month p90 is $20.72 at 100% Zoning, with negative $10.50 web and negative $12.52 iOS contribution under current reserves. [Post-run no-cost analysis](./PERMITEXT_ZONING_REMEDIATION_2_SEMANTIC_RESULT_2026-08-30.md) produced Zoning safety v7 and identified two frozen case/evidence defects without altering or rescoring the immutable result. Five execution paths and two graded failures remain unconfirmed. Cases 1, 3, and 11 remain held. A dedicated fail-closed public-release gate still requires a clean newly authorized semantic result, accepted cost and reliability, and exact-release web/TestFlight physical-iPhone acceptance. No further paid run is authorized, and public Zoning Research stays disabled. |
...
- [ ] Resolve the remaining five execution paths and two graded failures from the remediation-successor-2 result, including owner disposition of the two identified frozen case/evidence defects, reduce and accept cost, and confirm a later exact frozen successor in a newly authorized clean semantic run before enablement. The [immutable result and no-cost follow-up](./PERMITEXT_ZONING_REMEDIATION_2_SEMANTIC_RESULT_2026-08-30.md) keep the 50% completion rate and `$20.18` failed-work-amortized 100-turn projection controlling. Any later paid confirmation requires a new frozen successor, independent review, explicit one-run authorization, and cumulative cap.
