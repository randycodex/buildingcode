...
The distinct post-v8 package was locked at exact commit `7cc2af325dbb3c5c98e4e15e2c15196a4794cb76`, then separately authorized for all 30 ordered cases, one repetition, and a `$5` cumulative cap. Execution commit `9d4af1b31762568caa5accf63b52e275f0e39bde` differed from the package only in the fresh authorization record. Run `1521497c-8df4-4ed9-98ce-79ef2805d1a6` preserved the exact cohort/order, disabled web support, matching locks, clean execution commit, and settled ledger, but stopped at the same third ordered case. The failed-answer text was intentionally not retained, so the exact cause is indeterminate between generated wording and v8 recognition; cohort and evidence drift are ruled out. Subsequent no-cost Zoning safety v9 adds privacy-bounded clause diagnostics, a structural Appendix J source-boundary repair, safe-wording regressions, and matching no-boundary/parcel-specific counterexamples. Independent adversarial review then closed mixed-predicate, connector, modal, continuative, placement, parenthetical, named-subject, determiner, and modifier bypasses before returning a clean material-finding gate. Repair commit `1fae244d775192f55f0fd6ee17d90cb82648ba01` was first retained in locked package `c838b4d2c21554489eaad50b0e354cc6b6bc57f2`. The owner authorized that exact package, all 30 cases, one repetition, and a `$5` cap in commit `94f8ec013f3f9021fa59d691b81a80468a2f9f74`, but the runner stopped before provider dispatch because Node's default output buffer could not hold the pinned 1,216,521-byte application blob. Spend was `$0`, no run ID or result was created, and the unconsumed authorization is retained only in Git history. Replacement locked package `571367800030d49a103a999090eaa615baa361ec` raises only that local verification buffer to 4 MiB, uses fresh authorization ID `9aaade99-759b-41d6-ad73-3ef9b4a168f9`, and remains unset and inactive. Independent review reproduced the failure and confirmed the replacement with the full 30/30 zero-paid-request guard. This does not retroactively prove or rescore a paid result. No new paid run is authorized. [The retained v8 result](./PERMITEXT_ZONING_REMEDIATION_3_V8_CONFIRMATION_RESULT_2026-08-30.md) did not authorize public access, deployment, pricing, allowance changes, or the 24,000-character candidate.

Current development branch: `codex/zoning-research-beta1`

## Status dashboard

Checklist snapshot: **108 completed, 14 open**, plus a separately tracked 11-step final merge, deployment, TestFlight, and release sequence. Those 11 substeps are not included in the 14 main open-checkbox count and have not been authorized on this branch.
...
| Zoning Research Beta 1 expansion | V9 independently reviewed; replacement package locked; semantic, reliability, and cost gates open | The first exact remediation-3 run spent `$0.287113`, completed 2/30 operations with both passing at 4.00/4, then failed closed at the Appendix J case and left 27 unattempted. [No-cost classification and v8 repair](./PERMITEXT_ZONING_REMEDIATION_3_SEMANTIC_RESULT_2026-08-30.md) accept the reviewed source-level rule plus separate boundary regressions while keeping no-boundary and specific-site counterexamples failed. The distinct [post-v8 confirmation](./PERMITEXT_ZONING_REMEDIATION_3_V8_CONFIRMATION_RESULT_2026-08-30.md) then spent `$0.297314`, again passed the first two cases at 4.00/4, and again failed closed at case three with 27 unattempted. Its authorization is consumed, the failed operation consumed no user turn, and the text-level cause is unrecoverable. The subsequent no-cost v9 repair records only bounded hashes/flags rather than answer text and passed independent adversarial review. Its first authorized package stopped before provider dispatch on a local buffer limit and spent `$0`; replacement locked package `571367800030d49a103a999090eaa615baa361ec` fixes that bounded handoff and passes 30/30 mock preflight with zero paid requests and no active authorization. The two changed successor cases remain unconfirmed. Neither the historical `$13.04` nor new `$13.40` two-turn projection is sample-ready, so remediation 2's `$20.18` per 100 all-Zoning risk projection and `$20.72` 100%-Zoning mixed-month p90 remain controlling. Cases 1, 3, and 11 remain held. A dedicated fail-closed public-release gate still requires a clean newly authorized semantic result, accepted cost and reliability, and exact-release web/TestFlight physical-iPhone acceptance. Public Zoning Research stays disabled. |
...
- [x] Independently review and retain the bounded v9 repair, close every material adversarial finding, retain the first authorized package's `$0` pre-dispatch buffer stop, and prepare replacement locked package `571367800030d49a103a999090eaa615baa361ec` with immutable v8 lineage, a fresh unused authorization, retired historical dispatch paths, a zero-provider-call guard, and a complete 30/30 mock preflight. The package is not authorization to run.
- [ ] Resolve the remaining unconfirmed remediation-successor-2 paths/outcomes, reduce and accept cost, and confirm all 30 remediation-successor-3 cases in a newly authorized clean semantic run before enablement. Both the [first remediation-3 result](./PERMITEXT_ZONING_REMEDIATION_3_SEMANTIC_RESULT_2026-08-30.md) and [post-v8 confirmation](./PERMITEXT_ZONING_REMEDIATION_3_V8_CONFIRMATION_RESULT_2026-08-30.md) are not sample-ready; the [remediation-2 result](./PERMITEXT_ZONING_REMEDIATION_2_SEMANTIC_RESULT_2026-08-30.md) keeps the `$20.18` failed-work-amortized 100-turn projection controlling. No new paid run is authorized; any later paid confirmation requires explicit authorization naming exact package `571367800030d49a103a999090eaa615baa361ec`, all 30 ordered cases, one repetition, and a new cumulative cap no higher than `$5`.
