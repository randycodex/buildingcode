...
| Zoning Research Beta 1 expansion | Remediation-3 no-cost successor frozen; semantic, reliability, and cost gates open | The exact remediation-2 run spent $3.357895, completed 15/30 production operations, failed 15 without a user charge, and passed 13/15 graded answers. Its authorization is consumed, and its failed-work-amortized cost is $20.18 per 100 all-Zoning turns; mixed-month p90 is $20.72 at 100% Zoning, with negative $10.50 web and negative $12.52 iOS contribution under current reserves. [Post-run no-cost analysis](./PERMITEXT_ZONING_REMEDIATION_2_SEMANTIC_RESULT_2026-08-30.md) produced Zoning safety v7. The owner-approved correction of its two frozen case/evidence defects is now separately frozen in [remediation successor 3](./PERMITEXT_ZONING_SUCCESSOR_REMEDIATION_3_2026-08-30.md): exactly two cases changed and 30/30 no-cost canonical and disabled-24,000-character checks pass. Its exact-SHA paid runner is prepared but remains locked and fail-closed before dispatch. Five execution paths and two graded outcomes remain unconfirmed. Cases 1, 3, and 11 remain held. A dedicated fail-closed public-release gate still requires a clean newly authorized semantic result, accepted cost and reliability, and exact-release web/TestFlight physical-iPhone acceptance. No further paid run is authorized at this checkpoint, and public Zoning Research stays disabled. |
...
- [x] Prepare remediation successor 3's separate exact-SHA paid-run authorization and consuming runner; leave it locked, require all 30 ordered cases/one repetition/a cap no higher than `$5`, disable unbudgeted web-search fees, stop on the first execution error, require committed inputs and global/cohort locks, atomically enter a non-reusable `running` state before dispatch, bind one result to the preassigned run ID, and prove direct live execution cannot bypass consumption.
- [ ] Resolve the remaining five execution paths and two graded outcomes from the remediation-successor-2 result, reduce and accept cost, and confirm remediation successor 3 in a newly authorized clean semantic run before enablement. The [immutable result and no-cost follow-up](./PERMITEXT_ZONING_REMEDIATION_2_SEMANTIC_RESULT_2026-08-30.md) keep the 50% completion rate and `$20.18` failed-work-amortized 100-turn projection controlling. Any later paid confirmation requires independent review, an explicit exact-SHA one-run authorization, and a cumulative cap.
