...
| Zoning Research Beta 1 expansion | Expanded no-cost cohort frozen; paid quality confirmation pending | The complete corpus and no-cost safety contracts pass. The first $5-capped frozen diagnostic spent $1.857548, completed 20/21 production turns, and passed 11/20 graded answers. Passage/definition assembly, Appendix J structured-text handling, and official amendment metadata are remediated. The original cohort still passes 21/21 no-cost conversation creation; nine owner-approved Batch 1 cases were appended to a separate immutable successor that passes 30/30. Cases 1, 3, and 11 remain held. A newly authorized clean semantic run and web/iOS acceptance remain. Public Zoning Research stays disabled. |
...
- [x] Add the measured preliminary Zoning cost distribution to the 100-turn subscriber sensitivity model without changing price or allowance: provider p90 is $6.06 at 0% Zoning, $6.43 at 25%, $6.80 at 50%, and $7.54 at 100%.
...
- [ ] Confirm the nine prior scored failures plus one fail-closed operation in a clean semantic run under a new explicit owner authorization and cumulative spend cap before enablement.
