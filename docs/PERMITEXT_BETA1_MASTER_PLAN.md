...
Checklist snapshot: **89 completed, 26 open**. Eleven of the open items are the final merge, deployment, TestFlight, and release sequence, which has not been authorized on this branch.
...
| Zoning Research Beta 1 expansion | Expanded paid diagnostic retained; rubric and cost remediation open | The complete corpus and no-cost safety contracts pass. The latest $5-capped frozen diagnostic spent $3.247980, completed 28/30 production turns without charging the two failures, and passed 12/28 graded answers. Four no-cost pipeline/evaluator defects are repaired. Two Batch 1 answer keys conflict with selected enacted evidence and require owner re-review; Cases 1, 3, and 11 remain held. The expanded cost projection is $8.87 per 100 all-Zoning turns and 100% Zoning p90 is $9.33. A three-budget no-cost prototype preserved all 87 exact selections and eight structured sources; the retained 24,000-character supplemental candidate averaged 34,821 assembled characters but remains disabled. A newly authorized clean semantic run and web/iOS acceptance remain. Public Zoning Research stays disabled. |
