...
Checklist snapshot: **104 completed, 14 open**. Eleven of the open items are the final merge, deployment, TestFlight, and release sequence, which has not been authorized on this branch.
...
| Zoning Research Beta 1 expansion | Remediation-3 partial diagnostic retained; no-cost false-positive repair complete; semantic, reliability, and cost gates open | The exact remediation-3 run spent `$0.287113`, completed 2/30 operations with both passing at 4.00/4, then failed closed at the Appendix J case and left 27 unattempted. Its authorization is consumed, and no user was charged for the failed operation. [No-cost classification and v8 repair](./PERMITEXT_ZONING_REMEDIATION_3_SEMANTIC_RESULT_2026-08-30.md) now accept the compliant source-level Appendix J rule plus separate map/location boundary while keeping the no-boundary and specific-site counterexamples failed. The two changed successor cases were not reached. Its `$13.04` two-turn projection is not sample-ready, so remediation 2's `$20.18` per 100 all-Zoning risk projection and `$20.72` 100%-Zoning mixed-month p90 remain controlling. Cases 1, 3, and 11 remain held. A dedicated fail-closed public-release gate still requires a clean newly authorized semantic result, accepted cost and reliability, and exact-release web/TestFlight physical-iPhone acceptance. No further paid run is authorized, and public Zoning Research stays disabled. |
