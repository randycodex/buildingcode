...
- [x] Record the separate [Apple tax-handling boundary](./BETA1_APPLE_TAX_HANDLING_RECORD.md): App Store customer price includes taxes Apple collects and remits, Apple proceeds are net of applicable taxes and commission, and Stripe automatic tax must remain web-only. Verify read-only that the parent app displays `App Store software` and the subscription displays `Match to parent app`; leave owner classification confirmation and the first real financial report open.
