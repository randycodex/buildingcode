...
Checklist snapshot: **95 completed, 25 open**. Eleven of the open items are the final merge, deployment, TestFlight, and release sequence, which has not been authorized on this branch.
...
| Zoning Research Beta 1 expansion | Successor semantic, reliability, and cost gates failed | The exact owner-approved 30-case successor passed 30/30 no-cost checks, then its one authorized $5-capped semantic run spent $3.333192, completed 27/30 production turns, failed three without a user charge, and passed 18/27 graded answers. Failed-work-amortized cost is $10.01 per 100 all-Zoning turns; mixed-month p90 is $10.41 at 100% Zoning, with negative $0.19 web and negative $2.21 iOS contribution under current reserves. The authorization is consumed. Cases 1, 3, and 11 remain held. The retained 24,000-character candidate remains disabled. A dedicated fail-closed public-release gate now requires a clean semantic result, accepted cost, and exact-release web/TestFlight physical-iPhone acceptance. Public Zoning Research stays disabled. |
...
- [ ] Resolve the successor's three execution failures and nine graded fatal-gate failures, reduce cost, and confirm the repaired cohort in a newly authorized clean semantic run before enablement. The [no-cost failure triage](./PERMITEXT_ZONING_SUCCESSOR_FAILURE_TRIAGE_2026-08-30.md) now separates three evaluator defects, five product-completeness defects covered by new safeguards, one source/key mismatch requiring owner disposition, and three execution paths; current successor preflight correctly blocks at 29/30 rather than allowing another paid run against the unselected ZR 24-382 key.
