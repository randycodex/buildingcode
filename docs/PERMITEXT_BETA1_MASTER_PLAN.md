...
| Billing and paid continuation | Partially complete | Server billing contracts, local Stripe/Apple exercises, the provider-backed Stripe sandbox lifecycle, isolated Apple staging, and physical-iPhone purchase/account-restore/retry/cancellation-retention/expiration/refund/renewal evidence pass. Build 46 completed Apple's refund form and submission, then exposed stale post-refund verification and cached-iOS-entitlement defects; build 47 proved the repairs at the device layer. A separate five-minute renewal exercise exposed a PostgreSQL signed-date overflow, which is repaired and verified only on isolated staging. Billing failure/recovery, duplicate delivery, delayed delivery, controlled Production billing, and final serving-release verification remain. Additional-turn sales stay disabled. |
...
- [x] Create an isolated Apple Sandbox Vercel project with separate free Neon and private Blob resources, pass all staging-readiness checks, configure only the Sandbox notification URL, and upload staging-targeted builds through build 47.
- [ ] Complete Apple-created Sandbox and TestFlight subscription lifecycle evidence against the compatible staging backend. Purchase, no-charge confirmation, local Pro/100-turn activation, account ownership, Restore, failed-delivery retry, post-cancellation notification delivery, cancellation-period access retention, canceled-period expiration, refund submission, refund revocation, and renewal pass; billing failure/recovery, duplicate delivery, and delayed delivery remain open.
