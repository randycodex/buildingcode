...
| Zoning Research Beta 1 expansion | Remediation-3 partial diagnostic retained; semantic, reliability, and cost gates open | The exact remediation-3 run spent `$0.287113`, completed 2/30 operations with both passing at 4.00/4, then failed closed at the Appendix J case and left 27 unattempted. Its authorization is consumed, and no user was charged for the failed operation. [No-cost classification](./PERMITEXT_ZONING_REMEDIATION_3_SEMANTIC_RESULT_2026-08-30.md) identifies an over-broad mapped-location safety trigger; the two changed successor cases were not reached. Its `$13.04` two-turn projection is not sample-ready, so remediation 2's `$20.18` per 100 all-Zoning risk projection and `$20.72` 100%-Zoning mixed-month p90 remain controlling. Cases 1, 3, and 11 remain held. A dedicated fail-closed public-release gate still requires a clean newly authorized semantic result, accepted cost and reliability, and exact-release web/TestFlight physical-iPhone acceptance. No further paid run is authorized, and public Zoning Research stays disabled. |
