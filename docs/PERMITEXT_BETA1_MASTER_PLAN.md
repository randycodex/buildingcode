...
| Zoning Research Beta 1 expansion | Expanded paid diagnostic retained; owner rubric disposition open | The complete corpus and no-cost safety contracts pass. The latest $5-capped frozen diagnostic spent $3.247980, completed 28/30 production turns without charging the two failures, and passed 12/28 graded answers. Four pipeline/evaluator defects and the generic mapped-location, transition-history, definition, date-path, arithmetic, and stated-scenario completeness gaps are repaired under no-cost contracts. Two invalid answer keys and six exact scope dispositions are prepared for owner review without changing the frozen cohort; Cases 1, 3, and 11 remain held. The expanded cost projection is $8.87 per 100 all-Zoning turns and 100% Zoning p90 is $9.33. A three-budget no-cost prototype preserved all 87 exact selections and eight structured sources; the retained 24,000-character supplemental candidate averaged 34,821 assembled characters but remains disabled. A newly authorized clean semantic run and web/iOS acceptance remain. Public Zoning Research stays disabled. |
...
- [ ] Obtain owner disposition on the two source-bound replacement answer keys and six scope dispositions, then create a separately versioned successor only from approved changes while keeping the frozen result unchanged.
