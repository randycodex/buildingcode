...
Checklist snapshot: **82 completed, 26 open**. Eleven of the open items are the final merge, deployment, TestFlight, and release sequence, which has not been authorized on this branch.
...
| Zoning Research Beta 1 expansion | No-cost safety gate passed; safely disabled | The complete corpus is refreshed through the official August 13 edition, including both new Queens Appendix F maps; all 21 evidence cases retrieve their selected authority and have owner approval for Terra answer-key testing only; and Zoning-specific citation, passage-hash, location/map, special-district, table, arithmetic, amendment, and date contracts pass. Capped validation, economics, and web/iOS acceptance remain. |
