...
The distinct post-v8 package was locked at exact commit `7cc2af325dbb3c5c98e4e15e2c15196a4794cb76`, then separately authorized for all 30 ordered cases, one repetition, and a `$5` cumulative cap. Execution commit `9d4af1b31762568caa5accf63b52e275f0e39bde` differed from the package only in the fresh authorization record. Run `1521497c-8df4-4ed9-98ce-79ef2805d1a6` preserved the exact cohort/order, disabled web support, matching locks, clean execution commit, and settled ledger, but stopped at the same third ordered case. The failed-answer text was intentionally not retained, so the exact cause is indeterminate between generated wording and v8 recognition; cohort and evidence drift are ruled out. Subsequent no-cost Zoning safety v9 adds privacy-bounded clause diagnostics, a structural Appendix J source-boundary repair, safe-wording regressions, and matching no-boundary/parcel-specific counterexamples. Its safety, economics, durable-message, consumed-authorization, and 30/30 conversation-preflight contracts pass with zero provider calls. This does not retroactively prove or rescore the paid result. Authorization ID `151aa121-7962-48d1-80b3-56728e62fc75` is consumed, and no further paid run is authorized. [The retained v8 result](./PERMITEXT_ZONING_REMEDIATION_3_V8_CONFIRMATION_RESULT_2026-08-30.md) did not authorize public access, deployment, pricing, allowance changes, or the 24,000-character candidate.

Current development branch: `codex/zoning-research-beta1`

## Status dashboard

Checklist snapshot: **107 completed, 14 open**. Eleven of the open items are the final merge, deployment, TestFlight, and release sequence, which has not been authorized on this branch.
...
- [ ] Independently review and retain the bounded v9 repair, resolve the remaining unconfirmed remediation-successor-2 paths/outcomes, reduce and accept cost, and confirm all 30 remediation-successor-3 cases in a newly authorized clean semantic run before enablement. Both the [first remediation-3 result](./PERMITEXT_ZONING_REMEDIATION_3_SEMANTIC_RESULT_2026-08-30.md) and [post-v8 confirmation](./PERMITEXT_ZONING_REMEDIATION_3_V8_CONFIRMATION_RESULT_2026-08-30.md) are not sample-ready; the [remediation-2 result](./PERMITEXT_ZONING_REMEDIATION_2_SEMANTIC_RESULT_2026-08-30.md) keeps the `$20.18` failed-work-amortized 100-turn projection controlling. No further paid run is authorized; any later paid confirmation requires a new distinct locked package, explicit exact-package one-run authorization, and a new cumulative cap.
