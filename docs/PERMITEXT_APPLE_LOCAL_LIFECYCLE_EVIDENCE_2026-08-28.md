...
The iOS project now supports an explicit build-time staging backend override while retaining the Production URL as its default, and the server includes a fail-closed no-cost verifier for the isolated Apple Sandbox staging configuration. No staging backend was deployed and no provider configuration changed. The remaining Apple provider gate requires an Apple Sandbox/TestFlight account, a separately authorized isolated staging deployment, App Store Connect Sandbox notification configuration, and capture of the real notification UUIDs and transaction identifiers listed in `BETA1_BILLING_IDENTITY_RUNBOOK.md`. A controlled production purchase remains separately approval-gated.
