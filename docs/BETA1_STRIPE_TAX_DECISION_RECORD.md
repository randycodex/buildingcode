...
- An official New York Business Express notice and the authenticated Business License Information dashboard show that the Certificate of Authority registration was **Issued** on August 28, 2026. The dashboard's `View Certificate` link currently returns the DTF-17 application copy rather than the actual certificate. Receipt, saving, printing/display, the effective date shown on the certificate, and the assigned filing frequency therefore remain unconfirmed.
...
- The closest documented Stripe category for Permitext's principal Beta 1 offering is `txcd_10701400`, `Website Information Services - Business Use`: an online information, search, and comparison service accessed through SaaS for commercial use. This is a no-change recommendation, not an accepted legal classification or a live Stripe edit. New York's official software bulletin independently says licenses to remotely access prewritten software are taxable in New York.
...
1. Receive the actual Certificate of Authority by mail or another valid agency delivery path, then save it privately, print/display it as required, and privately record its effective date and assigned filing frequency. Do not place the certificate image, taxpayer ID, or home address in source control.
2. Obtain owner approval to replace the generic live Product code with the recommended `txcd_10701400`, `Website Information Services - Business Use`, or record a better-supported specific classification. No live code has changed.
...
1. Record the actual Certificate and filing-frequency evidence privately.
2. Confirm the recommended specific Stripe Product tax code and add the applicable New York registration in Stripe Tax.
