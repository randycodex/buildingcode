...
| TestFlight | Staging-targeted builds 42–47 were uploaded from verified archives. Build 45 is `Complete`, `Ready to Submit`, and associated with Internal Testers, but its refund button did not present Apple's sheet on the physical iPhone. Build 46 contains the direct-StoreKit replacement and completed the Apple refund form and submission. Build 47 became available to the owner, was installed, and displayed Free for the already-refunded subscription. |
| Sandbox accounts | Dedicated United States test accounts exist. The original lifecycle tester completed the no-charge physical-iPhone purchase, restore, cancellation, expiration, and refund exercise. A newly created tester uses `Monthly renewal every 5 minutes` and completed the renewal exercise described below. |

Build 41 predates the build-time staging-backend override and retains the Production backend. Builds 42–47 were archived with `PERMITEXT_BACKEND_API_BASE_URL=https://permitext-apple-sandbox.vercel.app`; they are intentionally Sandbox-evidence builds, not final Production release builds. The Apple-created lifecycle gate remains open for billing failure/recovery, duplicate-delivery, and delayed-delivery evidence.
