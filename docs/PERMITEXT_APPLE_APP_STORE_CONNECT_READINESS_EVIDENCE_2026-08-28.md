...
The failed physical revocation exposed two source defects. First, `/billing/apple/transactions/verify` could persist an older active transaction after a newer Apple notification had removed the entitlement. Second, the iOS backend-verification path did not clear its cached Pro entitlement when the server returned no entitlement. The local repair compares Apple's signed transaction date with the persisted notification date, makes the newer notification authoritative, prevents the PostgreSQL claim from racing that cursor, and applies a nil backend response before iOS resolves its StoreKit snapshot. `npm run test:billing` now retains a purchase → refund → stale relaunch-verification regression and proves that replay remains Free while a genuinely newer repurchase can activate Pro; it made zero paid provider calls. The complete isolated iOS entitlement/sync suite passes 112 tests with zero failures in `Test-permitext-2026.08.29_00-02-13--0400.xcresult`. This repair is not yet deployed to isolated staging or included in TestFlight, so the refund lifecycle remains open until the physical iPhone displays Free.
...
6. [ ] Deploy the locally verified stale-verification repair only to isolated staging, upload staging-targeted build 47, and confirm that the already-submitted Sandbox refund resolves to Free on the physical iPhone.
