...
The failures cluster around over-broad pinned passages, visual/map handling, citation-to-claim support, missing required concepts, and exact qualification coverage. The next implementation gate is to narrow Zoning passage/visual assembly and resolve the nine scored failures plus one fail-closed operation before a clean frozen rerun. The owner-supplied `Permitext_NYC_Zoning_Research_Evaluation_Cases_Batch_1.md` was not mixed into this immutable run. Its separate no-cost intake now maps all 42 unique cited ZR sections to canonical content and verifies all 12 calculations. All 12 remain drafts: nine may proceed to explicit owner review; one needs narrowing, one is a near-duplicate, and one remains held for incorporated-map evidence.
...
Review the retained failure evidence, narrow Zoning passage/visual assembly without weakening the deterministic safety boundary, and rebuild the affected cases for no-cost validation. Then present the nine source-checked candidate cases for explicit owner disposition while leaving the three held/revision cases outside the evaluation cohort. Do not run another paid cohort without a new explicit cumulative cap, and do not enable public Zoning Research.
