...
A clean staging-targeted build 48 archive was verified as version 1.0 build 48 with the expected bundle/team, non-exempt encryption disabled, the isolated staging backend, and the compiled repair marker. Apple accepted upload record `443a851d-5987-456f-b890-ee6eefdeec78` at approximately `2026-08-29 2:34:08 PM EDT`; App Store Connect reported the build `Complete`, `Ready to Submit`, and associated with Internal Testers, and the owner installed it. The expired five-minute tester correctly showed Free, while its uncleared StoreKit queue produced a no-charge wait message instead of falsely claiming a repurchase. A fresh United States tester configured for hourly monthly renewal then completed a no-charge build 48 purchase. Isolated staging returned HTTP 200 for three transaction-verification requests, and the owner explicitly confirmed that the physical iPhone displayed `Pro (Test)`. This passes the build 48 Sandbox identity-continuity acceptance. The associated server-notification delivery received HTTP 503 before ownership was bound. Read-only database inspection shows the fresh transaction, represented only by hash prefix `6ddcd2e01e58`, was bound seconds later but has no matching notification-state row. Because Apple does not retry failed Sandbox notifications, the next no-cost exercise is to retrieve the exact missed signed payload through Apple's Sandbox Notification History, submit it once after ownership binding, submit it again unchanged, and verify delayed recovery plus duplicate idempotency against hashed database state. Billing failure/recovery now passes; failed-delivery recovery and duplicate-delivery evidence remain open.
