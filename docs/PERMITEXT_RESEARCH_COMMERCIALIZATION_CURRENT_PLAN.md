...
The permanent no-cost `audit:zoning-evidence-cost` command now records the next economic engineering target. The 28 completed expanded cases averaged 6,262 owner-reviewed exact-passage characters but 42,033 assembled evidence characters; 11 reached at least 47,000 of the 48,000-character ceiling. This audit does not authorize a lower evidence budget. A safe prototype must preserve exact pins, reviewed structured sources, controlling discovered provisions, and recall before any paid rerun.
...
Obtain owner disposition on the two source-bound answer-key replacements, prototype a pin-preserving supplemental-evidence budget under no-cost recall contracts, and then decide whether a clean semantic confirmation is justified. Any later paid request requires a new explicit owner authorization and a new cumulative spend cap. Do not enable public Zoning Research.
