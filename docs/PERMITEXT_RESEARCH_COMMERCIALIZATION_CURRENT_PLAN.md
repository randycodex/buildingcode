...
The failures cluster around over-broad pinned passages, visual/map handling, citation-to-claim support, missing required concepts, and exact qualification coverage. The no-cost evidence remediation is now complete: reviewed terms are fail-closed and section-bounded, ZR 12-10 uses canonical named-definition excerpts, Appendix J carries its two structured Subarea index tables while unselected maps remain excluded, and Section 42-00 carries hashed official amendment-history metadata separate from current enacted text. `npm run eval:zoning` now passes 21/21 evidence sets through conversation creation with no paid model calls, and the broader Research chat, safety, discovery, and evaluation self-tests pass. This does not claim that the prior nine scored failures or one fail-closed operation are semantically resolved; that requires a newly authorized frozen run. Detailed evidence is in [PERMITEXT_ZONING_RESEARCH_REMEDIATION_2026-08-30.md](./PERMITEXT_ZONING_RESEARCH_REMEDIATION_2026-08-30.md). The owner-supplied `Permitext_NYC_Zoning_Research_Evaluation_Cases_Batch_1.md` was not mixed into the immutable run. Its separate no-cost intake maps all 42 unique cited ZR sections to canonical content and verifies all 12 calculations. All 12 remain drafts: nine may proceed to explicit owner review; one needs narrowing, one is a near-duplicate, and one remains held for incorporated-map evidence.
...
Present the nine source-checked candidate cases for explicit owner disposition while leaving the three held/revision cases outside the evaluation cohort. After those dispositions, freeze the remediated cohort and exact commit. Do not run another paid cohort without a new explicit cumulative cap, and do not enable public Zoning Research.
