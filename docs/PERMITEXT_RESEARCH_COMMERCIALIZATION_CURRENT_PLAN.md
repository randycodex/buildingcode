...
Build 45 adds an in-app, Apple-controlled refund-request entry point for a verified active Pro transaction. It is available only for an authorized Apple Pro entitlement, opens StoreKit's native refund sheet, and explains that opening the sheet neither cancels nor refunds the subscription. The complete entitlement/sync contract suite passed 112 tests with zero failures. The archive was verified as version 1.0 build 45 with the exact isolated staging backend, bundle, team, and encryption settings, then uploaded successfully. App Store Connect has received build 45 and currently reports it as processing. No physical-device refund request has been submitted yet, so the refund lifecycle case remains open.
