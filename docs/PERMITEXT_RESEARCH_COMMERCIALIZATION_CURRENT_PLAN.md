...
### Isolated Apple Sandbox deployment and build 42 upload

The owner authorized a dedicated `permitext-apple-sandbox` Vercel project with a custom `apple-sandbox` Preview environment, a separate free Neon database, and a separate private Blob store. The environment contains no Stripe secret or OpenAI key, has paid Research turns set to zero and the Research kill switch enabled, and passes all 17 fail-closed staging checks. Its public `/health` endpoint returns HTTP 200 with PostgreSQL storage and the exact release commit `50a7b6bfa900a2909401786f5704951a11bcdac5`. Production was not deployed or reconfigured.

Build 42 was then archived with the isolated staging host as its embedded backend, inspected for the expected version, build, bundle, team, and backend values, and uploaded successfully. Apple completed processing; App Store Connect reports build 42 as `Ready to Submit`, expiring in 90 days, and associated with Internal Testers. No Sandbox purchase, App Review submission, pricing change, Production notification URL, or paid provider call occurred.
