...
This is technical readiness, not policy approval or activation. The purchase flow fails closed while approved current versions are unconfigured. Counsel review, stable public URLs, production environment configuration, deployment, and live purchase verification remain open; no price, deployment, or paid-provider behavior changed in this step.
...
Obtain counsel review and tax-professional direction for the prepared customer documents, product classification, registrations, and tax-inclusive/exclusive presentation. After the final documents and stable URLs are approved, configure their exact current versions to activate the already-wired consent flow. Local readiness also requires the exact $7 per-user monthly Research ceiling; Production configuration and deployed verification remain open. Complete Apple Sandbox/TestFlight billing evidence and the separately approved controlled production Stripe exercise before public billing. Replace the refund and infrastructure planning reserves only when actual customer and Vercel billing data exist. Before deployment, set the production per-user monthly Research spend cap to $7 and rerun readiness so the live guardrail supports the retained 100-turn allowance. Production web and the next compatible TestFlight build must then confirm the shared response contract; additional-turn prices and public paid Research remain disabled.
