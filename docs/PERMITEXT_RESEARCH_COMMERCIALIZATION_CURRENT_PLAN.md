...
Next sequence: complete the Zoning-specific no-cost Research contracts, and only then use the owner's available API balance under a written cap for a frozen Zoning benchmark. Its measured routing and cost must be added to the subscriber economics model before enablement.
...
For the newly added Zoning Beta 1 workstream, finish the Zoning-specific no-cost Research contracts against the verified August 13 corpus. Then propose a written cap for the first frozen paid Zoning benchmark using the available API balance. Do not enable public Zoning Research during this step.
