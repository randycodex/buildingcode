...
The first frozen paid diagnostic is retained at `permitext-sync-server/evals/results/2026-08-30T16-28-27-054Z-5b54b6cf-2a04-4a4a-a920-edb2d65bf4f6.{json,md}`. Its charging controls and $5 cap passed, but the quality result did not: 11 graded answers passed, nine failed at least one critical condition, and one production turn failed closed before grading. The failure pattern identifies broad passage selection, map/visual handling, citation support, and qualification completeness as the next engineering targets. No price, allowance, Production configuration, or public eligibility changed.

The owner also supplied `Permitext_NYC_Zoning_Research_Evaluation_Cases_Batch_1.md` after the frozen run began. Its no-cost source-integrity intake is now recorded in `permitext-sync-server/evals/zoning-candidate-batch-1-intake.json`: all 12 source cases remain drafts, all 42 unique cited ZR section URLs map to current canonical sections, and all 12 stated calculations pass. Nine cases are recommended to proceed to explicit owner review, while Case 1 needs narrowing, Case 3 is held as a near-duplicate, and Case 11 is held for visual map evidence. The source document's seven READY and five BLOCKED labels were not treated as approvals, and none of the 12 was added to the frozen cohort. Public Zoning Research remains disabled.
