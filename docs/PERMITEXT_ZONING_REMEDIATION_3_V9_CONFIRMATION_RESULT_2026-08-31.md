...
## Subsequent no-cost v10 diagnosis, rejection, and v11 hardening
...
Prospective Zoning safety v10 added recognition for six explicit generic forms, but it was rejected before packaging or paid use. Independent adversarial mutation review found that 23 of 26 and 21 of 30 unsafe variants could escape through broad generic-parenthetical removal and field-level source/suffix or continuation acceptance. V10 was therefore not promoted and cannot be treated as a reviewed release candidate.

Zoning safety v11 replaces those broad paths with allowlisted generic/source parentheticals, fully anchored generic-source and evidence-only instruction grammar, field-level default denial for unreviewed specific targets and suffixes, cross-sentence actor and subject-ellipsis state, and structural rejection when an uncertainty boundary masks a project outcome before or after the boundary. Every reported unsafe form and benign false positive is retained as a regression. The bounded independent reviewer returned a clean gate only after the focused safety suite passed; public Research remained disabled throughout.

The consumed v9 guard validates its reviewed safety, economics, and application bytes from historical commit `1fae244d775192f55f0fd6ee17d90cb82648ba01`, rather than requiring future working files to remain byte-identical. Its authorization, retained results, hashes, and outcome are unchanged. The v11 safety contract and consumed-authorization contract pass, and the frozen 30-case mock preflight reports 30/30 evidence-ready with credentials unset, zero provider tokens, and zero paid calls. This is prospective no-cost engineering evidence only: no semantic confirmation, paid authorization, rescore, public enablement, merge, push, deployment, pricing change, or allowance change occurred.
...
The no-cost clause diagnosis, v10 rejection, and bounded v11 hardening/review are complete. The next Zoning step is to design and review a distinct locked v11 confirmation package without running it. Do not run another paid confirmation without that new exact package, a fresh explicit owner authorization for all 30 ordered cases and one repetition, and a new cumulative cap no higher than `$5`.

## Subsequent locked v11 package

That distinct package is now prepared and independently reviewed at exact commit `8d075b442083db3536de0ff9e90372802ddeadaa`. It preserves this v9 result and its consumed authorization as immutable historical lineage; it does not edit, rescore, or reactivate either. Locked v11 authorization ID `ee72ca2f-5410-4ce9-a6d6-30deb8ff5169` has no owner phrases, scope values, execution commit, attempt, or run. Its exact input hashes, signed one-time runner handoff, reviewed runtime overrides, focused and hostile-environment contracts, 30/30 mock preflight, complete repository check, and independent review pass without a provider call. Preliminary commits `682e7fcfd26390207e310b228be43fb74d72b1ab` and `981c02a2b284fb2207b9ffba4a9c4100dfa56d44` were superseded before authorization.

The next gate is no longer package design. It is a fresh exact owner decision naming package `8d075b442083db3536de0ff9e90372802ddeadaa`, all 30 ordered cases, one repetition, and a maximum cumulative API spend of `$5`. Until then, no v11 paid run is authorized. Evidence: [PERMITEXT_ZONING_REMEDIATION_3_V11_CONFIRMATION_PACKAGE_2026-08-31.md](./PERMITEXT_ZONING_REMEDIATION_3_V11_CONFIRMATION_PACKAGE_2026-08-31.md).
