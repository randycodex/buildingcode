...
## Edition clarity follow-up — 2026-09-22

W003 remediation implemented locally: Saved evidence rows, Project evidence rows, Note reference choices and resolvable existing Note references show their source edition. Report source summaries now preserve edition and source-library version through the HTTP response. New immutable evidence snapshots retain explicit edition; both PDF renderers label individual evidence and distinguish included passage editions from the Project default. Legacy Report rendering reads recorded provenance without rewriting snapshots; absent provenance is disclosed. The 1968 Building Code is identified separately from its enclosing administrative-code package.

Validation: actual 2014 BC 101.1 and 2022 BC 1010.1.2.2 passed through isolated HTTP Project links → Report source list → generated immutable Report with both editions preserved. The historical 1968 reference test also passes. Mixed-edition PDF fixture rendered and both pages visually inspected. Report contract, Research handoff presentation, Note-reference availability, existing-Note label refresh, and offline cache contracts pass. iOS Debug build succeeds. Local web Saved shows `Paragraph · 101.1 · 2014` and `Section · 1010.1.2.2 · 2022` together; Report picker shows `BC · 1010.1.2.2 · 2022 New York City Construction Codes`.

Release boundary: these are local source, HTTP, PDF, browser and build checks. No Production deployment, TestFlight publication, or physical-iPhone visual acceptance for this change is claimed. Previously generated PDFs remain unchanged. Note references whose original saved source is unavailable retain their existing label rather than guessing an edition. No paid model requests were used for this follow-up.

## Project deletion follow-up — 2026-09-22

Implemented locally: single and bulk Project deletion confirmations explicitly use the global warning layer above Project management, with Cancel/Move to Trash actions and 30-day Account → Trash recovery wording. Explicit document-body warning destinations are no longer treated as column-scoped overlays. Before deletion, existing sync/conflict and Trash-service preflight must succeed; recovery failure leaves the records intact and displays a global error. Existing best-effort retired Workboard cleanup remains unchanged.

Validation: browser-created disposable `QA Trash Recovery`; confirmation visibly above disabled manager; Cancel restored enabled controls and retained selection; confirmed deletion removed the disposable Project from Active; Account → Trash listed its name and expiration; Restore / Undo returned it to the Project menu. Returned to the original Flatiron QA workspace afterward. Trash unit/HTTP/web checks, unavailable-recovery and Cancel regression checks, retired Workboard storage-failure regression, and offline cache checks pass. No Production deployment or physical-iPhone retest is claimed.

## W005 Project Research association — 2026-09-22

Reproduced the actual identity mismatch locally: Saved passed a sync storage record ID (`user:project:codeVersion:clientID`) rather than the shared client Project ID. The server authorized that alias but stored it verbatim as primaryProjectID. This explained failed native Project-name matching and absence from the canonical Project Report source list.

Fixed the Saved launch to use projectDetailKey for Projects (reference collections remain unassigned). Conversation creation and explicit Project reassignment now resolve an authorized Project before persisting its canonical identity; creation fingerprints use that same identity so alias/canonical retries are consistent.

Verified locally: browser launch from the Flatiron Project's 2022 saved passage persisted canonical `web-project-muc0pm2q`; a deterministic Project-summary question returned the correct saved address and facts; that exact answer appeared in the Flatiron Report source picker and was added to its draft. The visible conversation is `c6c9ef0c-4b50-4b59-b4a9-14cb2cec94ae`; answer mode project_context, modelRequested false. Three columns remain open (Saved, Report, Research).

HTTP regression covers storage-ID creation, canonical-ID replay, history/get payloads, Project summary, Report inclusion, moving via another Project's storage ID, unassignment, and immutable historical answers/manifests. Client summary/context, Reader recovery, and offline checks pass. Updated the prior edition-label assertion to match the already-shipped local edition fix. No external/provider requests were required. Production and physical-iPhone verification remain pending. This change prevents new malformed associations; it does not silently rewrite historical answers or existing malformed conversations.

## 2026-09-22 — Follow-up on walkthrough items 5–10

### 5 — Native citation first-open reliability (local development)

Found an edition-publication race: `isInitialContentLoaded` intentionally stays true while the previous code edition remains readable. Research published the pending citation before the replacement content task completed, so Search could consume it against the previous edition. Citation navigation now waits for content publication and ignores canceled loads; universal section links share the same barrier. Added a held-publication regression case for historical 2014 content and updated the asynchronous navigation check. `build-for-testing` passes; these new unit cases have compiled but have not yet executed (no Simulator runtime is installed).

Physical Randy iPhone, locally built Debug app, 2026-09-22 approximately 01:36: opened the controlled Research answer's BC 101.1 citation on the first tap. Reader showed Building Code 2022 / 101.1 / enacted text without Retry. This verifies the ordinary first-open path, not a forced cross-edition race on-device. No TestFlight or Production release is claimed.

### 6 — Note archive empty-state navigation (local browser passed)

The Note rail/header previously hid when the visible list was empty, taking Archive/Active navigation with it. Keep both containers present and allow the non-mutating archive toggle for read-only users. Verified in localhost workspace: empty archive retains Show active notes; created `QA Archive Navigation`, saved content, archived the last active Note, opened Archive from the resulting empty active view, restored the Note, and returned from empty Archive to its active listing. Disposable Note retained for review.

### 7 — Research notice contrast, plus requested composer cleanup (physical iPhone passed)

The prominent notice button inherited the app's white dark-mode tint. Give Continue to Research explicit blue background and white text. Verified readable blue/white button on the physical iPhone, and Continue successfully opened the controlled answer. Added an isolated DEBUG launch option to reset only the synthetic fixture account's notice acknowledgement. The owner requested removal of the two lines above the composer; removed the routine remaining-turn line and duplicate changed-source instruction there. The actionable changed-source notice remains in the conversation. Verified both lines absent, with the notice retained. Mirroring scrambled an earlier typed test question; replaced it using sequential keys, visually verified `What is 101.1?`, then submitted to the canned local transport. No provider calls were made. Relaunched the app without fixture arguments and told the owner the phone was free to disconnect.

### 8 — Offline readiness (Chrome disconnected read/search and regression tests passed)

Reproduced concrete shell failures: obsolete `/marketing/home.css?v=3` returns 404, preventing worker installation; unbounded `serviceWorker.ready` left Preparing offline app pending. Removed obsolete stylesheet and aligned the marketing script URL; added worker-failure detection and a 45-second preparation timeout. Initial real Chrome download completed all 578 chapters, but disconnected cold reload then exposed missing cached module imports (`/web/sync-identity.js` and the older versioned workspace-state dependency). Added exact imported URLs and current analytics/favicon assets. Shell v1199 / app v556. Added tests for asset existence, transitive static imports, worker activation, redundant worker failure, cache failure, and timeout; offline contract and installer recovery pass.

Actual Google Chrome UI verification: Downloaded for Offline Use appeared; DevTools Network preset set to Offline; cold workspace reload displayed Offline and retained Pro. Search `101.2` returned 19 matches grouped by code/edition; expanded 2022 Building Code, opened its result in a new Reader, and verified complete BC 101.2 Scope text plus adjoining chapter provisions with networking still disabled. Restored No throttling and closed DevTools; workspace returned to Synced. No macOS-wide network settings changed.

### 9 — Web PDF download (standard Chrome passed)

Opened existing QA Flatiron Report draft, exported immutable version 1 (manifest `21104718-50f9-46dd-a2af-3ceb24394d80`). Export automatically opens print preview; canceled that preview to separately exercise Download Web PDF. Chrome completed `QA-Research-Flatiron-Code-Research-Report-web-pdf.pdf`, 28.6 KB, and opened the downloaded three-page PDF successfully. Actual path: `/Users/randy/Library/Mobile Documents/com~apple~CloudDocs/Downloads/QA-Research-Flatiron-Code-Research-Report-web-pdf.pdf`. PDF viewer exposes the Project facts and professional-use notice. This resolves direct download in Chrome; prior in-app browser failure remains browser-specific/unproven. No download code change was required.
