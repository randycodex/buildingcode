...
Implementation and offline acceptance: explicit construction-code editions now select their available corpus in the same conversation; ordinary follow-ups retain the most recent edition focus, while explicit comparisons preserve both. Historical research does not establish project applicability. Explicit EBC requests load its actual text with future-effective July 17, 2027 metadata. The unavailable 2008 edition is disclosed without substituting 2022. Exact pinned IDs, ancestor references and cross-references preserve source edition; a 2014 ramp passage cannot acquire a same-numbered 2022 identity. Requested-but-unavailable corpora cannot receive the enacted-support badge merely because another corpus supplied citations.

The exact screenshot question after the prior 2022 turn and project default retrieves actual BC68 ramp evidence including §27-377. Focused actual-corpus tests cover edition transitions, shorthand/generic follow-ups, code families, comparisons, construction-year negatives, unavailable 2008, EBC retrieval and pinned/cross-reference identity. Existing corpus, historical-section, code-basis, evidence-assembly, conversation-topic, trust-boundary, answer-quality and truncation regressions passed without external model calls. EBC supplemental cross-references still have unresolved targets; this is not a claim of complete cross-reference coverage or acceptance of every generated legal answer. No native code changed for this correction; phone verification/TestFlight gates from the prior recovery work remain open. Production deployment is recorded separately after publication.

The preceding Research recovery fix (`1fd2c92c5d10da18d947d06aefc252e893f984c0`) is deployed: Vercel `dpl_EbCPePNmGTDzj3AKM1vj9wwgcttk` reached READY, and production `/health`, `/release`, recovery v122 and shell v1147 were verified. Native physical recovery/migration acceptance and the replacement TestFlight release remain pending.
...
| Original item | Reconciled status and evidence | Limits / remaining action |
| --- | --- | --- |
| 1. Review changes and passed evidence | Complete. Prior adaptive-restoration/offline-save/Search results reviewed; latest housing activation and DEBUG locator correction reviewed. Relevant popup, return-position, normal-navigation screenshots and opening video samples inspected. | Historical pending statements below are retained as history, not current tasks. |
| 2. Reader opening / exact restoration | Complete for agreed samples. Build 84 settled 1968 restoration and edition/Search gates passed. Direct build 89 passed first and repeated HMC chapter 2, 2014 BC chapter 7 and EBC chapter 15 openings, preserving the remembered passage within four points. Six 10-fps transition contact sheets show text during the sampled animations and no Opening row or full blank destination. Physical table gesture, long definition/Close, linked Note caret and larger-text checks are recorded below. | Prepared/prewarmed samples are not universal cold-cache, every-chapter or every-frame acceptance. VoiceOver is owner-deferred. |
| 3. Notebook failure recovery | Complete within agreed controlled and live-phone checks. Owner performed airplane-mode/reconnect/Retry on build 81 and reported sync; subsequent production/phone checks found the original Note and one appended phrase, without duplicate Notes. Actual controlled HTTP lost-response replay retained one Note, one activity and the original mutation. Build 84 cached-refresh typing and normal edit/Done/reopen passed. | Exact transient radio-outage error wording was not independently captured. Other interruption scenarios are not inferred. Redundant normal Save remains removed. |
| 4. Development installation / phone verification | Complete. Build 89 updated the existing app in place. Four targeted physical tests passed: housing full-body/all-35-ranges unit test, real glyph popup/Close/return, normal five-tab navigation, and three-chapter first/repeated openings. Normal Permitext relaunched without fixture arguments afterward. Prior build 84 normal Note and existing Research answer/draft restoration evidence remains applicable. | This is a direct development install, not a TestFlight installation. No reset, uninstall, simulator or duplicate build directory. |
| 5. Integration gates | Complete within agreed boundaries: actual HTTP Research interruption/replay with zero provider calls; disposable live archive/restore; controlled native partial-property feedback plus live provenance inspection; safe synthetic session revocation returning 401 while retaining the original Note. | No owner-account revocation, paid-provider outage or live upstream partial outage is claimed. |
| 6. Complete remaining-work reconciliation | Complete. Original UX/UI work is separated from further source extraction/applicability work in [the definition backlog](PERMITEXT_DEFINITION_BACKLOG.md). Bounded light/dark and current-phone larger-text evidence is retained below. | Further Report work, ten unresolved authoritative-source entries and VoiceOver remain explicitly deferred. All-screens/all-text-sizes coverage is not claimed. |
| 7. Commit / integrate / release | Physical acceptance is complete; final source/evidence commit, remote integration and final TestFlight publication are being completed and will be recorded in a later release checkpoint. Latest already-uploaded TestFlight was 88; phone is direct development 89. | Do not label local activation as deployed or TestFlight-ready until the release checkpoint confirms it. |

### Final physical evidence

- `/tmp/permitext-housing-physical-unit.log`: `testTitle26HousingReportingFullBodiesAndAll35Ranges` passed; complete source bodies, exact referral citation, all 35 ranges and negative scopes verified.
- `/tmp/permitext-housing-physical-ui-final.log` and `/tmp/permitext-column-ux-build/Logs/Test/permitext-housing-physical-final.xcresult`: actual §26-2602 Affordable housing unit glyph, full imported body, §26-2201 citation, visible Close and fresh post-dismissal viewport passed. Screenshots in `/tmp/permitext-housing-physical-final-images` were inspected. The first run failed because the DEBUG locator phrase did not contain the target term; corrected the phrase, rebuilt and passed. No release behavior changed for that test correction.
- `/tmp/permitext-closeout-opening.log` and `/tmp/permitext-column-ux-build/Logs/Test/permitext-closeout-opening.xcresult`: normal five-tab test passed (32.270 s), three-chapter opening test passed (57.617 s), zero failures. Recording `/tmp/permitext-closeout-opening-images/7E7F15DE-E5AD-4237-A280-49A8DE732247.mp4`; six inspected contact sheets `/tmp/permitext-opening-89-{hmc,2014,ebc}-{first,repeat}.png`. DEBUG Reader fixtures have two tabs; the separately tested normal app has all five. Attachment retention was restored to deleteOnSuccess after export.

Eleven housing-reporting definitions are now physically accepted alongside 70 focused tests, 674 integrated browser checks and both offline contracts. Area median income remains withheld. Full wording, provenance, exact chapter identity and edition boundaries are preserved. This finishes the already-started definition batch; it does not complete all definition coverage.
...
The final generic build is held with approximately 558 MiB free. The storage guard audit → clean → audit removed no targets; the prior general-only build does not verify this new companion. Current companion changes are preserved in local commit `079e25e2b` and remain unpublished. Production remains the verified general Class A release `89c876191c7b14aed17ea1344c92480b68230fdc`. Phone remains direct development build 82, untouched; no TestFlight release occurred. Physical popup/touch/table/Dynamic Type/reference-return checks and new-build Research/Note sanity remain pending. Broader definition applicability remains open; further Report work and the ten unresolved authoritative-source entries remain deferred.
