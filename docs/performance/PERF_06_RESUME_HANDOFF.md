> Latest PERF-14: locally complete. Public timing instrumentation, isolated fresh-process harness, shared chapter HTML reads and parsed all-heading reuse are implemented. Full Chapter 33 improved from reproduced 30-second timeouts to 635–637 ms in three controlled samples; 18 cases pass without failures and preserve identical responses. Frozen pre-change heading/body hashes, figure bindings, all Chapter 33 windows, cancellation, cache/privacy, backend and full smoke checks pass. Evidence: `PERF_14_FIRST_USE_INVESTIGATION.md` / `PERF_14_FINAL_MEASUREMENTS.json`. Next is PERF-15 web startup trace; do not request phone for that. Physical iOS acceptance remains open. Owner preview localhost8796 remains untouched; backend8797 has not been restarted for PERF-14, so does not yet include its server changes. No merge/push/deployment performed.

> Current checkpoint: PERF-13 locally complete for web; see PERF_13_REVISION_SAFE_PUBLIC_CACHE.md and its HTTP/browser/projection evidence. Public text and figure caches are revision-bound; browser updates/rollback/reconnect invalidate safely; 578 regenerated indexes preserve all search content. Focused suites, smoke and deployment verification pass. Next single task: PERF-14 first-use request tails, using new cache baseline. Synthetic backend8797 session60695; fixture8801 session3859; owner preview8796 untouched. No phone/simulator used; user returned. Local branch only, no merge/push/deployment.

> Current checkpoint: PERF-12 is locally complete for web. Query coalescing reduces populated-workspace keyboard capture from 40 writes/55,161 characters to four writes/6,156 characters (background activity caveat; not latency). Immediate reload, held-request cancellation, simulated composition and delete/retype browser checks pass. Shared chapter request leases protect other consumers. Focused suites and all smoke components pass in a segmented run after two stale PERF-11 assertion updates. See PERF_12_SEARCH_TYPING_AND_CANCELLATION.md and browser evidence. Next single task: PERF-13 public content caching by revision. Owner preview8796 remains untouched; synthetic backend8797 session76619 and proxies remain available. Phone unavailable while owner is out; no simulator. Work is local to codex/permitext-performance, not merged/pushed/deployed.

> Latest local commit: 4bb3b27ae, PERF-11 startup data reuse, Reader normalization identity and Report Retry. Full smoke, access/hydration/offline contracts pass. Browser v564 shows one early Project foundation read, six panes, no loading shells; later ~5s read remains to attribute. Account A-to-B and same-user token replacement browser evidence pass. Active offline check uses perf11-offline at proxy8800 (session98159), forwarding test backend8797; its initial download failed because local /web/analytics.js routing does not find generated public/web/analytics.js. Agent perf11_access_audit is correcting local static routing. No phone needed; owner preview8796 stays untouched.
