...
Native chapter loads already coalesce; authored per-section decode requests can still duplicate work when the same cold passage is requested concurrently. This change bounds retention and protects correctness during those races; it does not claim to coalesce every cache type. Search indexes and per-view completed preview strings are retained and need aggregate resource measurements before selecting further budgets. Physical memory stability and latency acceptance remain open.
