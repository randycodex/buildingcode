...
This is evidence that reduced source scope reduces matching work, not proof of app startup or displayed-result latency. Harness metadata/token preparation is not the complete AuthoredCodeStore initializer. No UI, background warming or completed-search persistence is measured; repeated matching explicitly reruns the engine rather than measuring a saved-search cache hit. OS file caches were not cleared. The benchmark passes resultLimit:nil;2022/2014each returned500observed matches. There is no500result cap in this tested call. Parity is query/corpus-specific, not an exhaustive search assertion. Snippet generation is enabled in the benchmark, whereas the app initially requests lightweight results without snippets, so these timings are not the initial-results app path. All other packs contributed286results that the reduced scope intentionally excludes. Such scope must therefore remain explicit and user-controlled.
...
## Schema2 canonical manifest binding — September25

The exporter and isolated Swift installer now use schema2: bundlePath plus structured sourceIdentities matching the app identity representation. Revisions bind the canonical path and source set along with payload hashes and compatibility. After integrity verification, the installer derives the identity set from the included bundle.json and requires exact equality, valid code/jurisdiction references and no duplicate identities. Old schema1 prototype exports are rejected and must be regenerated; no shipped-app migration is implied.

Validation: three Python catalog/export tests pass, including all22 native identities, deterministic revisions, changed namespace revision invalidation, and rejection before output creation. Six macOS Swift tests pass; fourteen identity/metadata mutations reject before copying while preserving the prior verified active revision. Existing corruption, storage, interruption, rollback and offline-reopen tests remain passing. A real276file Existing Building pack installed in0.201s (one host sample) and reopened in a fresh process with its export path unavailable. Exact receipt: PERF_17_SCHEMA2_INSTALL_REOPEN.json. This supersedes the prior paragraph stating that installer manifests retain prototype identity strings.

Remaining: application-installed catalog discovery and revision selection, real Reader compatibility, trusted transport/catalog publication, interrupted transfer resume, cache invalidation and reference integration, existing-install migration, and physical measurements. No resource removal, phone change, release or production download support.
