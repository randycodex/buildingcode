...
## Development-only resource capture for41.23

The existing compile-gated recorder now adds active-only1Hz TASK_VM_INFO resident bytes, process resident high-water mark, physical footprint and thermal state. Lifecycle active/inactive entries make gaps explicit; reads raced with a lifecycle change are rejected. Resource buffer is capped at600 entries including transitions; truncation is explicit and rejected by the analyzer. Nonzero Mach errors omit measurements instead of reporting zero. The resource array extends schema1; old timing captures remain readable. No query/account/content fields or network transport added.

The existing Swift6 contract passes enabled/disabled builds, real Darwin memory reads,1Hz timer, inactive/raced reads, error representation, cap/drop accounting and bounded persistence. Seven Python analyzer tests pass including resource gaps/failure/truncation. Recorder compilation remains opt-in; normal builds omit its path/writer. Capture should start from a fresh process and last about5minutes. Memory includes instrumentation overhead; sampled footprint maxima may miss brief peaks, resident peak covers process lifetime. No CPU, frame, scheduling-delay or leak-attribution claim. Build41.23/device capture pending.

## 41.23 mixed-use physical resource capture

A fresh process produced runD6885290-6673-4BF8-9E2B-F60C4F381968 with320 valid memory samples over326.059 seconds,24 milestones, zero failed reads and zero dropped events/resource entries. Raw/summary: PERF_18_RELEASE_41_23_MIXED_RESOURCE_EVENTS.json and PERF_18_RELEASE_41_23_MIXED_RESOURCE_SUMMARY.json. Observed sequence: Building2022 Chapter9, section picker jump901.2, Saved, existing project and Notebook image, Reader return retaining901.2, Building Chapter2, one sprinkler query/group expansion and107.9.1 detail with903.1.2 reference, Reader return, Home/background and foreground, then final recovery dwell. No content/account/source edits. This is one bounded mixed-use pilot, not sustained heavy interaction throughout every second or a populated Saved stress test.

Physical footprint first122.8 MiB, minimum105.5 MiB, sampled peak270.1 MiB, last206.1 MiB. Resident sampled maximum456.9 MiB; kernel process resident peak457.2 MiB. Thermal state remained nominal (raw0). Explicit inactive/active transitions recorded and sampling resumed. Footprint declined from peak; final usage remained above initial, so no leak-free/bounded-long-run claim or causal allocation attribution follows. Instrumentation overhead is included. CPU/main-thread stalls/display frames/cache ownership remain unmeasured; T5 is partial, not complete.

Callback samples in this run: Chapter9 256.475 ms; Chapter2 112.131 ms. Sprinkler completed matching792.599 ms with no completed-cache hit;2022 Building315,2014 Building271,1968 Building52 were visible. Its107.9.1 result body271.144 ms and references310.996 ms. These are individual callback samples, exclude input debounce and are not p95/display-frame measures. Content/edition/reference rendering was observed separately.
