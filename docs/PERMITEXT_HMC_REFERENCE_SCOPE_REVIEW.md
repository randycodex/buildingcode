...
## September 17 — Local activation and actual-source verification

The generator now applies the reviewed Harassment binding locally. Only its registry record changes; all original bodies, identities and aliases are preserved. Actual-source verification checks all 125 occurrences: 38 accepted and 87 excluded. The browser fixture passes 362 checks, including the 38 real-source links and exact ranges; native source tests preserve the 45-paragraph, 11,330-character body and every occurrence boundary. The native long-popup test passes complete text, final clause/citation, reachable Close and exact source-viewport return. Parent inspected the actual top and bottom screenshots in `/tmp/permitext-hmc-scroll-position-attachments`.

That rendered work exposed a separate distant-section Reader positioning defect. A local geometry correction now passes a stricter 30-second no-input heading-position test, picker return and manual scrolling; broader regressions are still running. See the latest continuation/closeout checkpoints for current acceptance. Definitions v79, Reader v486 and shell v1138 remain local pending final build/review/publication. The combined HMC source and matcher suite, including the inactive Class A proposal, now passes 100 tests. No physical-phone or TestFlight acceptance is claimed.

### Final native presentation and Reader regression verification — September 17

The complete Harassment popup, final clause/citation, persistent Close and exact passage return pass in the final eight-test simulator batch (`/tmp/permitext-reader-eight-final-ui.log`). Parent inspected final screenshots in `/tmp/permitext-reader-eight-final-images`. Strict initial/picker retention, immediate user-scroll cancellation, requested HTML fallback/footer, EBC/lazy responsiveness, independent Reader restoration and historical Search also pass. The latest generic artifact explicitly reports build 83; compiled/public registry bytes match. These checks do not establish physical acceptance or change the original complete definition. Publication remains separately recorded in the column closeout.
