...
- The no-cost source audit originally found that Stripe Checkout did not enable automatic tax, declare inclusive/exclusive tax behavior, or collect a billing address for tax. The local implementation now has a dormant automatic-tax path: when explicitly configured, subscription Checkout sends `automatic_tax[enabled]=true` and requires a billing address. Production fails closed while the mode or Price tax behavior is unconfigured, and the live readiness audit rejects a Stripe Price whose resolved behavior does not match the explicit local decision. For the current USD Price, the verifier resolves Stripe's documented `Default (inferred by currency)` behavior to `exclusive`; no replacement Price is required solely for that label.
- On August 30, 2026, the owner approved `PERMITEXT_STRIPE_TAX_MODE=automatic` and `PERMITEXT_STRIPE_PRICE_TAX_BEHAVIOR=exclusive`, with the exact web purchase disclosure `$20/month plus applicable taxes shown by Stripe.` `exclusive` means applicable tax is added above the $20 base price; `inclusive` would absorb applicable tax inside the displayed $20 price and reduce retained revenue. On September 2, the owner reported possession of the actual New York Certificate of Authority, confirmed that it is privately saved and printed/displayed, approved and submitted Product tax code `txcd_10701400`, `Website Information Services - Business Use`, added the New York Stripe Tax registration with collection starting immediately, and staged both activation values in Vercel Production. The owner-supplied DTF-17 application shows `09/18/2026`; actual-certificate corroboration remains open. Official New York guidance confirms quarterly filing for this registration, with the first return due December 21, 2026. No deployment or customer charge occurred; filing setup, exact policy deployment, and real taxed-Checkout evidence also remain open in [BETA1_STRIPE_TAX_DECISION_RECORD.md](./BETA1_STRIPE_TAX_DECISION_RECORD.md).
- The source guard does not receive or display the actual New York certificate, decide where Permitext has collection obligations, configure tax filing, or verify a real taxed Checkout. The 5% tax line remains an unresolved downside reserve, not a tax rate. Provider and key-presence evidence is retained without the taxpayer ID in [PERMITEXT_BETA1_STRIPE_TAX_PROVIDER_ACTIVATION_2026-09-02.md](./PERMITEXT_BETA1_STRIPE_TAX_PROVIDER_ACTIVATION_2026-09-02.md).
...
- Actual real-Checkout tax calculation, expected refunds, and measured launch-volume infrastructure allocation remain unverified commercial inputs.
