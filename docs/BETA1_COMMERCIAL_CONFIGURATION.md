...
- App Store tax handling is separate from Stripe. Apple documents the App Store customer price as inclusive of applicable taxes it collects and remits, and developer proceeds as customer price minus applicable taxes and commission. Stripe automatic tax therefore remains web-only. An authenticated read-only inspection verified that the parent app displays `App Store software` and the subscription displays `Match to parent app`. Owner classification confirmation and the first real Apple financial report remain open; the no-change record and owner checklist are in [BETA1_APPLE_TAX_HANDLING_RECORD.md](./BETA1_APPLE_TAX_HANDLING_RECORD.md).
