...
- On August 30, 2026, the owner approved `PERMITEXT_STRIPE_TAX_MODE=automatic` and `PERMITEXT_STRIPE_PRICE_TAX_BEHAVIOR=exclusive`, with the exact web purchase disclosure `$20/month plus applicable taxes shown by Stripe.` `exclusive` means applicable tax is added above the $20 base price; `inclusive` would absorb applicable tax inside the displayed $20 price and reduce retained revenue. The disclosure is prepared locally, but neither activation value is set in Production. Certificate, Product tax-code, registration, deployment, and real taxed-Checkout evidence remain open in [BETA1_STRIPE_TAX_DECISION_RECORD.md](./BETA1_STRIPE_TAX_DECISION_RECORD.md).
