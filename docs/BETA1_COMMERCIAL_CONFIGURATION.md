...
- The no-cost source audit originally found that Stripe Checkout did not enable automatic tax, declare inclusive/exclusive tax behavior, or collect a billing address for tax. The repaired path is now deployed: configured subscription Checkout sends `automatic_tax[enabled]=true` and requires billing information. Production fails closed while the mode or Price tax behavior is unconfigured, and the live readiness audit rejects a Stripe Price whose resolved behavior does not match the explicit decision. For the current USD Price, the verifier resolves Stripe's documented `Default (inferred by currency)` behavior to `exclusive`; the controlled real transaction verified that tax was added above the base price.
- On August 30, 2026, the owner approved `PERMITEXT_STRIPE_TAX_MODE=automatic` and `PERMITEXT_STRIPE_PRICE_TAX_BEHAVIOR=exclusive`, with the exact web purchase disclosure `$20/month plus applicable taxes shown by Stripe.` `exclusive` means applicable tax is added above the `$20` base price; `inclusive` would absorb applicable tax inside the displayed `$20` price and reduce retained revenue. On September 2, the owner reported possession of the actual New York Certificate of Authority, confirmed that it is privately saved and printed/displayed, approved and submitted Product tax code `txcd_10701400`, `Website Information Services - Business Use`, added the New York Stripe Tax registration with collection starting immediately, and deployed both activation values with the exact policies. The owner-supplied DTF-17 application shows `09/18/2026`; actual-certificate corroboration remains open. Official New York guidance confirms quarterly filing for this registration, with the first return due December 21, 2026. A separately authorized real Checkout then charged `$20.00` plus `$1.78` New York tax, and its full `$21.78` refund correctly ended Pro and returned the account to Free. Filing operations remain open in [BETA1_STRIPE_TAX_DECISION_RECORD.md](./BETA1_STRIPE_TAX_DECISION_RECORD.md).
- The source guard does not receive or display the actual New York certificate, decide where Permitext has collection obligations, or configure tax filing. The 5% tax line remains a conservative downside reserve, not a tax rate; the single controlled `8.875%` New York transaction is acceptance evidence, not a replacement incidence forecast. Provider evidence is retained without the taxpayer ID in [PERMITEXT_BETA1_STRIPE_TAX_PROVIDER_ACTIVATION_2026-09-02.md](./PERMITEXT_BETA1_STRIPE_TAX_PROVIDER_ACTIVATION_2026-09-02.md), with redacted transaction evidence in [PERMITEXT_BETA1_PRODUCTION_STRIPE_LIFECYCLE_2026-09-02.md](./PERMITEXT_BETA1_PRODUCTION_STRIPE_LIFECYCLE_2026-09-02.md).
- App Store tax handling is separate from Stripe. Apple documents the App Store customer price as inclusive of applicable taxes it collects and remits, and developer proceeds as customer price minus applicable taxes and commission. Stripe automatic tax therefore remains web-only. An authenticated read-only inspection verified that the parent app displays `App Store software` and the subscription displays `Match to parent app`; the owner accepted leaving that classification unchanged for Beta 1. The first real Apple financial report remains open; the no-change record and owner checklist are in [BETA1_APPLE_TAX_HANDLING_RECORD.md](./BETA1_APPLE_TAX_HANDLING_RECORD.md).
- The real controlled Checkout verified one `$1.78` tax calculation and one full refund. Customer refund incidence and measured launch-volume infrastructure allocation remain unverified commercial inputs.
...
- On September 2, 2026, the live Product was updated and independently reread as `txcd_10701400`, `Website Information Services - Business Use`, because Permitext provides professional online code information, search, and comparison through SaaS. Stripe's separate account preset remains generic, but the Product-level code overrides that preset for this Product under Stripe's documented tax-code precedence.
- Stripe confirmed that the New York registration was added successfully and that Sales tax collection starts immediately. The Locations view shows one New York registration; its `Needs attention` status concerns separate filing setup. The owner confirmed private certificate storage and printing/display, and the DTF-17 application records `09/18/2026`; confirmation from the actual Certificate remains open. Official New York guidance confirms quarterly filing and a first deadline of December 21, 2026. The Production values and exact policies are deployed, and the real controlled taxed Checkout/refund passed; the operational filing process remains open.
...
- Vercel Production `STRIPE_PRO_PRICE_ID` uses the `$20` Price, and the verified serving deployment completed a real controlled transaction at that base amount.
