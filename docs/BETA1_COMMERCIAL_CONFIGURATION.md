...
- Stripe confirmed that the New York registration was added successfully and that Sales tax collection starts immediately. The Locations view shows one New York registration; its `Needs attention` status concerns separate filing setup. The owner confirmed private certificate storage and printing/display, and the DTF-17 application records `09/18/2026`; confirmation from the actual Certificate and the assigned filing frequency, a filing process, deployment of the newly staged Production values, and a real taxed Checkout remain open.
