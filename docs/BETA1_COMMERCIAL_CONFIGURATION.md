...
- On August 30, 2026, the owner approved `PERMITEXT_STRIPE_TAX_MODE=automatic` and `PERMITEXT_STRIPE_PRICE_TAX_BEHAVIOR=exclusive`, with the exact web purchase disclosure `$20/month plus applicable taxes shown by Stripe.` `exclusive` means applicable tax is added above the $20 base price; `inclusive` would absorb applicable tax inside the displayed $20 price and reduce retained revenue. On September 2, the owner reported possession of the actual New York Certificate of Authority, approved and submitted Product tax code `txcd_10701400`, `Website Information Services - Business Use`, added the New York Stripe Tax registration with collection starting immediately, and staged both activation values in Vercel Production. No deployment or customer charge occurred; certificate display/effective-date/filing-frequency evidence, filing setup, exact policy deployment, and real taxed-Checkout evidence remain open in [BETA1_STRIPE_TAX_DECISION_RECORD.md](./BETA1_STRIPE_TAX_DECISION_RECORD.md).
...
- Stripe confirmed that the New York registration was added successfully and that Sales tax collection starts immediately. The Locations view shows one New York registration; its `Needs attention` status concerns separate filing setup. Certificate display/effective-date/filing-frequency evidence, a filing process, deployment of the newly staged Production values, and a real taxed Checkout remain open.
