...
- On August 30, 2026, the owner approved `PERMITEXT_STRIPE_TAX_MODE=automatic` and `PERMITEXT_STRIPE_PRICE_TAX_BEHAVIOR=exclusive`, with the exact web purchase disclosure `$20/month plus applicable taxes shown by Stripe.` `exclusive` means applicable tax is added above the $20 base price; `inclusive` would absorb applicable tax inside the displayed $20 price and reduce retained revenue. The disclosure is prepared locally, but neither activation value is set in Production. New York Business Express shows the Certificate registration as issued on August 28, while receipt/display of the actual certificate, the assigned filing frequency, Product tax-code approval, provider registration, deployment, and real taxed-Checkout evidence remain open in [BETA1_STRIPE_TAX_DECISION_RECORD.md](./BETA1_STRIPE_TAX_DECISION_RECORD.md).
- This source guard does not receive or display the actual New York certificate, select or change the live Stripe Product tax code, create a Stripe Tax registration, decide where Permitext has collection obligations, or verify a real taxed Checkout. Official Stripe guidance favors a specific category over the current generic electronic-services code; `Website Information Services - Business Use` is the closest documented fit for Permitext's principal Beta 1 offering and remains a no-change recommendation pending owner approval. The 5% tax line remains an unresolved downside reserve, not a tax rate.
...
- The Product currently uses Stripe tax code `txcd_10000000`, shown as `General - Electronically Supplied Services`. Stripe says that generic code should yield to a specific software, cloud, or website code when one fits. The closest documented fit is `txcd_10701400`, `Website Information Services - Business Use`, because Permitext provides professional online code information, search, and comparison through SaaS. This recommendation is recorded for owner approval; the live Product was not changed.
- The Stripe Tax collecting-locations view showed no live transactions and no collecting-location row. That view also states it does not track the home jurisdiction, so it is not treated as proof of New York provider registration. Business Express separately confirms the state registration is issued; actual-certificate receipt/display, Stripe provider registration, and a real taxed Checkout remain open.
