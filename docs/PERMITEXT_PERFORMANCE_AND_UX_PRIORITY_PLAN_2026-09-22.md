...
**Status:** Account-scoped preferences, pre-decode Search filtering, scope-aware result caching, enabled Browse projections and speculative warmup exclusion are implemented locally. Actual-helper contracts and generic iOS compilation pass. Exact-source metadata navigation, guarded enable/open prompts, Settings category switches and Browse/Search recovery are implemented locally; defaults still enable all sources until explicitly changed. Rendered acceptance remains open. Physical acceptance and web parity remain open. Exact category identities and full-catalog versus enabled-projection boundaries are documented in `performance/PERF_18_ACTIVE_SOURCE_IMPLEMENTATION.md`. Production download work under PERF-17 remains open; no content removal approved.
