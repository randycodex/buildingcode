...
Status: **HISTORICAL PARTIAL PASS — Reader and continuity observations retained; Research authorization consumed in a failed attempt; build 51 superseded by build 52.** See the [failure result](./PERMITEXT_RESEARCH_FAILURE_RECOVERY_2026-09-03.md#observed-attempt), [build 52 release and device observations](./PERMITEXT_RESEARCH_RECOVERY_RELEASE_2026-09-03.md), and [separate build 52 live result](./PERMITEXT_BUILD52_RAMP_LIVE_TEST_2026-09-03.md). Historical release identifiers below do not identify today's backend or authorize another test.
...
Status: **CONSUMED — the authorized build-51 attempt failed; no retry or further paid turn is authorized.**
...
Recorded outcome: the request began September 3 at 21:30:44 UTC and returned HTTP 502 after 105,018 ms with `RESEARCH_VERIFICATION_FAILED`; no answer was delivered. The iPhone displayed the misleading interruption message. The actual cost and persisted allowance-ledger outcome for that attempt were not independently verified. The [failure-recovery record](./PERMITEXT_RESEARCH_FAILURE_RECOVERY_2026-09-03.md#observed-attempt) controls these findings. The later build-52 success used a different, separately authorized Unassigned conversation; it does not turn this failed Project-context attempt into a pass or renew either authorization.
