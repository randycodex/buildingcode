...
Class A retains the 24 reviewed general-meaning occurrences and now adds four exact §27-2045 operative links, each presenting the complete general meaning and a separate local companion. All nine defining occurrences remain plain. The main HMC book has 43 eligible records and nine withheld meanings; the complete registry has 5,664 entries, preserving all 5,663 original IDs, full bodies and sources. Local verification passes 131 JavaScript tests, 443 browser checks, all 37 native occurrence ranges and the 41.596-second rendered two-source popup check. Parent inspected the final native text/citations/Close screenshot. The generic build is held with approximately 558 MiB free; storage guard audit → clean → audit removed no targets. These companion changes remain unpublished. Phone remains direct development build 82. See [the Class A review](PERMITEXT_HMC_CLASS_A_SCOPE_REVIEW.md) for scope, compatibility and evidence boundaries.
