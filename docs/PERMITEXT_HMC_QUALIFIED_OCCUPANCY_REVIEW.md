...
## Hotel binding candidate

The source-guarded `bind-hmc-hotel.mjs` now re-extracts item 12, preserves the original ID/body/source and proposes only §27-2041 with the source-attested plural `hotels`. The compiled binding is tested inside the full registry against the complete inventory: one ordinary plural link, no link inside apartment hotels, no link for the rule-defined exempt luxury hotel, and no links in defining prose. Source/input drift tests reject altered chapters, bodies, aliases, identity, duplicate and missing entries. All 65 HMC tests pass (`/tmp/permitext-hmc-hotel-binding-tests.log`). This module is not yet wired into generation; Hotel remains withheld in published data. Generator integration and browser/native verification remain pending.
