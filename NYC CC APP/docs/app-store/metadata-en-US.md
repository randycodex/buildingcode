...
Preparation checkpoint, updated September 7: the owner-set password passed standard web sign-in without an email code, the owner sees credentials in Apple's form, and the designated account now has explicit no-charge Pro access verified on web. App version and subscription review notes are saved and retained after reload. Native login and backend-feature acceptance remain step 2; no final submission claim is made. See the [forms/grant receipt](../../../docs/PERMITEXT_APP_STORE_FORMS_2026-09-07.md) and [reviewer-access checkpoint](../../../docs/PERMITEXT_REVIEWER_ACCESS_2026-09-07.md). Reconcile final device instructions before submission. Keep the reviewer grant active through acceptance/review, then revoke when no longer needed.
...
**Internal submission blocker:** verify the reviewer subscription and backend-feature path on the actual final candidate. The supplied account's complimentary backend Pro grant is now explicit and disclosed in Apple notes; it is separate from a purchase. Do not describe a Sandbox purchase as activating Production backend Pro or weaken the transaction-environment guard. The completed staging Sandbox lifecycle does not establish this final-candidate path.
