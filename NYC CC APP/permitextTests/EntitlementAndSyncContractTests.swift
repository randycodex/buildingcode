...
final class ReaderDefinitionContractTests: XCTestCase {
    ...
    func testHousingMissingInventoryStaysWithheldAndPreservesCompleteGroups() throws {
        ...
        for term in missing {
            ...
            XCTAssertEqual(entry.applicability, ["Person", "Self-closing door", "Unoccupied dwelling unit", "Fireproof", "Nonfireproof", "This code"].contains(term) ? "definition-chapter" : "review-required", term)
            XCTAssertEqual(entry.source.file, "2026-enacted-administrative-code/chapters/30000077.html")
            XCTAssertEqual(entry.source.anchor, "section-31001849")
        ...
        for chapter in ["1", "2", "3", "4", "5"] {
            ...
            XCTAssertTrue(registry.entries(for: context).allSatisfy { !missing.filter { !["Person", "Self-closing door", "Unoccupied dwelling unit", "Fireproof", "Nonfireproof", "This code"].contains($0) }.contains($0.term) })
        }
    }
