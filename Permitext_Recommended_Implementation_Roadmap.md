...
**Status: expanded draft diagnostic program implemented; public gate remains blocked.** Permitext now has a separate retrieval dataset mapped to the canonical selected-evidence Research cases, a free deterministic evaluation harness, and a generated knowledgeable-human review packet. The current twenty cases remain drafts with zero approved retrieval gates. At diagnostic depth 12, the current implementation recalls the expected sections and passages in all nineteen answerable draft scenarios and correctly identifies the remaining scenario as missing section context. The draft set now covers Mechanical Code 404.1, a prior-code building that crosses the Administrative Code's more-than-110-percent floor-surface-area threshold, a prior-code structural alteration that crosses the Building Code's wind-surface-area trigger, re-establishment of a former occupancy after an intervening change, the complete six-condition mercantile/business Certificate-of-Occupancy exception, existing-installation plumbing repair boundaries, the conjunctive percentage-and-area exception for prior-code roof recovering, the five-condition partial Certificate of Occupancy provision for certain pre-1938 buildings, a fire-district question governed by official Appendix D map images, a referenced plumbing table, and an outside-library boundary question that mentions Buildings Bulletins, the Zoning Resolution, and the Housing Maintenance Code. Every answerable draft currently has 100% expected-section and expected-passage recall at depth 12.
...
3. Expand deterministic retrieval coverage for mechanical systems, existing-building questions, non-text tables or maps, and outside-agency boundaries, including explicit review and immutable attachment for selected official images. Keep Buildings Bulletins outside the current library with explicit source-boundary disclosure only.
...
3. Mechanical, prior-code floor-surface-area, prior-code structural wind,
   ...
   prior-code roof recovering, pre-1938 partial Certificate of Occupancy,
   ...
   exact bytes, hashes, and metadata with the Research source and immutable
   answer record. Unreviewed, forged, stale, oversized, unsupported, or
   incomplete-inventory visual inputs are rejected. Complete official tables retain
   ...
   authorities receive official source links. Buildings Bulletins are now an
   explicit scope exclusion: source-boundary disclosure remains, but Bulletin
   ingestion and retrieval are outside the current implementation scope.
   Knowledgeable-human review of the visual scenario, paid live-model
...
4. The answerable-versus-insufficient-evidence distinction remains a hard case
   gate, and all nineteen answerable draft cases currently achieve complete
